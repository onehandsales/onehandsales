...
- 현재 언어 선택 UI에 노출하는 URL locale slug는 `ko`, `en-us`, `en-ca`다. `ja`, `en-gb`, `en-sg`, `en-au`는 추후 확장 후보로만 보류한다.
...
- legacy redirect는 기존 `/companies`, `/contacts`, `/products`, `/deals`, `/schedules`, `/meeting-notes`, `/business-cards`, `/import`, `/trash`, `/more`와 각 상세/생성/`new/full` 경로를 대응되는 `/app/*`로 이동시킨다. `/schedules/week`는 `/app/schedules/week`로 이동하고, `/import/review/:importJobId`는 `/app/import/review/:importJobId`로 이동한다. `/settings` legacy route는 제거됐으며 compatibility redirect를 유지하지 않는다.
...
- 현재 로그인 화면 노출 provider는 Google, LINE, Apple이다. Google 가입/로그인은 QA 통과 상태이며, LINE/Apple 실제 provider smoke는 Supabase/provider 운영 설정과 secret 준비 상태에 따라 별도 확인하거나 환경 `N/A`로 기록한다.
- Kakao는 로그인 기능에서 제거했으며 runtime provider로 노출하지 않는다. Prisma enum의 `KAKAO` 값은 legacy 호환용으로만 남긴다.
