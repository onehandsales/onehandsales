...
export function PublicSiteHeader({ onLogin }: PublicSiteHeaderProps) {
  // 1. 처리 흐름에 필요한 navigate 값을 준비한다.
  const navigate = useNavigate();
  // 2. 처리 흐름에 필요한 { copy } 값을 준비한다.
  const { copy } = usePublicSiteLanguage();
  // 3. 처리 흐름에 필요한 publicSitePath 값을 준비한다.
  const publicSitePath = usePublicSitePath();
  // 4. 처리 흐름에 필요한 [isMobileMenuOpen, setIsMobileMenuOpen] 값을 준비한다.
  ...
  useEffect(() => {
    ...
    // 2. 이후 단계에서 사용할 previousOverflow 값을 준비한다.
    const previousOverflow = document.body.style.overflow;
    // 3. 이후 단계에서 사용할 desktopMediaQuery 값을 준비한다.
    const desktopMediaQuery = window.matchMedia("(min-width: 1280px)");
    // 기능 : Escape 입력 시 메뉴를 닫습니다.
    // 4. 이후 단계에서 사용할 closeOnEscape 값을 준비한다.
    const closeOnEscape = (event: KeyboardEvent) => {
      if (event.key === "Escape") {
        setIsMobileMenuOpen(false);
      }
    };
    // 기능 : 데스크톱 화면에서 메뉴를 닫습니다.
    // 5. 이후 단계에서 사용할 closeOnDesktop 값을 준비한다.
  ...
  // 6. 이후 단계에서 사용할 handleLogin 값을 준비한다.
  const handleLogin = () => {
    if (onLogin) {
      onLogin();
      return;
