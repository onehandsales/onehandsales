...
export function AdminAuthProvider({
  ...
  // 1. 화면 상태와 동작에 필요한 [user, setUser] 값을 준비한다.
  const [user, setUser] = useState<AdminMe | null>(null);
  // 2. 화면 상태와 동작에 필요한 [role, setRole] 값을 준비한다.
  const [role, setRole] = useState<AdminAuthRole | null>(null);
  // 3. 화면 상태와 동작에 필요한 [isPending, setIsPending] 값을 준비한다.
  const [isPending, setIsPending] = useState(false);
  // 4. 화면 상태와 동작에 필요한 [error, setError] 값을 준비한다.
  const [error, setError] = useState<string | null>(null);

  // 기능 : 전달받은 access token으로 서버의 현재 관리자 정보를 검증합니다.
  // 5. 화면 상태와 동작에 필요한 verifyAdminMe 값을 준비한다.
  ...
  // 6. 화면 상태와 동작에 필요한 logout 값을 준비한다.
  ...
  // 7. 화면 상태와 동작에 필요한 clearError 값을 준비한다.
  const clearError = useCallback(() => {
    setError(null);
  }, []);

  // 기능 : access token을 전달해 Admin Web 로그인을 시도합니다.
  // 8. 화면 상태와 동작에 필요한 loginWithAccessToken 값을 준비한다.
  ...
  // 9. 화면 상태와 동작에 필요한 value 값을 준비한다.
