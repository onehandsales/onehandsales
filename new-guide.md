...
이 문서는 OneHand CRM 피벗의 제품 방향을 정리한다. 이후 `README.md`, `AGENT`, `BE`, `FE` 문서를 갱신할 때 이 문서를 우선 기준으로 삼는다.
