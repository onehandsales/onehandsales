...
- Deal next action
- DealActivity CRUD
- API 요약: `TODO/MVP-STARTER_PLAN/COMMON/API-SPEC/G06-G12-CORE-DOMAIN-API.md`
- 엔드포인트 구현 계약: `TODO/MVP-STARTER_PLAN/COMMON/API-SPEC/G06-G12-ENDPOINT-CONTRACT.md`
...
- 경로: `/deals` 또는 홈 `/` 일부
- 표시 순서: 딜명 -> 회사/담당자 -> 단계 -> 금액 -> 가능성 -> 다음 행동 -> 마감일
- 주요 UI: 단계 탭, 검색, 필터, 빠른 생성 버튼, 딜 row/card

#### 딜 빠른 등록 modal

- 필수 입력: 딜명, 금액
- 선택 입력: 회사, 거래처(담당자), 제품, 단계, 가능성, 다음 행동, 예상 종료일
- 회사/거래처/제품은 검색 combobox를 사용한다.
...
딜 빠른 생성 중 회사/거래처/제품이 없을 때 modal을 벗어나지 않고 최소 정보로 즉시 생성할 수 있게 한다.

### 화면 구성

- 검색 결과 없음 상태에서 `새 회사 만들기`, `새 거래처 만들기`, `새 제품 만들기` 표시
...
딜의 핵심 요약, 활동 로그, 개인 메모, 관련 정보를 빠르게 확인하고 수정할 수 있게 한다.

### Desktop 화면

- 딜 목록 우측 상세 패널
- 항상 먼저 보이는 정보: 딜명, 회사/거래처, 단계, 금액, 가능성, 다음 행동, 마감일
- 섹션: 기본 정보, 활동 로그, 일정/회의록 placeholder, 제품/연결 정보, 개인 메모
