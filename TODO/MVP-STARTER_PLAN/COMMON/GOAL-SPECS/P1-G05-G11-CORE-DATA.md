...
- 로그인 버튼은 Backend의 `/api/auth/:provider/start`로 이동해 Supabase Auth provider 화면을 연다.
- Supabase Auth callback은 Backend가 처리하고, local User/AuthSession 동기화 후 httpOnly session cookie를 발급한다.
...
- Supabase Auth callback 처리가 동작한다.
- httpOnly session cookie가 발급된다.
- Supabase user와 local User/UserOAuthAccount/UserSetting/AuthSession이 동기화된다.
...
- 주요 UI: 기본 정보, 회사 로그, 연결 거래처/딜/제품 요약
