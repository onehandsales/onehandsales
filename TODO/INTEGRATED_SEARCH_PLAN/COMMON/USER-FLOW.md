...
6. 각 상세 화면은 기존 단건 상세 API를 사용해 최신 상세 정보를 다시 조회한다.
...
|---|---|
| Company | 회사명, 분야명, 지역명 |
| Contact | 담당자명, 이메일, 휴대폰, 회사명, 부서명, 직급명 |
| Product | 제품명, 카테고리명, 상태명 |
| Deal | 딜명, 상태, 회사명, 담당자명 |
| Schedule | 일정 제목, 장소, 메모, 연결 딜명 |
| MeetingNote | 회의록 본문 요약 필드, 다음 계획, 필요 행동, 회사/담당자/제품/딜 스냅샷 |
...
현재 User Web router에는 일정 단건 route가 없으므로 Frontend 후속 goal에서 `/schedules/:scheduleId` route와 상세 화면을 추가해야 한다.

## 5. 예외 흐름

- `q`가 두 글자 미만이면 Backend는 빈 groups를 반환한다.
- 검색 결과가 없으면 Frontend는 빈 상태를 표시한다.
- 인증이 없으면 401을 반환한다.
- `types`에 허용되지 않는 값이 있으면 400 domain validation error를 반환한다.
