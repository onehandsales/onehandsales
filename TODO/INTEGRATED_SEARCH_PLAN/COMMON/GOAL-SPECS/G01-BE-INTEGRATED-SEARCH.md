...
Backend에 `GET /api/search`를 추가해 User Web이 현재 사용자 소유 데이터를 한 번에 검색할 수 있게 한다.
...
- 도메인별 결과는 `limit` 이하로 반환된다.
- Backend `pnpm typecheck`가 통과한다.
...
- 검증: `pnpm typecheck`, `pnpm lint`, `pnpm test -- search`
