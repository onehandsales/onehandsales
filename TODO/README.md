...
- 현재 즉시 실행 가능한 문서 작업은 `TODO/API_SPEC_TEMPLATE_NORMALIZATION_PLAN`의 G99 최종 검토다. QA에서 발견된 S0/S1/S2 결함이나 새 요구사항은 `TODO` 바로 아래에 새 계획 폴더를 만들거나, `TODO/SERVICE_QA_PLAN`의 이슈 로그에서 후속 구현 계획으로 승격한다.
...
- `API_SPEC_TEMPLATE_NORMALIZATION_PLAN`: In Progress / Documentation-only / G06 Completed / G99 Next. API-SPEC 템플릿 누락 보강 대상과 제외 대상을 정리하고 활성 문서부터 정규화하는 계획이다.
