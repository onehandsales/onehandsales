...
| DataImport | templates, upload/mapping/confirm/logs, in-memory pre-confirm job |
...
|---|---|---|---|
| DB/Prisma ops | `RQA-005` Blocked 이력 | DB target, migration status, seed/generate 정책 | 사용자 결정 전 보류 가능 |
...
| ImportJob persistence | pre-confirm job in-memory | table, TTL, cleanup, resume API, 개인정보 보관 정책 | 아직 구현 금지 |
...
| Notification | Backend 없음 | domain, delivery channel, settings, rate limit | 아직 구현 금지 |
| MeetingNote provider log | raw/provider log table 없음 | transcript retention, audit, privacy policy | 아직 구현 금지 |
| Admin operation | `/admin/api/me` 외 없음 | masking, raw access reason, audit log, support flow | 아직 구현 금지 |
| Payment/subscription | 없음 | plan, entitlement, payment provider, admin ops | 별도 큰 계획 필요 |
| Product analytics | 없음 | event taxonomy, data pipeline, privacy | 별도 계획 필요 |
...
1. 제품화 UX에서 실제 필요한 API gap인지 확인한다.
2. 개인정보/보안/운영 정책이 얽힌 후보를 먼저 정책으로 확정한다.
3. release follow-up API는 작은 계약 단위로 쪼갠다.
4. ImportJob, Notification, Admin, Payment는 각각 별도 계획으로 분리한다.
