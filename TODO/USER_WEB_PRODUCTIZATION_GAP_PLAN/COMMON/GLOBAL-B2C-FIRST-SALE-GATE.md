...
| Data reliability | migration, seed, backup/restore, import job 유실, provider log, 장애 대응 기준이 있다. | ImportJob persistence, Google Calendar token encryption/redaction, callback/redirect QA, 06 범위 DB target/migrate/seed gate, 07 MeetingNote provider call log subset은 완료. backup/restore, Admin provider audit/retention, 장애 대응 기준은 별도 계획으로 남긴다. |
...
| Data reliability/DB gate | 06 범위 DB/Prisma gate, ImportJob persistence, 07 MeetingNote provider call log subset은 완료. 운영 DB 적용 절차, Admin provider audit/retention, backup/restore 기준은 판매 신뢰와 연결된다. |
