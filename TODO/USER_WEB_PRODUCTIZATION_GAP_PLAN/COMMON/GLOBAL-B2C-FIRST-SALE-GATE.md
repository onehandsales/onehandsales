...
| Retention | 다음 행동, 일정, 딜 지연, 회의록 follow-up을 사용자가 놓치지 않는다. | Notification이 첫 판매 필수인지, 홈/일정 UX로 대체 가능한지 결정한다. |
...
| Retention/notification decision | 알림을 만들지 않더라도 다음 행동을 놓치지 않게 하는 대체 UX가 필요하다. |
