...
- [ ] 08 운영 DB migration deploy와 Google/LINE/Apple 실제 provider smoke는 운영 확인 단계로 남음
...
| Global UX | 판매 국가 기준 언어, 날짜/시간, 통화, 전화번호, 주소, UX writing이 어색하지 않다. | 08에서 `/app` `ko-KR`/`en` i18n, 사용자 국가/기본 통화, Product/Deal currency, Contact KR/US phone, Company country/region/address, Import/Export localization, Google/LINE/Apple auth를 구현했다. 추가 시장별 UX writing과 provider smoke는 운영 확인으로 남긴다. |
...
| Data reliability | migration, seed, backup/restore, import job 유실, provider log, 장애 대응 기준이 있다. | ImportJob persistence, Google Calendar token encryption/redaction, callback/redirect QA, 06 범위 DB target/migrate/seed gate, 07 MeetingNote provider call log subset, 08 migration 파일 작성과 Prisma 검증은 완료. 08 운영 DB deploy, backup/restore, Admin provider audit/retention, 장애 대응 기준은 별도 계획으로 남긴다. |
...
| Data reliability/DB gate | 06 범위 DB/Prisma gate, ImportJob persistence, 07 MeetingNote provider call log subset, 08 migration 작성/검증은 완료. 08 운영 DB deploy, Admin provider audit/retention, backup/restore 기준은 판매 신뢰와 연결된다. |
| Auth provider operation QA | Google/LINE/Apple 구현은 완료됐지만 실제 provider smoke는 Supabase/provider secret 설정 후 판매 전 확인해야 한다. |
