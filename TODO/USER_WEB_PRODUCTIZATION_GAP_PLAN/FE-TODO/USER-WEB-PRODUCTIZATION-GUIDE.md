...
| `/app/settings` | profile/devices, 국가/앱 언어/기본 통화, Google Calendar 연결/캘린더 선택/연결 해제, Gmail/Microsoft follow-up provider 연결/재연결/해제, SMS 발신번호 인증 UX 구현 | 첫 판매 전 계정 삭제/데이터 삭제/구독 상태 UX가 충분한가. Gmail/Microsoft actual send smoke closeout은 완료 상태인가 | global settings는 08 완료. Account/data request는 11 완료. Payment subscription UX는 12와 연결. Google Calendar 설정은 `NBA-015` 완료. Follow-up provider settings와 provider smoke closeout은 05/PRE12 완료 |
