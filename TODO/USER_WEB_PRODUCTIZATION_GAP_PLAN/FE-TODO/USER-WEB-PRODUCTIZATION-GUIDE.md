...
| `/app` route analytics | `AppShell`에서 보호된 `/app` route 진입을 routeKey allowlist 기반으로 전송 | 사용자는 analytics 전송 여부를 보지 않는다. Admin dashboard와 billing conversion UI는 아직 별도 | `09_PRODUCT_ANALYTICS` 완료. Admin/Billing 연결은 11/12 |
...
| `/app/meeting-notes` | 수동/AI/STT draft/딜 연결, 상세 AI 후속 작업, 다음 행동 후보 편집 저장, follow-up draft 수정/복사 구현 | 회의록 목록에서도 최신/다음 행동 맥락이 충분히 보이는가. 상세 AI 후보는 자동 저장/자동 발송하지 않는가 | `NBA-004` detail subset, `NBA-011` provider log subset 완료. 목록 summary, 자동 발송/알림, Admin audit는 후속 |
| `/app/business-cards` | OCR/upload/confirm, KR/US phone validation 구현 | provider 실패와 모바일 촬영 UX가 자연스러운가 | `NBA-005` 후보. 08 phone 완료 |
...
- 완료된 Product Analytics 범위를 넘어서는 Admin analytics dashboard, billing/paywall/churn runtime event, 모바일/PWA 세부 analytics event는 11/12/10 계약 없이 확장하지 않는다.
