...
Frontend는 provider 로그인과 Supabase callback 처리를 담당한다. Backend는 Supabase access token을 검증하고, 내부 `User`, `UserOAuthAccount`, `UserSetting`, `AuthDevice`, `AuthSession`을 생성하거나 갱신한다.
...
User Web과 Admin Web은 각 앱의 기존 env 패턴을 따른다. 필요한 값은 다음 범위다.

| 이름 | 설명 |
|---|---|
| `VITE_API_URL` 또는 기존 API URL env | Backend API base URL |
| `VITE_SUPABASE_URL` | Supabase project URL |
| `VITE_SUPABASE_ANON_KEY` | Supabase anon key |
| `VITE_SUPABASE_REDIRECT_URL` | Supabase OAuth callback URL |
| `VITE_AUTH_MODE` | 선택. `supabase`를 기본값으로 하고 local mock은 명시적으로만 사용 |

실제 env 이름은 각 FE 앱의 `src/lib/env.ts` 패턴에 맞춘다.
...
`credentials: "include"`를 반드시 사용한다. 이 요청의 응답에서 httpOnly refresh cookie가 설정된다.
...
- `user.role`이 `ADMIN`이면 Admin Web 접근 가능 후보가 된다. 최종 Admin Web 접근은 `/admin/api/me`로 다시 확인한다.

## 7. Device Slot Conflict
...
```json
{
  "accessToken": "<new_backend_app_access_token>",
  "accessTokenExpiresAt": "2026-06-10T12:15:00.000Z",
  "refreshToken": null,
  "user": {
    "id": "uuid",
    "supabaseUserId": "supabase-user-id",
...
`credentials: "include"`를 사용해서 refresh cookie clear를 반영한다.
...
```json
{
  ...
  "permanentDeleteAt": "2026-07-10T12:00:00.000Z"
}
```

Frontend 처리:

- 성공 시 memory access token 제거
- Supabase Auth `signOut`
- 로그인 화면 또는 탈퇴 완료 화면으로 이동
...
## 16. Current Backend Prerequisite
