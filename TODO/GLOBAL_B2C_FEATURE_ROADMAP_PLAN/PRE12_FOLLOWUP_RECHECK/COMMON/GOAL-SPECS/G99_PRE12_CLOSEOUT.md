...
상태: Pending / final classification 반영됨
목표: 선택된 pre-12 후속 작업을 닫고, 12 착수 전 보류 목록을 상위 문서에 반영한다.

## 1. 선행 조건

- G00 완료 및 `COMMON/FINAL-CLASSIFICATION.md` 작성
- 필요한 경우 G01, G05 완료
- G02~G04 후보가 post-12 보류 대상으로 분류 완료
...
- 12 착수 전 남은 보류 목록 확정
- `COMMON/FINAL-CLASSIFICATION.md`의 12 전에 할 것 5개 중 실제 closeout 대상 반영

## 3. 제외 범위

- 12 Billing 구현
- post-12 새 TODO 생성
- 12 완료 후 `POST12_*` 재검토 문서 생성
- UX/UI 전체 polish 계획 생성

## 4. 완료 기준

- 12 전 처리한 항목과 12 이후로 보류한 항목이 표로 남는다.
- `PRE12-F04`, `PRE12-F31`, `PRE12-F32`, `PRE12-F33`, `PRE12-F34` 외에는 12 전 작업으로 남기지 않는다.
