...
| 후보 | PRE12 ID | 분류 | 판단 |
| --- | --- | --- | --- |
| `/app` `ja`, `zh-TW` 번역과 시장별 UX writing | `PRE12-F17` | post-12-seed | public/auth locale과 다르다. `/app`은 `ko-KR/en`만 완료다. |
| `zh-CN` 중국 본토 지원 | `PRE12-F18` | defer / 시장 진입 결정 필요 | 08 결정 로그에서 현재 후보가 아니라고 분리했다. |
| 전 세계 국가/통화/전화번호 확장 | `PRE12-F19` | post-12-seed | 현재 구현은 KR/US, KRW/USD로 제한되어 있다. |
| USD cent/minor unit과 금액 정밀도 | `PRE12-F20` | billing-blocked / amount-precision 후속 | Product/Deal amount model과 12 Billing money model을 함께 정해야 한다. |
| 국가별 상세 주소 검증 | `PRE12-F21` | billing-blocked / post-12-seed | Company address는 자유 입력이다. 세금/청구/약관 정책이 먼저 필요하다. |
| Contact 개인 주소 | `PRE12-F22` | post-12-seed / CRM 확장 | 08은 Contact address를 명시적으로 제외했다. |
...
| app i18n 직접 keying, Settings OAuth 계정 라벨, bundle 최적화 polish | `PRE12-F25` | post-12-seed / UXUI quality | 08 blocker가 아니라 제품화 유지보수 후보로 둔다. |
