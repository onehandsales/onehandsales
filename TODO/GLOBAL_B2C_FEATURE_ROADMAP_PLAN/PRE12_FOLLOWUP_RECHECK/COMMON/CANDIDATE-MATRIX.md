...
최종 3분류 정본은 `FINAL-CLASSIFICATION.md`다. 아래 후보 표의 `기본 분류`는 세부 성격을 남기기 위한 값이고, 12 착수 전 실행 여부는 `FINAL-CLASSIFICATION.md`의 12 전에 할 것 / post-12 / billing 충돌 분류를 따른다.
...
12 전에 새 API, DB migration, User Web/Admin Web 기능 구현으로 올릴 후보는 없고, 선택된 12 전 closeout 5개는 BEFORE_12에서 모두 닫혔다.
...
| --- | --- | --- | --- | --- | --- |
| PRE12-F01 | 다음 행동 reminder | 02 제외, 06에서 재설계 언급 | Notification source는 `SCHEDULE`, `DEAL`만 있다. 06의 다음 행동 범위는 Notification reminder가 아니라 DealActivity `NEXT_ACTION_CREATED/COMPLETION_CHANGED` 기록이다. | post-12-seed / notification policy | 12 전 구현하지 않는다. Notification source/setting/scheduler 정책은 post-12에서 재검토한다. |
...
| PRE12-F41 | Marketing opt-in and communication consent policy | USER/NEXT 10 반영 기준, 09 growth/analytics 재대조 | Source plan에는 Marketing opt-in이 billing/paywall/churn runtime event와 함께 12 범위로 남아 있다. 현재 코드의 `FollowUpConsentNotice`는 사용자가 직접 보내는 follow-up EMAIL/SMS 고지 확인이고, public contact form의 `marketingAgreement`는 lead/contact submission field다. 사용자 계정 단위 marketing opt-in preference, withdrawal, campaign channel consent, consent audit snapshot, billing/growth analytics event linkage는 없다. | billing-blocked / growth-compliance | 12 Billing/growth/privacy policy와 consent/withdrawal/audit 기준이 확정되기 전 구현 금지. 기존 notification settings나 `FollowUpConsentNotice`에 끼워 넣지 않는다. |
| PRE12-F42 | BusinessCard mobile advanced camera preview/crop | 10 SCOPE 제외, UXUI review, 실제 FE 코드 재대조 | 10은 native file/camera picker 기반 `input type=file`, `accept="image/*"`, `capture="environment"`로 완료됐다. 실제 User Web에는 BusinessCard 전용 `getUserMedia`, `ImageCapture`, custom camera preview/crop/canvas capture flow가 없다. | post-12-seed / mobile advanced capture | 모바일 사용량/기기 QA/접근성/파일 처리 정책이 확인될 때 별도 advanced capture 계약으로 판단한다. 10을 재오픈하거나 단순 file input을 임의 대체하지 않는다. |
| PRE12-F43 | Server draft and media/raw storage policy | 10 SCOPE/G04 제외, 실제 BE/FE 코드 재대조 | 10 local draft는 FE IndexedDB primary/localStorage fallback, 24시간 TTL, restore/discard UX로 닫혔다. `UserDraft`, `MobileDraft`, `/api/drafts/*`, audio/image binary, transcript 전문, provider raw response 저장 API/table은 없다. | defer / trust-policy / post-12-seed | retention, 삭제권, account deletion 실제 처리, encryption, quota, raw access audit, redaction 기준이 확정되기 전 구현 금지. 10 local draft 완료 범위에 server draft나 blob/raw 저장을 끼워 넣지 않는다. |
