...
이 문서는 `PRE12_FOLLOWUP_RECHECK`를 `/goal` 단위로 실행할 때의 순서를 고정한다. 현재 확정 구현 goal은 없다. 2026-08-07 기준 최종 분류는 `FINAL-CLASSIFICATION.md`를 따른다.
...
| G01 | `G01_06_SCOPE_GUARD_AND_CODE_AUDIT` | Not selected / covered by final classification | 06 완료 결과가 Notification/MeetingNote/Follow-up/list summary/activity lifecycle 범위를 잘못 넓히지 않았는지 확인하는 audit 후보였고, 최종 분류에서는 별도 12 전 작업으로 선택하지 않는다. | 기능 구현 금지 |
| G02 | `G02_NEXT_ACTION_REMINDER_CONTRACT` | Classified post-12 | 다음 행동 reminder는 12 전 구현하지 않고 post-12 Notification policy 후보로 남긴다. | pre-12 구현 금지 |
| G03 | `G03_MEETING_NOTE_FOLLOW_UP_REMINDER_CONTRACT` | Classified post-12 | 회의록 follow-up reminder와 자동 발송 후보는 post-12로 분리한다. | pre-12 구현 금지 |
| G04 | `G04_RECORD_SUMMARY_CONTRACT` | Classified post-12 | NBA-003 Company/Contact/Product/generic summary/timeline은 2026-08-06 A 결정으로 post-12 seed다. MeetingNote list summary도 post-12-seed로 유지한다. | pre-12 구현 금지 |
...
| 후보 | 처리 방식 |
| --- | --- |
| `PRE12-F04` | 운영 Gmail/Microsoft provider smoke 결과 기록 |
| `PRE12-F31` | 10 Mobile Field Use 문서 체크리스트 정합성 정리 |
| `PRE12-F32` | User Web route/architecture 문서 정합성 정리 |
| `PRE12-F33` | 11 Admin Operation 문서 체크리스트/goal index 정합성 정리 |
| `PRE12-F34` | Admin Web architecture/legacy route 정합성 정리 |

후속 후보 기준으로 위 5개 외에는 12 전 구현 또는 문서 closeout 대상으로 올리지 않는다. G01 audit 후보는 `FINAL-CLASSIFICATION.md`의 후보 분류에 흡수하고 별도 12 전 작업으로 남기지 않는다. G99는 선택된 pre-12 후보 closeout 이후 상위 문서 반영 여부를 점검하는 메타 closeout이므로 후보 분류에 넣지 않는다.

## 3. 선행 조건

- G00 분류 결과는 `FINAL-CLASSIFICATION.md`를 따른다.
- 12 전 처리 대상 5개 외에는 어떤 후보도 구현하지 않는다.
...
- G02, G03은 2026-08-07 최종 분류에서 post-12로 보냈으므로 12 전 병렬 구현 대상이 아니다. G04의 NBA-003 분기는 2026-08-06 A 결정으로 pre-12 구현 계약화 대상에서 제외됐고, NBA-004 MeetingNote list summary는 post-12-seed로 유지한다.
