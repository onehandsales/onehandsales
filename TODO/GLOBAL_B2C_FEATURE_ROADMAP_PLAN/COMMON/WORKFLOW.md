...
12 완료 후에는 `COMMON/POST-12-REVIEW-AND-FOLLOWUP.md`를 따른다.

1. 12 완료 기록과 실제 BE/FE/Prisma 변경을 확인한다.
2. 01~12 폴더 전체를 다시 읽고 완료/미완성/후속/제외 상태를 재분류한다.
3. `TODO/NEXT_BACKEND_API_BACKLOG_PLAN`과 `TODO/USER_WEB_PRODUCTIZATION_GAP_PLAN`을 함께 읽고 잔여 후보를 대조한다.
4. 실제 코드 기준으로 오래된 문서 표현과 최신 구현 상태의 충돌을 표시한다.
5. 새 작업이 필요하면 기존 01~12 완료 폴더를 재개하지 않고 새로운 TODO 폴더를 만든다.
6. UX/UI 디자인 유지보수는 post-12 재검토와 필요한 후속 작업 분류가 끝난 뒤 별도 계획으로 진행한다.
