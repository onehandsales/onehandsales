...
- `AGENT/SOFTWARE_AGENT/FRONT_AGENT/ARCHITECTURE/FRONTEND.md`
...
- 현재 Settings 화면의 `en-US`, `en-GB`, `ja-JP`, `zh-TW` 옵션은 08 1차 범위와 충돌하므로 G02/G03에서 `ko-KR`, `en`만 남긴다.
- `UpdateMyProfileUseCase.normalizePreferredLocale`는 현재 `en-*`, `ja-JP`, `zh-TW`까지 저장할 수 있으므로 08 1차 범위에 맞게 `ko-KR | en`만 반환하도록 좁힌다.
