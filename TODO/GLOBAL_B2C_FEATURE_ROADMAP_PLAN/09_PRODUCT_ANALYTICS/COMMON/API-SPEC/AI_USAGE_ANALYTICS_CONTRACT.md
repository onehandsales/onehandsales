...
09에서는 새 `AiUsageDaily` table을 만들지 않는다. 12 Billing에서 AI plan/quota/paywall을 만들 때 `AiUsageDaily` 또는 `UsageMeter` 중 하나를 source of truth로 반드시 다시 확정한다.
...
12에서 AI quota 또는 plan 제한이 생기면 아래를 확정한다.
