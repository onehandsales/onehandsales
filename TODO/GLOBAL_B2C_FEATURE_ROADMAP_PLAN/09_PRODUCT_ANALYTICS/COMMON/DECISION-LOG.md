...
- Billing/paywall/churn의 최종 상태 전이는 `12_BILLING_SUBSCRIPTION_TAX`에서 만든다.
...
- 12 Billing에서 AI plan/quota/paywall을 만들 때 `AiUsageDaily` 또는 `UsageMeter` 중 하나를 source of truth로 반드시 다시 확정한다.
