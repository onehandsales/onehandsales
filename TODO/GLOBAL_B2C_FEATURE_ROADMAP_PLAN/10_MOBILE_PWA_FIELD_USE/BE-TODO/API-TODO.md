...
- [ ] `POST /api/meeting-notes/stt-draft`가 mobile `MediaRecorder` blob/File 전송을 받을 수 있는지 검증한다.
- [ ] audio missing/type/size validation test를 추가하거나 보강한다.
- [ ] STT/AI provider failure가 safe error로 전달되는지 확인한다.
- [ ] audio binary/blob를 DB에 저장하지 않는다.
- [ ] MeetingNote row는 사용자가 최종 저장하기 전까지 만들지 않는다.

## 5. G05 Notification API 구현 TODO

- [ ] 기존 notification settings/subscription API를 재사용한다.
- [ ] browser push permission을 server setting true와 동일하게 취급하지 않는다.
- [ ] subscription endpoint/key를 log/analytics에 남기지 않는다.
- [ ] 회원가입/약관 동의로 browser push 자동 허용 처리하는 API를 만들지 않는다.
- [ ] marketing/advertising opt-in 신규 API는 10번에서 만들지 않는다.

## 6. G06 Analytics API 구현 TODO

- [ ] 09 Product Analytics collector allowlist에 10번 mobile field event를 추가한다.
- [ ] forbidden payload key validation을 유지한다.
- [ ] client event request에서 userId/deviceId를 받지 않는다.
- [ ] server event `business_card_ocr_failed` payload는 safe code, retryable, provider/model, fileSizeBucket 정도만 허용한다.
- [ ] analytics 저장 실패가 원래 business mutation을 rollback하지 않게 한다.
