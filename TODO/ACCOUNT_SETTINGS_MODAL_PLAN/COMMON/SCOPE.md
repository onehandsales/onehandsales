...
이 계획은 설정 기능의 정보를 새로 설계하는 작업이 아니라, 이미 구현된 `/app/settings`의 설정/요청/연동 기능을 계정 모달의 `Settings` 섹션으로 단계별 이관해 사용 흐름을 짧게 만드는 UX/UI 개선이다.

핵심 기준은 다음과 같다.

- 사용자는 현재 업무 화면을 떠나지 않고 계정 메뉴에서 설정을 열 수 있어야 한다.
- 계정 모달 `Profile`에 이미 있는 읽기 전용 계정 정보는 `Settings`에 다시 복제하지 않는다.
- `/app/settings`는 최종적으로 사용자-facing 설정 page 역할을 내려놓고, 기존 진입점은 Settings modal-open 또는 compatibility bridge로 정리한다.
...
| `/app/settings` hard delete 즉시 처리 | Google Calendar OAuth allowlist가 `/app/settings`를 허용하고 있으므로, BE 변경 없이 바로 삭제하면 callback 복귀가 깨질 수 있다. 기본안은 bridge이며, hard delete는 별도 BE 계획으로 분리한다. |
