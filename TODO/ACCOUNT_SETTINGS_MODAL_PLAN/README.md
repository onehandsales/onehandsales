...
상태: Implemented / Follow-up migration complete / Google Calendar remaining decision
...
- 2026-08-21 기준 `Settings` 섹션에는 app defaults form, `AccountDataRequestsSettingsSection`, `FollowUpDeliverySettingsSection`이 들어가 있다.
...
- `/app/settings`와 legacy `/settings` route는 사용자-facing settings page를 렌더링하지 않고 account settings modal bridge로 동작한다.
...
- 구현 구조는 한 번에 전체 화면을 복제하지 않고, 계정 모달 `Settings` 안으로 section 단위로 이관한다. 첫 이관 대상인 `AccountDataRequestsSettingsSection`과 후속 대상인 `FollowUpDeliverySettingsSection`은 이관 완료됐다.
- Google Calendar 설정은 아직 Settings modal 내부 section으로 이관하지 않았다. 현재는 `/app/settings?googleCalendar=connected`를 `/app/schedules?account=settings`로 bridge해 기존 schedule 화면의 callback handler를 유지한다.
...
| `COMMON/GOAL-SPECS/G02_ACCOUNT_MODAL_INTEGRATION.md` | `/app/settings`의 기능성 section을 Settings modal로 단계별 이관. Account data request와 follow-up delivery는 완료, Google Calendar는 남은 결정 |
| `COMMON/GOAL-SPECS/G03_ROUTE_LINK_QA.md` | `/app/settings` 사용자-facing page 제거와 link/OAuth callback/modal-open QA |
...
3. `G02`에서 `/app/settings`의 account data request section을 계정 모달 `Settings`로 옮겼다.
4. `G03`에서 `/app/settings` link, legacy route, OAuth return path를 modal-open 또는 bridge 흐름으로 검증했다.
5. follow-up delivery settings와 follow-up OAuth callback query 처리를 Settings modal로 이관했다.

남은 결정:

1. `GoogleCalendarSettingsSection`을 Settings modal로 이관할지, 현재처럼 schedules 화면 소유 callback handler와 bridge를 유지할지 결정한다.
2. BE allowlist 변경 없이 `/app/settings` route를 완전히 삭제하지 않는다.
