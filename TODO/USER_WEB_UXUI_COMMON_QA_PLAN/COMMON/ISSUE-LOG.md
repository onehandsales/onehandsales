...
## G06 UX Writing States A11y Closeout Summary
...
- 계정 설정 success 문구를 능동형 `저장했어요.`로 바꿨다.
...
- G06 수정은 문구와 상태 closeout에 한정했고 화면 구조를 Notion/Attio와 pixel-level로 복제하지 않았다.
- 한국어 CRM 업무 화면에 `Customer`, `상품`, `오프더레코드`, `custom object`, `custom field`, `generic export` 노출 후보가 없다.
- 남은 `수 없습니다` 계열은 약관/개인정보 법무 고지 문장이라 UX writing guide의 짧은 업무 문구 교정 범위에서 제외했다.
...
아래는 이슈 심각도 기준의 처리 우선순위다. 실제 `/goal` 실행 순서는 `COMMON/GOAL-WORK-ORDER.md`를 따른다. 현재 G01~G06까지 완료됐다.
...
G06에서 처리했다. 법무/개인정보 장문과 영어권 public/landing locale copy는 업무 화면 UX writing closeout 범위가 아니므로 유지한다. 한국어 CRM 업무 화면의 해요체, loading, empty, error, validation, success 주요 위반은 정리됐다.
