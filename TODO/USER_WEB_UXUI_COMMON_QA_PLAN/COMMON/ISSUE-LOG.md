...
3. G05: UX-006 중 남은 회의록/명함 스캔/Trash 목록 row/table이 compact/tablet layout 또는 명확한 내부 스크롤을 갖는지 정리한다. 회사/담당자/제품은 2026-07-18 G04에서 처리 완료했다.
...
G04에서 회사/담당자/제품 목록/상세/생성 흐름은 처리했다. 담당자 list response에는 연결 딜 수가 없고, 세 도메인 list response에는 실제 최신 활동/다음 행동 summary가 없어 BE/API 후속으로 기록했다.

UX-006 전체는 아직 닫지 않는다. `/app/meeting-notes`, `/app/business-cards`, `/app/trash`는 G05에서 이어서 확인한다.
