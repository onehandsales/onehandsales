...
| 계정 모달 Settings 안의 Billing 후보 | 내 구독 상태, 결제 관리, billing portal 진입 |
...
| `/app?account=settings` 계정 모달 | profile/devices, 국가/앱 언어/기본 통화, provider 연결, account/data request는 구현되어 있다. `/app/settings` 사용자-facing route는 제거됐다. | 구독 상태 UX와 billing portal 진입은 Paddle confirmed scope에서 계정 모달 안의 Billing 후보로 검토한다. |
...
- 기존 계정 모달 Settings에 임시 subscription 상태 문구만 끼워 넣고 실제 billing 계약 없이 판매 가능처럼 보이게 하는 것
