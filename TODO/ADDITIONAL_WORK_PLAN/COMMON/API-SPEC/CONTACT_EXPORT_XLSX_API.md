...
이 문서는 거래처 목록 페이지의 내보내기 버튼에서 사용할 xlsx 다운로드 API 계약을 정의한다.
...
거래처 목록 페이지에서 현재 적용된 검색/필터 조건에 맞는 거래처 전체 데이터를 xlsx 파일로 다운로드한다.

- 페이지네이션 없음
- `page` query는 사용하지 않는다.
- `username` 검색어는 export에도 적용한다.
- `companyId`, `contactDepartmentId`, `contactJobGradeId` 필터는 export에도 적용한다.
- 검색어와 필터가 동시에 있으면 모든 조건을 만족하는 거래처 전체를 내보낸다.
- 필터가 있으면 필터링된 전체 거래처 데이터를 내보낸다.
- 필터가 없으면 현재 사용자의 전체 거래처 데이터를 내보낸다.
...
FE는 거래처 목록 화면의 현재 검색어와 필터 query를 전달하되 `page`는 제거한다.
...
| 컬럼명 | 값 기준 | 설명 |
|---|---|---|
| `회사명` | `Company.companyName` | 거래처가 속한 회사 이름 |
| `거래처명` | `Contact.username` | 거래처 이름 |
| `핸드폰번호` | `Contact.mobile` | 거래처 핸드폰번호 |
| `이메일` | `Contact.email` | 거래처 이메일 |
| `부서` | `ContactDepartment.departmentName` | 거래처 부서명 |
| `직급` | `ContactJobGrade.jobGradeName` | 거래처 직급명 |
...
거래처 목록 화면에서 다음 조건이 적용된 상태라고 가정한다.
...
이때 `page=1`은 export에 적용하지 않는다. xlsx에는 거래처명에 `홍`이 포함되고, 회사/부서/직급 필터를 모두 만족하는 거래처 전체가 들어간다.

xlsx 예시:

| 회사명 | 거래처명 | 핸드폰번호 | 이메일 | 부서 | 직급 | 등록일 |
|---|---|---|---|---|---|---|
...
현재 사용자의 전체 거래처가 `createdAt DESC` 기준으로 내려간다.
...
6. 기존 거래처 목록과 같은 검색어와 필터 조건을 구성한다.
7. `Contact.userId = currentUserId` ownership 조건을 기본으로 적용한다.
8. 페이지네이션 없이 조건에 맞는 전체 거래처를 조회한다.
...
- audit log: 없음
- request id: 사용
- redaction: 거래처명, 핸드폰번호, 이메일, 메모 원문, 개인 비밀 메모 원문 logging 금지
...
- 거래처 목록 화면의 현재 검색어와 필터 query를 export API에 전달한다.
- `page`는 전달하지 않는다.
...
- 파일 컬럼명은 `회사명`, `거래처명`, `핸드폰번호`, `이메일`, `부서`, `직급`, `등록일`로 고정한다.
...
- 필터가 없으면 현재 사용자의 전체 거래처가 xlsx에 포함된다.
- `username` 필터가 있으면 해당 검색어 조건에 맞는 거래처만 포함된다.
- `companyId`, `contactDepartmentId`, `contactJobGradeId` 필터가 있으면 해당 조건에 맞는 거래처만 포함된다.
- `username`, `companyId`, `contactDepartmentId`, `contactJobGradeId`가 함께 있으면 네 조건을 모두 만족하는 거래처만 포함된다.
- xlsx 컬럼명은 `회사명`, `거래처명`, `핸드폰번호`, `이메일`, `부서`, `직급`, `등록일`이다.
- id 계열 값은 xlsx에 포함되지 않는다.
- 정렬은 `createdAt DESC`, `id DESC` 기준이다.
- 다른 사용자의 거래처, 회사, 부서, 직급이 섞이지 않는다.
