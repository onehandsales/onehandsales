...
| `거래처 수` | 연결된 `Contact` 개수 | 해당 회사에 연결된 거래처 수 |
...
- 파일 컬럼명은 `회사이름`, `회사분야`, `회사지역`, `거래처 수`, `등록일`로 고정한다.
- N+1 count 쿼리가 생기지 않도록 회사 조회 단계에서 거래처 수를 함께 집계한다.
