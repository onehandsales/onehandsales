# /goal G05 BE Product Export XLSX

## /goal 입력문

아래 문서를 먼저 읽고, 제품 목록 페이지의 현재 필터 조건에 맞는 제품 데이터를 xlsx로 내려받는 내보내기 API를 추가해줘.
...
`GET /api/products/export/xlsx` API를 추가해 제품 목록 필터 조건에 맞는 전체 제품 데이터를 xlsx 파일로 반환한다.
...
- 기존 Product 단건 API는 변경하지 않는다.
- Company API는 변경하지 않는다.
- Contact API는 변경하지 않는다.
- 범용 Import/Export 모듈, ExportJob, 비동기 queue는 이 goal에서 구현하지 않는다.
- 제품 가격, 메모 원문, 개인 비밀 메모 원문은 조회하거나 파일에 포함하지 않는다.
- `export/xlsx` route가 `:productId` route에 가로막히지 않도록 controller 선언 순서를 확인한다.
