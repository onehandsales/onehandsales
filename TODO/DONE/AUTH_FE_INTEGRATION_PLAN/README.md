...
5. BE는 `[완료] BE-TODO/G01-BE-USER-PROFILE-DEVICES.goal.md`의 완료 결과를 확인한다.
...
- BE 완료 근거: `BE/prisma/schema.prisma`, `BE/prisma/migrations/20260611000000_add_company_domain/migration.sql`, `BE/src/modules/auth`, `BE/src/modules/user`
- BE 검증: `prisma:validate`, `prisma:generate`, `typecheck`, `lint`, `test`, `build` 통과
- FE 인증 연동: 완료
- FE 설정 탭: 완료
- FE 완료 확인일: 2026-06-14
- FE 완료 근거: `FE/user-web/src/features/auth`, `FE/user-web/src/pages/settings`, `FE/admin-web/src/features/auth`
- FE 검증: `FE/user-web` typecheck/lint/build 통과, `FE/admin-web` typecheck/lint/build 통과
...
- `COMMON/API-SPEC/AUTH_USER_API_DETAIL.md`에 모든 API의 계약 상태, transaction, observability 기준이 적혀 있다.
