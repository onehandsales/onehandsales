...
- 기존 record table/list 구조를 유지하되 row density와 업무 판단 정보 표현을 강화하는 방향

목록 밀도 기준:

- 데스크톱 목록은 52~56px 수준 row height와 15개 기본 표시를 장기 목표로 본다.
- 단, page size 변경은 Backend 상수, 응답 `pageSize`, API/DB 문서, 테스트 계약까지 함께 확인해야 하므로 FE 단독으로 바꾸지 않는다.
...
- 목록에서 핵심 비교 정보가 보인다.
- 회사는 회사명, 분야, 지역, 담당자, 진행 딜, 다음 행동 또는 현재 응답에서 가능한 최근 활동이 우선 보인다.
- 담당자는 이름, 회사, 부서/직급, 연락처, 연결 딜, 현재 응답에서 가능한 최근 활동이 우선 보인다.
- 제품은 제품명, 카테고리/타입, 연결 딜 수, 현재 응답에서 가능한 최근 활동 또는 사용 맥락이 우선 보인다.
- 회의록은 제목/요약, 연결 회사/담당자/딜, 작성일, 다음 행동 맥락이 우선 보인다.
