...
G07은 사용자별 AI 사용횟수와 비용을 볼 수 있는 1차 기반을 만든다. 동시에 `TODO/PADDLE_PLAN`에서 다룰 event를 09 runtime 범위에 섞지 않도록 정리한다.
...
- `TODO/PADDLE_PLAN`에서 결정할 고려사항 기록
...
6. `TODO/PADDLE_PLAN`에서 확정할 고려사항을 코드 TODO가 아니라 문서/constant 주석에 남긴다.
...
3. `TODO/PADDLE_PLAN`에서 실제 UI/API와 함께 최종 확정한다.
...
- reserved taxonomy constant: `// 기능 : Paddle Billing에서 최종 확정할 reserved 분석 이벤트 이름입니다.`
...
- [x] `TODO/PADDLE_PLAN`에서 결정할 AI quota/UsageMeter 고려사항이 남았다.
