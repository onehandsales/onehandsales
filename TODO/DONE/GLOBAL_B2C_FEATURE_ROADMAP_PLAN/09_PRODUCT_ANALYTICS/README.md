...
09의 목표는 완성형 BI나 관리자 대시보드가 아니다. 09는 Global B2C 유료 판매를 위해 반드시 필요한 분석 정본을 만든다. Admin 화면/API는 `11_ADMIN_OPERATION`에서 구현했고, 결제/paywall/churn의 최종 상태 전이는 이관된 `TODO/PADDLE_PLAN`에서 post-beta에 확정한다.
...
- `AiProviderCallLog` 기반 AI usage summary는 G07에서 구현됐고, `groupBy=DAY`는 현재 `User.timeZone` 기준으로 계산한다.
...
| Billing/growth | paywall/trial/coupon/referral/churn event name은 09에서 reserved만 한다. 최종 계약은 `TODO/PADDLE_PLAN`이 override한다. |
| Client page view | 1차는 핵심 `/app` route view만 수집한다. public site, UTM, 광고 attribution은 제외한다. |
| Runtime taxonomy | 2026-07-30 사용자 재확인 기준 현재 최소 event set을 유지한다. Notification/Google Calendar/follow-up/PWA/Admin/Billing 세부 event는 10/11 완료 범위 또는 `TODO/PADDLE_PLAN`/후속 계획에서 결정한다. |
...
- `TODO/PADDLE_PLAN`에서 확정할 billing/paywall/churn event 이름은 reserved list에만 남아 있다.
...
- Admin analytics full UI/API는 11에서 완료됐고, Billing/paywall/churn 구현은 `TODO/PADDLE_PLAN`, 모바일 PWA 세부 event는 10 또는 후속 분석 계획 범위로 유지한다.
