...
- `paywall_viewed`, `upgrade_clicked`, `trial_started`, `coupon_applied`, `referral_invited`, `subscription_started`, `subscription_canceled`, `churn_survey_submitted`는 09 runtime allowlist에 넣지 않고 `TODO/PADDLE_PLAN` reserved로만 둔다.
...
- `AiUsageDaily`: 09에서는 만들지 않는다. `TODO/PADDLE_PLAN`의 AI plan/quota에서 source of truth 여부를 확정한다.
...
- `BillingEvent`, `UserSubscription`, `UsageMeter`: `TODO/PADDLE_PLAN`에서 확정한다.
