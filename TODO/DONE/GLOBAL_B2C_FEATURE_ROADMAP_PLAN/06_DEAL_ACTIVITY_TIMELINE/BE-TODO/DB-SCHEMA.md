...
실제 source of truth는 `BE/prisma/schema.prisma`와 `BE/prisma/migrations/20260726010000_add_deal_activity/migration.sql`이다.
...
model DealActivity {
  ...
  @@index([userId, dealId, occurredAt(sort: Desc), id(sort: Desc)])
  @@index([userId, activityType, occurredAt])
  @@index([userId, sourceType, sourceId])
}
```

G02에서 Prisma relation 이름 충돌은 없음을 확인했다. FK는 1차 기준으로 `ON DELETE RESTRICT ON UPDATE CASCADE`를 사용한다.
...
- DB target은 원격 Supabase로 확인했다. 공유/운영성 DB에 무단 migrate/seed를 실행하지 않았다.
- table/column/index에 COMMENT 또는 한글 SQL 주석을 남긴다.
- `body`는 민감정보가 포함될 수 있으므로 structured log에 남기지 않는 기준을 함께 구현한다.
- timeline 조회 index는 Prisma desc index를 명시해 `occurredAt desc, id desc` 정렬을 직접 만족한다. 실제 migration SQL 주석에도 이 결정을 남겼다.
