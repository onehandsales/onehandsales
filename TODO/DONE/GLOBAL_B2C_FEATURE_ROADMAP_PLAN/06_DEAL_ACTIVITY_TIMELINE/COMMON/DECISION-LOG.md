...
| UX/UI polish | 지금 06 후속으로 하지 않는다. | UX/UI는 로드맵 DONE 이후 별도 전면 유지보수 계획에서 한 번에 다룬다. |

이 결정은 잔여 후보를 삭제한다는 뜻이 아니다. `PRE12_FOLLOWUP_RECHECK`에 근거를 보존하고, 새 TODO 승격 여부는 필요성이 확인될 때 다시 판단한다.
