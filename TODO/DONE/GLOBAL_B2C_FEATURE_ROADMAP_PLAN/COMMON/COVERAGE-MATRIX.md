...
- [x] Pre-12 후속 후보 재분류: `PRE12_FOLLOWUP_RECHECK`와 `TODO/DONE/BEFORE_12_TASKS` closeout 완료. 기존 12 착수 전 01~11 제외/후속/보류 후보를 구현 없이 분류했고 선택된 5개만 닫았다.
- [x] Billing/Paddle 이관: 기존 12 Billing 후보와 NEXT/USER의 결제 관련 후보를 `TODO/PADDLE_PLAN`으로 이관 완료
- [x] Roadmap closeout: 2026-08-11 기준 이 상위 로드맵은 DONE으로 닫고, UX/UI 유지보수와 100명 베타를 다음 active direction으로 둔다

## 1. 목적

이 문서는 01~11 완료 기능과 이관된 12 Billing 후보가 어느 슬롯에 배정됐는지 보존한다.

입력은 `TODO/NEXT_BACKEND_API_BACKLOG_PLAN`의 Backend/API/DB 후보와 `TODO/USER_WEB_PRODUCTIZATION_GAP_PLAN`의 제품화 gap이었다. 결제 관련 후보는 이제 `TODO/PADDLE_PLAN`에서 관리한다.

이 matrix는 active 구현 지시가 아니다. 완료 이력, 제외 범위, 이관 기준을 확인하기 위한 archive 문서다.
...
| Growth | paywall/trial/coupon/referral/churn survey 실험 | 09, `TODO/PADDLE_PLAN` | 09 Done: reserved event 이름과 runtime 제외 경계 확정. 실제 paywall/billing 적용과 churn survey flow는 Paddle Plan에서 결정 |
...
| Billing | Pricing/plan/trial | `TODO/PADDLE_PLAN` | 기존 12에서 이관. UX/UI 유지보수와 100명 베타 이후 확정 |
| Billing | Subscription/entitlement | `TODO/PADDLE_PLAN` | plan별 기능/AI 제한은 Paddle confirmed scope에서 결정 |
| Billing | Paywall/upgrade flow | `TODO/PADDLE_PLAN`, 09 | 09에서 reserved taxonomy를 분리했다. 실제 billing event 발생은 Paddle 구현 후 연결 |
| Billing | AI usage plan/overage | `TODO/PADDLE_PLAN`, 05, 07, 09 | 09에서 비용 분석 foundation 완료. 제한/과금과 overage 정책은 Paddle 구현 전 확정 |
| Billing | Payment provider/webhook | `TODO/PADDLE_PLAN` | Merchant of Record 우선, Stripe 직접 결제 fallback |
| Billing | Failed payment recovery | `TODO/PADDLE_PLAN` | 결제 실패 복구 UX/API와 운영 대응은 Paddle plan에서 결정 |
| Billing | Tax/invoice/refund/chargeback | `TODO/PADDLE_PLAN` | 국가별 판매 정책과 MoR 기준으로 결정 |
| Billing | Coupon/referral | `TODO/PADDLE_PLAN`, 09 | 09에서 reserved taxonomy/foundation만 확정했다. 결제 적용과 실험 운영은 Paddle 이후 판단 |
| Billing | Churn survey/cancel reason | `TODO/PADDLE_PLAN`, 09 | 해지 flow, survey source, billing-linked churn 분석은 Paddle source event 이후 연결 |
| Billing | Billing Admin 연동 | `TODO/PADDLE_PLAN` | 11 Admin 운영은 결제/구독을 제외했다. Billing Admin 화면/API는 Paddle confirmed scope에서 결정 |
| Pre-12 recheck | 01~11 제외/후속/보류 후보 재분류 | `PRE12_FOLLOWUP_RECHECK` | Done: 13번 기능 슬롯이 아니다. 기존 12 전에는 선택된 `PRE12-F04`, `PRE12-F31`, `PRE12-F32`, `PRE12-F33`, `PRE12-F34`만 BEFORE_12에서 닫았고, 확정 API/DB/FE 계약 없는 후보는 구현하지 않았다 |
| Roadmap closeout | 01~11 완료와 Billing 이관 | 2026-08-11 | `GLOBAL_B2C_FEATURE_ROADMAP_PLAN`은 DONE으로 닫고, Billing은 `TODO/PADDLE_PLAN`, UX/UI/기능 유지보수는 별도 계획으로 분리한다 |
...
- 2026-08-11 이후 발견된 미구현/후속 후보는 기존 01~11 완료 폴더를 되돌려 열지 않고 새 TODO 폴더, 유지보수 계획, 또는 `TODO/PADDLE_PLAN`으로 승격한다.
