...
상태: Gate Baseline / 11 반영 / Billing moved to `TODO/PADDLE_PLAN`
기준일: 2026-08-11
...
- billing/paywall/churn 실제 conversion source와 결제/구독/세금 운영은 `TODO/PADDLE_PLAN`으로 이관했다.
- Global roadmap은 2026-08-11 DONE으로 닫고, Product UX first-sale gate는 다음 유지보수/UXUI 계획과 100명 베타에서 다시 확인한다.
...
- `TODO/PADDLE_PLAN`: 결제, 구독, 세금, 환불, invoice, failed payment
...
이 로드맵 안에서 12를 바로 구현하지 않는다. first-sale gate는 다음 유지보수/UXUI 계획, 100명 베타, 그리고 이후 `TODO/PADDLE_PLAN` 구현 시점에 다시 닫는다.

검토 순서:

1. 핵심 `/app` 업무 흐름이 베타 사용에서 반복 업무 도구처럼 동작하는지 확인한다.
2. Billing, subscription, tax, refund, invoice, failed payment, entitlement, paywall이 `TODO/PADDLE_PLAN`에서 어디까지 confirmed됐는지 확인한다.
3. paid conversion, churn, ARPU, AI cost/user 분석 source가 Paddle 구현 이후 실제 이벤트와 연결됐는지 확인한다.
4. Product UX first-sale gate에서 회사, 담당자, 제품, 딜, 일정, 회의록, 명함, import, search, trash, export, account/data request, billing 상태 흐름을 다시 확인한다.
5. Trust/policy gate에서 약관, 개인정보, 환불, 세금, 인보이스, chargeback, 계정 삭제, 데이터 export/delete, retention이 실제 Backend 데이터 처리와 충돌하지 않는지 확인한다.
6. 미완성 또는 후속으로 남은 항목은 기존 완료 폴더를 재개하지 않고 새 TODO 폴더나 `TODO/PADDLE_PLAN`에서 다룬다.
