...
`08_GLOBAL_DATA_I18N`, `11_ADMIN_OPERATION`, 이관된 `TODO/PADDLE_PLAN`에는 이미 공통 baseline 결정이 있다. 이번 03은 그 baseline을 침범하지 않고, 주간 보고서 기능 안에서 필요한 timezone/date-only 안정성만 반영한다.
