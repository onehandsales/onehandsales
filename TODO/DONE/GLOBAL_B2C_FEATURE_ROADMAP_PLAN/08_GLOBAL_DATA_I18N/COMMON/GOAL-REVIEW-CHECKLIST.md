...
상태: G03 Reviewed / G04 Reviewed / G05 Reviewed / G06 Reviewed / G07 Reviewed / G08 Reviewed / G09 Reviewed / G10 Reviewed
...
- [x] goal 파일에 Request 계약이 있거나 영향 없음으로 명시되어 있다.
- [x] goal 파일에 Response 계약이 있거나 영향 없음으로 명시되어 있다.
- [x] goal 파일에 Business Logic이 명시되어 있다.
- [x] goal 파일에 User Flow가 명시되어 있다.
- [x] goal 파일에 DB/Prisma 영향이 있거나 변경 없음으로 명시되어 있다.
- [x] `COMMON/GOAL-IMPLEMENTATION-MATRIX.md`에서 실제 수정 후보 파일과 완료 산출물을 확인했다.
- [x] API가 있으면 계약 상태, 소비자, 호환성, DTO 이름, success status가 명시되어 있다.
- [x] mutation이면 transaction 필요 여부와 rollback 범위가 명시되어 있다.
- [x] mutation/provider/batch이면 observability event key, request id, redaction 기준이 명시되어 있다.
- [x] DB 변경이 있으면 `BE/prisma/schema.prisma`, `BE/prisma/migrations`, `BE/prisma/seed.ts`를 확인했다.
- [x] 새 table/column/enum/index가 있으면 Prisma schema와 migration SQL에 한글 주석 또는 COMMENT가 있다.
- [x] Backend 신규/수정 코드에 `// API : ...`, `// 역할 : ...`, `// 기능 : ...` 주석 규칙이 적용됐다.
- [x] Frontend 신규/수정 코드에 `// 기능 : ...` 주석 규칙이 적용됐다.
- [x] 실행한 검증 command와 결과를 기록했다.
- [x] 실행하지 못한 검증은 사유를 기록했다.

## 2. G01 Document Contract Sync

- [x] API spec request/response가 현재 코드와 충돌하지 않는다.
- [x] `BE/prisma` 기준과 주석 스타일을 확인했다.
- [x] 기존 AGENT 문서 갱신 대상이 확정됐다.
- [x] G02~G10 blocking 질문이 없다.

## 3. G02 User Global Settings

- [x] profile request/response가 `preferredLocale`, `timeZone`, `countryCode`, `defaultCurrencyCode`를 포함한다.
- [x] User 신규 column이 `BE/prisma` 기준과 한글 주석 규칙을 따른다.
- [x] 신규 가입 기본값과 기존 사용자 fallback이 문서와 일치한다.
- [x] `/app/settings` 저장 후 현재 화면 반영 흐름이 동작한다.
...
- [x] 신규 Backend request/response 변경 없음이 명시되어 있다.
- [x] 핵심 `/app` 화면의 문구가 `ko-KR`, `en`으로 동작한다.
- [x] 긴 영어 문구가 layout을 깨지 않는다.
- [x] DB/Prisma 변경 없음이 확인됐다.

## 11. G10 QA Document Closeout

- [x] 08 전체 request/response 계약이 구현 결과와 일치한다.
- [x] 08 전체 business logic과 user flow가 구현 결과와 일치한다.
- [x] 08 전체 DB/Prisma 변경에 한글 주석/COMMENT가 있다.
- [x] BE/FE 검증 command 결과와 미실행 사유가 기록됐다.
- [x] AGENT 문서의 이전 정책이 갱신됐다.
