...
결제, 구독, plan, entitlement, invoice, refund, failed payment recovery, paid conversion, churn, ARPU, LTV/CAC는 이관된 `TODO/PADDLE_PLAN`에서만 다룬다. 11의 Admin 화면에는 결제/구독 탭, 결제 상태, plan 상태, 복구 비용 처리 버튼을 만들지 않는다.
...
- 2026-08-09 `TODO/DONE/BEFORE_12_TASKS` G04 closeout에서 checklist, goal index, BE/FE TODO 상태를 실제 구현 상태와 맞췄다.
...
| 결제/구독 | 11에서 완전히 제외한다. `TODO/PADDLE_PLAN`에서 처리한다. |
