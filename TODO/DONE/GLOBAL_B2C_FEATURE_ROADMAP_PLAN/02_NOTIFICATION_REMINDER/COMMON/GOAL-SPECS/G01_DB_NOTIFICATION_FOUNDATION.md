...
- `BE-TODO/DB-SCHEMA.md`의 SQL DDL 초안을 migration 대조 기준으로 사용한다.
...
- migration에 enum type, table, index, FK, check constraint가 포함되어 있다.
- migration에 `COMMENT ON TYPE`, `COMMENT ON TABLE`, `COMMENT ON COLUMN`, `COMMENT ON INDEX`가 포함되어 있다.
