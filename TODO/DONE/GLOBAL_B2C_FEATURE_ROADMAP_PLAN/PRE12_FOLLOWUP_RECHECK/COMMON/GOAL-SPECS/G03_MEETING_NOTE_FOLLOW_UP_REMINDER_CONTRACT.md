...
목표: 회의록 follow-up reminder와 자동 발송 후보를 분리하고, 후속 재검토 때 필요한 정책 질문을 남긴다.

2026-08-07 최종 분류 기준은 `../FINAL-CLASSIFICATION.md`다. 회의록 follow-up reminder와 자동 발송은 PRE12에 할 것에 포함하지 않는다.
...
| 후보 | 의미 | 현재 기본 상태 |
| --- | --- | --- |
| follow-up draft reminder | 회의록 작성 후 후속 연락 초안을 만들거나 확인하라고 알림 | 후속 seed |
| follow-up send reminder | 초안이 있는데 아직 보내지 않았다고 알림 | 후속 seed |
| follow-up automatic send | 사용자가 확인하지 않아도 이메일/SMS를 발송 | 후속 seed / 정책 결정 필요 |
...
- `CANDIDATE-MATRIX.md`의 `PRE12-F02`, `PRE12-F03`은 후속 seed로 남는다.
- `COMMON/API-SPEC`에는 PRE12 confirmed API를 만들지 않는다.
