...
이 goal은 구현 goal이 아니다. `BE`, `FE` 코드 변경, API 계약 확정, Prisma migration 생성, PWA/offline 구현, 신규 User Web route 활성화는 하지 않는다.
...
| 범위 | 현재 사실 |
| --- | --- |
| 모바일 기본 방향 | 10은 mobile browser field-use를 완료 범위로 닫았다. |
...
| PWA/offline | `FE/user-web`에서 `vite-plugin-pwa`, `workbox`, `manifest.webmanifest`, offline shell/full offline sync 구현은 확인되지 않았다. |
...
| FE generic ExportJob 잔여 코드 | `PRE12-F09` | `/app/export` route는 `/app`으로 redirect되고 BE `ExportJob`/`/api/exports` 구현은 없다. 다만 FE에 `ExportScreen`, `/api/exports` client/hook/type이 남아 있다. 후속 재검토 전에는 route/API를 열지 않는다. |
...
| PWA/offline packaging | `PRE12-F30` | 10은 mobile browser field-use 완료다. PWA install/offline shell/full offline sync, install attribution은 10 미완성이 아니라 별도 PWA roadmap 후보다. |
| BusinessCard advanced camera preview/crop | `PRE12-F42` | 10 원문은 custom `getUserMedia` camera preview/crop을 mobile advanced capture 후속으로 제외했다. 실제 FE는 browser file/camera picker 기준이므로 10 완료를 재오픈하지 않는다. |
...
| 후보 | PRE12 ID | 분류 | 판단 |
| --- | --- | --- | --- |
| generic ExportJob/PDF | `PRE12-F09` | 후속 seed | 03/11 후속이다. FE 잔여 코드는 있어도 사용자 노출 route와 BE API/model은 없다. |
| PWA packaging과 install attribution | `PRE12-F30` | 후속 seed / 별도 PWA roadmap | 10을 재오픈하지 않는다. PWA/offline/full offline sync/install attribution 계약은 후속 또는 별도 PWA roadmap에서 만든다. |
| BusinessCard mobile advanced camera preview/crop | `PRE12-F42` | 후속 seed / mobile advanced capture | 모바일 사용량, device QA, 접근성, fallback, image handling 기준이 확인될 때 별도 계약으로 판단한다. |
| Server draft and media/raw storage policy | `PRE12-F43` | defer / trust-policy / 후속 seed | retention, 삭제권, account deletion 실제 처리, encryption, quota, raw access audit, redaction 정책 없이는 구현하지 않는다. |
...
- [x] PWA/offline은 `PRE12-F30`으로 유지하고 10 미완성으로 보지 않는다고 기록했다.
...
- [x] generic ExportJob은 `PRE12-F09`이며 FE 잔여 코드가 있어도 후속 재검토 전 route/API 활성화 대상이 아니라고 기록했다.
...
- [x] `UserDraft`, server draft DB, media/raw 저장, custom camera preview/crop, PWA/offline, `/app/export`, `/api/exports` 구현 금지를 명시했다.
