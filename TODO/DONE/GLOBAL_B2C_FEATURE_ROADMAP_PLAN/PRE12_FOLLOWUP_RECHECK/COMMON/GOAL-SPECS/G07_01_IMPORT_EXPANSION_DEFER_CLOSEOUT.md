...
상태: Decided
목표: 01에서 제외된 import scale/source/Admin 확장 후보가 01 미완성이 아니라 후속 재검토 seed임을 문서로 닫는다.
...
아래 항목은 PRE12 구현 또는 계약화 대상이 아니다.
...
- 01 README와 최종 서비스 형태 문서는 위 항목을 01 최종형 밖의 별도 TODO 또는 후속 후속 범위로 분리한다.
- 현재 Global B2C PRE12 목표는 기존 완료 슬롯의 의미를 깨지 않고, Paddle 이관 전에 후속 후보를 분류하는 것이다.
...
- PRE12 `BE/src`에 import worker, schedule import, meeting-note import, ImportJob Admin API를 추가하지 않는다.
- PRE12 `FE/user-web`에 대용량 import progress, 일정/회의록 import source UI, Admin-only import cleanup 화면을 추가하지 않는다.
- 후속 재검토 전 Prisma schema에 import queue/source/Admin 운영 table을 추가하지 않는다.
...
- `README.md`, `BE-TODO/API-TODO.md`, `BE-TODO/DB-SCHEMA.md`, `FE-TODO/USER-WEB-TODO.md`에 PRE12 구현 금지와 후속 seed 판단이 남는다.
- 01 완료 상태는 재오픈하지 않는다.
