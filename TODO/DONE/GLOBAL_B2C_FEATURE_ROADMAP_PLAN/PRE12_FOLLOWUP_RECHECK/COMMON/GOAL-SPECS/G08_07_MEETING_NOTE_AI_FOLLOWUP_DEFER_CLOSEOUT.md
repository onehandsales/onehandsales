...
| --- | --- | --- | --- |
| 회의록 follow-up reminder | `PRE12-F02`, `G03` | 후속 seed | 알림 source, 취소, 재시도, 사용자 설정, draft/send 상태 계약 전 구현 금지 |
| MeetingNote follow-up 자동 발송 | `PRE12-F03`, `G03` | 후속 seed / 정책 결정 필요 | 명시적 사용자 동의, 발송 예약, 실패/재시도, unsubscribe, 비용 정책 전 구현 금지 |
| MeetingNote list latest/next summary | `PRE12-F08`, `G04` | 후속 seed | `GET /api/meeting-notes` 응답 필드 또는 별도 summary endpoint 계약 전 구현 금지 |
| AI data cleanup 제안 저장/적용 | `PRE12-F14` | 후속 seed / 별도 data quality 계획 | 09 Product Analytics 또는 별도 data quality TODO에서 필요성, 적용 권한, 감사 로그, rollback 기준 결정 |
...
| MeetingNote AI 후보 자동 업무 mutation | `PRE12-F40` | 후속 seed / AI policy | 사용자 확인 없는 저장, 자동 일정 생성, 자동 딜 변경, Contact/MeetingNote/Deal mutation은 정책/감사/rollback 계약 전 구현 금지 |
