...
2026-08-07 최종 분류 기준은 `../FINAL-CLASSIFICATION.md`다. 다음 행동 reminder는 PRE12에 할 것에 포함하지 않는다.
...
5. 후속 product follow-up으로 만들 경우 첫 포함 범위는 어디까지인가?

## 3. 포함 가능 범위

후속 TODO 또는 `POST12_*` 재검토에서 계약이 confirmed로 올라간 뒤에만 아래를 검토한다.
...
- billing/paywall/plan별 reminder 제한은 PRE12/Billing 결정하지 않는다.
- PRE12 `NotificationSourceType`, `NotificationType`, `UserNotificationSetting`을 확장하지 않는다.

## 5. 완료 기준

- `CANDIDATE-MATRIX.md`의 `PRE12-F01`은 후속 seed로 남는다.
- `COMMON/API-SPEC`에는 PRE12 confirmed API를 만들지 않는다.
- 후속 재검토 전 구현 금지를 유지한다.
