...
이 문서는 `PRE12_FOLLOWUP_RECHECK`가 어떤 후보를 다루고, 어떤 후보는 기존 완료 슬롯 또는 후속 후보로 남기는지 고정한다.

최종 3분류 정본은 `FINAL-CLASSIFICATION.md`다. 이 문서는 PRE12 새 기능 구현을 열지 않는다.
...
| 01 Import 확장 후보 분리 | 대용량 import worker, 일정/회의록 import, ImportJob Admin 전용 화면/API가 01 미완성이 아니라 별도 후속 seed임을 고정한다. |
...
| Paddle Billing 구현 | 결제/구독/세금과 plan/payment/invoice/refund/failed payment/paywall/churn/paid conversion은 `TODO/PADDLE_PLAN` 결정 없이는 기준을 확정할 수 없다. |
...
최종 실행 분류는 `FINAL-CLASSIFICATION.md`의 PRE12 처리 대상 / 후속 seed / billing 충돌을 따른다. 완료 참조는 작업 분류에서 제외한다. 아래 상태값은 후보의 세부 성격을 설명하기 위한 보조 값이다.

| 상태 | 의미 |
| --- | --- |
| `done` | 실제 구현과 QA가 이미 닫힌 항목 |
| `closed-by-BEFORE_12` | `TODO/DONE/BEFORE_12_TASKS`에서 2026-08-09 기준 닫힌 PRE12 운영 smoke 또는 문서 정합성 항목 |
| `PRE12-follow-up-needed` | PRE12 별도 goal로 처리할 수 있고, billing 결정과 직접 충돌하지 않는 항목. 2026-08-10 현재 이 상태로 남은 후보는 없다. |
| `PRE12-doc-cleanup` | 실제 기능 구현은 닫혔지만 문서 체크리스트, architecture 설명, dead-code 메모 같은 정합성 정리가 필요한 항목. 2026-08-10 현재 이 상태로 남은 후보는 없다. |
| `follow-up-seed` | 후속 재검토에서 새 TODO로 승격할지 판단할 항목 |
| `billing-blocked` | Paddle/Billing 결정 없이는 구현 기준을 확정할 수 없는 항목 |
