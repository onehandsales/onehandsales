...
판단: `CONTACT_API.md`는 담당자 API 목록과 공통 DTO 요약을 보관하는 개요 문서로 유지한다. API별 `API 이름`, `API 식별자`, `Request 이름`, `Response 이름`, 비즈니스 로직, Error FE 처리/log level, Transaction, Observability, FE/BE 처리 기준의 현재 정본은 `CONTACT_API_DETAIL.md`다.
