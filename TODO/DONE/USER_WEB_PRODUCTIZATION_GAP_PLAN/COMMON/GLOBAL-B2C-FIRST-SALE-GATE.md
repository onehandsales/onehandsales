...
- [x] `05_AI_WEEKLY_SALES_REPORT` G10 Gmail/Microsoft actual email provider adapter 구현 및 자동 검증 완료. Provider smoke closeout은 2026-08-10 PRE12/BEFORE_12 배포 환경 smoke verified 기준 완료 처리
- [x] Product UX 중 DealActivity timeline, Deal list products/latestActivity, Contact dealCount는 `06_DEAL_ACTIVITY_TIMELINE`에서 완료. Company/Contact/Product latest summary와 generic summary endpoint는 2026-08-06 A 결정에 따라 PRE12 gate가 아니다.
...
- [x] Global B2C 01~11 + PRE12 closeout 완료, Billing은 `TODO/PADDLE_PLAN`으로 이관
...
| Retention | 다음 행동, 일정, 딜 지연, AI report follow-up, 회의록 follow-up을 사용자가 놓치지 않는다. | 일정/딜 Notification reminder, 주간 일정 보고서, Google Calendar read-only import와 Google-origin schedule reminder, 05 AI weekly report/follow-up delivery, 딜 activity timeline, 회의록 follow-up draft, browser push permission UX는 완료. 실제 SMTP/Web Push provider smoke도 2026-08-04 사용자 확인 기준 배포 환경에서 완료됐고, 05 Gmail/Microsoft provider smoke closeout도 2026-08-10 PRE12/BEFORE_12 배포 환경 smoke verified 기준 완료 처리했다. 회의록 follow-up 알림/자동 발송은 후속 기능으로 분리한다. |

기능/UXUI 유지보수와 100명 베타 이후에는 `TODO/PADDLE_PLAN` 기준으로 결제 관련 gate를 다시 닫는다. 이때 실제 BE/FE/Prisma 상태를 함께 대조한다.
...
| Retention follow-up | 일정/딜 알림, 주간 일정 보고서, Google Calendar read-only import, 05 AI weekly report/follow-up delivery, Gmail/Microsoft provider smoke closeout, 회의록 follow-up draft, browser push permission UX, 실제 SMTP/Web Push provider smoke, 09 activation/retention snapshot foundation, 10 mobile field event는 구현 완료. 회의록 follow-up 알림/자동 발송과 운영 모니터링 고도화 기준은 별도 후속으로 판단한다. |
...
- 미완성/후속으로 남은 항목은 필요성이 확인될 때 새 TODO 폴더로 승격한다. UX/UI 디자인 유지보수는 결제보다 먼저 별도 계획으로 진행한다.
