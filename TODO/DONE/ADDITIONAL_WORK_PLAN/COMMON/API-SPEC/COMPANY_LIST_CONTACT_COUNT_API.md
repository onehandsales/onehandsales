...
회사 목록 페이지네이션 조회 시 각 회사 item에 해당 회사에 연결된 담당자 수를 추가한다.
...
| 필드 | 타입 | nullable | 설명 |
|---|---|---:|---|
| `contactCount` | number | 아니오 | 해당 회사에 연결된 담당자 수 |
...
6. `items[].contactCount`에 회사별 담당자 수를 넣어 응답한다.
...
- 회사 목록 item에서 `contactCount`를 담당자 수 표시 용도로 사용할 수 있다.
- `totalCount`를 담당자 수로 해석하지 않는다.
...
- 다른 사용자의 담당자 수가 섞이지 않도록 ownership 기준을 유지한다.

## 12. 검증 기준

- 담당자가 없는 회사는 `contactCount: 0`을 반환한다.
- 담당자가 1개 이상 연결된 회사는 실제 연결 수를 반환한다.
- 검색/필터 적용 시 `totalCount`는 회사 개수 기준으로 유지된다.
- `contactCount` 추가로 정렬, 페이지네이션, 필터 결과가 바뀌지 않는다.
- 다른 사용자의 회사 또는 담당자 수가 섞이지 않는다.
