...
아래 문서를 먼저 읽고, 회사 목록 페이지네이션 API 응답에 회사별 담당자 수 `contactCount`를 추가해줘.
...
- `contactCount`는 각 회사에 연결된 담당자 수다.
- 회사별 담당자 수는 현재 사용자 ownership 기준을 벗어나면 안 된다.
...
- 담당자가 있는 회사는 실제 연결 수를 반환한다.
- 검색/필터 적용 시 `totalCount`는 회사 개수 기준으로 유지된다.
- 다른 사용자의 담당자 수가 섞이지 않는다.
