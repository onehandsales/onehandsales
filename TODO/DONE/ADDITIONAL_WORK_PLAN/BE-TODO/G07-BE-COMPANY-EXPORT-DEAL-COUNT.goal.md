# /goal G07 BE Company Export Deal Count

## /goal 입력문

아래 문서를 먼저 읽고, 회사 xlsx export API에 회사별 딜 수 `dealCount` 컬럼을 추가해줘.
...
- 회사 xlsx export row 생성 로직
- 관련 테스트
- 관련 API 계약 문서 상태 갱신

## 비즈니스 규칙

- 기존 export query는 변경하지 않는다.
- export에는 현재 회사 목록 검색어와 필터 조건을 적용한다.
- export에는 `page`를 적용하지 않는다.
- 파일 컬럼에 기존 `담당자 수`가 있으면 유지하고 `딜 수`를 추가한다.
- `딜 수`는 현재 사용자 소유 `Deal`만 집계한다.

## 구현 제한

- JSON 회사 목록 API 구현은 G06에서 처리한다.
- 회사 단건 상세 응답은 변경하지 않는다.
...
- xlsx에 `딜 수` 컬럼이 포함된다.
- 필터가 없으면 현재 사용자 전체 회사가 포함된다.
- 필터가 있으면 조건에 맞는 회사만 포함된다.
- 각 row의 `딜 수`가 실제 연결 딜 수와 일치한다.
