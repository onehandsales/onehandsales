...
- `TODO/DONE/MVP-STARTER_PLAN/COMMON/API-SPEC/README.md`: FE/BE 공통 API 계약 정본
...
- local/integration/E2E test DB는 Docker PostgreSQL을 사용할 수 있다.
- Controller는 application service만 호출한다.
- API 명세는 `AGENT/SOFTWARE_AGENT/CONVENTION/API_SPEC.md`의 필수 항목을 따른다.
- 상세 API 명세는 `TODO/DONE/MVP-STARTER_PLAN/COMMON/API-SPEC`를 정본으로 본다. 구현 시에는 해당 범위의 `*-ENDPOINT-CONTRACT.md`를 API별 처리 흐름 정본으로 본다.
...
- MVP 기능은 실제 OpenAI/OCR/Google Calendar/SMTP/Web Push VAPID adapter를 기본 구현으로 가진다.
- 자동 테스트는 외부 Provider 없이도 stub/mock adapter로 핵심 E2E를 실행할 수 있다.
