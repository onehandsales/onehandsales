...
G01은 2026-08-09 사용자 지시에 따라 acceptance closeout으로 완료 처리했다.
...
이번 점검에서는 코드/문서/자동 검증이 통과했고, Gmail/Microsoft OAuth 연결 row가 현재 `BE/.env` 기준 Prisma DB에서 확인됐다. 실제 allowlist 발송과 allowlist 밖 차단의 `FollowUpDeliveryAttempt` row는 독립 확인하지 않고, 사용자의 "동작한다고 가정하고 통과" 지시에 따라 assumed pass로 닫았다.
...
| DB safe evidence | current Prisma DB all-time `ExternalEmailConnection` / `FollowUpDeliveryAttempt` aggregate | ACCEPTED_BY_USER_ASSUMPTION, connection rows confirmed and send attempt rows not independently verified |
...
아래 항목은 2026-08-09 사용자 acceptance 지시에 따라 assumed pass로 닫았다. 독립 DB 발송 증거가 필요한 재감사에서는 `FollowUpDeliveryAttempt` safe field를 다시 확인한다.
...
- [x] 사용자 기준 Gmail/Microsoft smoke가 모두 동작한다고 보고됐다.
- [x] 현재 `BE/.env`의 Prisma DB에서 Gmail/Microsoft connection row가 확인됐고, send/allowlist block 증거는 사용자 지시에 따라 assumed pass로 닫았다.

## 7. 재감사 조건

G01은 닫았지만, 실제 운영 증거를 다시 감사하려면 다음을 확인한다.
...
아래 상태는 사용자 acceptance 기준이다. 문서에는 실제 이메일 주소, OAuth code/state, token, 제목, 본문 원문을 쓰지 않는다.

| 단계 | 기존 request | response/DB 확인 | 완료 여부 |
| --- | --- | --- | --- |
| settings 진입 | `GET /api/follow-up-delivery/settings` | Gmail/Microsoft connection 상태가 masked email로 노출됨 | Accepted / Assumed Pass |
| Gmail OAuth 시작 | `POST /api/follow-up-delivery/email-connections/google/connect` | `authorizationUrl`, `stateExpiresAt` 반환. raw state는 DB에 저장되지 않음 | Accepted / Assumed Pass |
| Gmail callback | `GET /api/follow-up-delivery/email-connections/google/callback` | `ExternalEmailConnection.provider=GOOGLE`, `status=CONNECTED`, send scope 확인 | Accepted / Assumed Pass |
| Gmail allowlist 발송 | `POST /api/follow-up-messages/:messageId/send` | message `status=SENT`, attempt `status=SENT`, provider raw body 미저장 | Accepted / Assumed Pass |
| Gmail allowlist 밖 차단 | `POST /api/follow-up-messages/:messageId/send` | attempt `status=FAILED`, `safeErrorCode=FollowUpEmailSmokeRecipientNotAllowed`, `externalCallSkipped=true` | Accepted / Assumed Pass |
| Microsoft OAuth 시작 | `POST /api/follow-up-delivery/email-connections/microsoft/connect` | `authorizationUrl`, `stateExpiresAt` 반환. raw state는 DB에 저장되지 않음 | Accepted / Assumed Pass |
| Microsoft callback | `GET /api/follow-up-delivery/email-connections/microsoft/callback` | `ExternalEmailConnection.provider=MICROSOFT`, `status=CONNECTED`, `Mail.Send` scope 확인 | Accepted / Assumed Pass |
| Microsoft allowlist 발송 | `POST /api/follow-up-messages/:messageId/send` | message `status=SENT`, attempt `status=SENT`, provider raw body 미저장 | Accepted / Assumed Pass |
| Microsoft allowlist 밖 차단 | `POST /api/follow-up-messages/:messageId/send` | attempt `status=FAILED`, `safeErrorCode=FollowUpEmailSmokeRecipientNotAllowed`, `externalCallSkipped=true` | Accepted / Assumed Pass |
| redaction 검증 | log/DB detail 확인 | token, raw provider response, recipient email 원문, sender email 원문, 제목/본문 원문 없음 | Accepted / Assumed Pass |
...
| 2026-08-09 최종 확인 email `FollowUpDeliveryAttempt` group | 0, user acceptance 기준 assumed pass |

해석:

- 이전 점검 시점에는 사용자가 확인한 smoke가 다른 Backend 또는 다른 DB에서 실행됐을 가능성이 있었다.
- 이후 Gmail/Microsoft OAuth 연결 row는 현재 DB에서 확인됐다.
- `FollowUpDeliveryAttempt` 발송/차단 row는 독립 확인하지 않고 사용자 acceptance 지시에 따라 assumed pass로 닫았다.
...
- 따라서 15:58 KST 자료만으로는 실제 send/allowlist block 완료 증거로 사용할 수 없었다. 최종 closeout은 이후 Gmail/Microsoft connection row 확인과 사용자 acceptance 지시를 기준으로 닫았다.
...
- Gmail actual send/allowlist block `FollowUpDeliveryAttempt` row는 독립 확인하지 않고 사용자 acceptance 지시에 따라 assumed pass로 닫았다.

Microsoft 연결 추가 확인:

- 2026-08-09 기준 `ExternalEmailConnection`에 `provider=MICROSOFT`, `status=CONNECTED` row 1건이 확인됐다.
- 원문 provider account email은 문서에 기록하지 않았다.
- Microsoft actual send/allowlist block `FollowUpDeliveryAttempt` row는 사용자 acceptance 지시에 따라 assumed pass로 닫았다.

## 10. 최종 판정

- G01 완료 여부: Yes
- 완료 성격: User-Assumed Provider Smoke Accepted
- 코드 수정 결과: Gmail OAuth callback의 provider 부가 query whitelist failure와 browser callback JSON 노출 문제를 수정했다.
- 문서 수정 결과: 05 G10 pending 상태와 BEFORE_12 G01 상태를 사용자 acceptance 기준으로 closeout 처리했다.
