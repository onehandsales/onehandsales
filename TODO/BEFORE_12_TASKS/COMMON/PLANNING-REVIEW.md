...
- 판정: `BEFORE_12_TASKS`는 `/goal` 실행 가능한 문서 구조로 정리됐다.
- 이유: PRE12 최종 분류의 12 전 항목 5개가 G01~G05로 1:1 연결됐고, G06 handoff가 12 Billing 착수 가능 판정을 담당한다.
- 구현 상태: 이번 계획은 구현 계획이 아니라 closeout 실행 계획이다. 새 API/DB/route 구현은 범위 밖이다.
- 핵심 blocker: G01은 Gmail과 Microsoft 365 production-equivalent smoke가 모두 성공해야 완료다. env/callback/account 미준비는 완료가 아니라 blocker다.
...
| Resolved | G01 완료 기준 | 기존 문서에는 실행 불가 사유 기록만으로도 closeout처럼 읽히는 문구가 있었다. | Gmail/Microsoft 모두 실제 success가 필수이며 미실행은 blocker로 명시한다. |
...
단, G01 실행 시점에는 사용자가 준비한 `.env`와 provider console callback URL이 실제로 필요하다.

## 7. 충돌 사항

현재 문서 구조 기준의 unresolved 충돌은 없다.

G01 실행 결과 Gmail 또는 Microsoft smoke가 실패하면 G06에서 12 착수 가능으로 판정할 수 없다.

## 8. 추가 사용자 결정 필요 여부

현재 문서 고도화와 goal 분리에는 추가 질문이 필요 없다.
