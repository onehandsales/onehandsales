...
2026-08-06 `06_DEAL_ACTIVITY_TIMELINE` 후속 재검토 A 결정 반영: `NBA-003` 잔여 Company/Contact/Product latest summary, generic summary endpoint, record별 상세 timeline은 12 전 API 계약으로 만들지 않는다.
...
2026-08-10 기준 12 착수 전 01~11 pre-12 재대조와 BEFORE_12 closeout은 완료됐고, Admin provider failure source 편중 cursor pagination Finding도 11 품질 보정으로 닫혔다. `NBA-003` 잔여 record summary는 12 전 계약화하지 않는다. 다음 작업은 12이며, 12 완료 뒤 `TODO/GLOBAL_B2C_FEATURE_ROADMAP_PLAN/COMMON/POST-12-REVIEW-AND-FOLLOWUP.md`를 따라 남은 API 후보를 다시 분류한다.
...
- 2026-08-06 A 결정으로 보류된 `NBA-003` 잔여 record summary는 post-12 재검토 전까지 API contract를 `confirmed`로 승격하지 않는다.
