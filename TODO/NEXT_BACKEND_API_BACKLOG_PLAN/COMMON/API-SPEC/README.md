...
2026-08-05 `05_AI_WEEKLY_SALES_REPORT` 반영 완료: AI weekly report API와 follow-up delivery API, Gmail/Microsoft 실제 email provider adapter 계약은 구현/자동 검증 완료 상태다. 운영 credential/callback/allowlist 기반 provider smoke는 pending이다.
...
사용자 결정 기준으로 12 착수 전 01~11 pre-12 재대조를 진행한다. 2026-08-05 기준 01~05는 진행/확인 완료로 보고 다음 대상은 06~11이다. 01~11 재대조 후 12를 진행하고, 12 완료 뒤 `TODO/GLOBAL_B2C_FEATURE_ROADMAP_PLAN/COMMON/POST-12-REVIEW-AND-FOLLOWUP.md`를 따라 남은 API 후보를 다시 분류한다.
