...
- [x] `09_PRODUCT_ANALYTICS`: Done (2026-07-30)
  ...
  - 현재 의미: product analytics foundation은 완료다. Admin analytics UI/API는 11, billing/paywall/churn runtime source는 12 후보로 남긴다.
- [x] `NBA-005 BusinessCard provider failure code/message contract`: Done (2026-07-31)
  ...
  - 현재 의미: 사용자 safe failure contract는 완료다. Admin provider failure dashboard와 운영 추적은 11 후보로 남긴다.
...
완료 이력으로 승격된 `NBA-001`, `NBA-002`, `NBA-003` Deal subset, `NBA-004` MeetingNote detail subset, `NBA-005`, `NBA-006`, `NBA-008`, `NBA-009`, `NBA-010`, `NBA-011` provider log subset, `NBA-014`, `NBA-015`, `08_GLOBAL_DATA_I18N`, `09_PRODUCT_ANALYTICS`, `10_MOBILE_PWA_FIELD_USE` 외의 후보 API 계약은 `draft` 또는 `후보` 상태로만 남긴다.
...
| NBA-011 | MeetingNote provider call log and Admin audit | AGENT 문서, `07_MEETING_NOTE_AI_PROVIDER_LOG` | ops/security | No | 부분 완료. AI/STT/next action/follow-up provider 호출 비용, latency, 실패 상태를 원문 없이 추적할 수 있다. | Partial Done: User API provider log write 구현. Admin/internal 조회 API는 후보 | Partial Done: `AiProviderOperation` meeting-note 값, `AiProviderCallLog.targetType/targetId/index` 구현. 별도 transcript/raw table은 만들지 않음 | Done for User Web safe failure. Admin future는 없음 | raw request/response, prompt, 회의 원문, transcript, follow-up body, contact 원문 저장 금지 QA 완료. Admin raw access/retention 정책은 후속 | MeetingNote Admin provider audit/retention policy | provider log subset 완료. Admin/internal audit 조회, raw access reason, retention/cleanup은 후속 후보로 유지한다. |
...
1. `NBA-007`, `NBA-011` 잔여 Admin audit/retention, `NBA-012`, `NBA-013` 보안/운영 계약 정리
...
4. Backup/restore, 장애 대응은 Global B2C data reliability/operation gate에서 별도 결정. LINE/Apple 실제 auth provider smoke는 2026-07-29 사용자 확인 기준 운영 완료
5. Product Analytics foundation은 09에서 완료됐고 mobile field-use event는 10에서 완료됐다. Admin analytics dashboard와 billing-linked conversion/churn flow는 각각 11/12에서 결정한다.
