...
| NBA-006 | ImportJob persistence/resume API | AGENT 문서 | product feature | Yes | 업로드, 매핑, 검증 중 새로고침/탭 이동 복구가 가능해진다. | 새 endpoint | table, migration, TTL/cleanup 후보 | import 화면, client type | 업로드 데이터 보관, 부분 저장, 만료, 개인정보 삭제 정책이 필요하다. | Persistent ImportJob plan | 현재 in-memory 한계는 Known limitation이며 새 persistence 설계가 필요하다. |
...
4. `NBA-003`, `NBA-004`, `NBA-006`, `NBA-009`, `NBA-010`, `NBA-011` product feature 설계
