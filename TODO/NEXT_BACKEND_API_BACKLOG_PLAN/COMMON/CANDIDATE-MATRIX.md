...
| NBA-006 | ImportJob persistence/resume API | AGENT 문서 | product feature | Yes | 업로드, 매핑, 검증 중 새로고침/탭 이동 복구가 가능해진다. | Done: `/api/imports` 계열 persistence/resume API 구현. 2026-08-03 G05~G08 최종형 보강 구현 대기 | Done: ImportJob/Row/Error/UploadedFile schema 및 migration 구현. G05~G08은 기존 schema 중심 cleanup/retention/validation 보강 | Done: import review resume UX와 client state 구현. G07/G08은 row detail 만료/제한 초과 안내 영향 | redaction, ownership, TTL/delete tracking QA 완료. G05~G08 완료 시 terminal cleanup, 원본 file binary 즉시 삭제, `ImportUserLogRow` 30일 cleanup, 10MB/5,000행 제한까지 닫힘 | `TODO/GLOBAL_B2C_FEATURE_ROADMAP_PLAN/01_IMPORT_JOB_PERSISTENCE` G05~G08 | G01~G04는 완료. G05~G08 구현/QA/문서 업데이트 후 01/NBA-006은 최종 서비스 형태 기준으로 완전 종료한다. |
