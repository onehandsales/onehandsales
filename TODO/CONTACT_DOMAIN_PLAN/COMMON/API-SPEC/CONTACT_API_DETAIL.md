...
이 문서는 `CONTACT_DOMAIN_PLAN`에서 사용하는 거래처(담당자) API의 요청값, 응답값, 내부 비즈니스 로직, 연결 DB, transaction, observability, 에러, FE/BE 처리 기준을 고정한다.
...
- 거래처 목록은 20개 단위 페이지네이션이다.
- 거래처 일반/개인 비밀 메모 로그는 10개 단위 cursor 무한스크롤이다.
...
거래처 목록 화면에서 거래처 이름 검색, 회사 필터, 거래처 부서 필터, 거래처 직급 필터, 20개 단위 페이지네이션을 제공한다.
...
| query | `username` | string | 아니오 | trim 후 빈 문자열이면 미적용 | 거래처 이름 부분 검색어 |
| query | `companyId` | string | 아니오 | UUID | 회사 필터 ID |
| query | `contactDepartmentId` | string | 아니오 | UUID | 거래처 부서 필터 ID |
| query | `contactJobGradeId` | string | 아니오 | UUID | 거래처 직급 필터 ID |
...
5. `contactDepartmentId`가 있으면 현재 사용자의 거래처 부서인지 검증한다.
6. `contactJobGradeId`가 있으면 현재 사용자의 거래처 직급인지 검증한다.
...
| 필드 | 타입 | nullable | 설명 |
|---|---|---:|---|
| `items` | `ContactListItemResponse[]` | 아니오 | 거래처 목록 |
| `page` | number | 아니오 | 현재 페이지 |
| `pageSize` | number | 아니오 | 20 |
| `totalCount` | number | 아니오 | 조건에 맞는 전체 거래처 수 |
...
거래처 목록 필터와 거래처 생성/수정 화면에서 선택할 회사 목록을 제공한다.
...
| 필드 | 타입 | nullable | 설명 |
|---|---|---:|---|
| `items` | `ContactJobGradeResponse[]` | 아니오 | 거래처 직급 목록 |
| `items[].id` | string | 아니오 | 거래처 직급 ID |
| `items[].jobGradeName` | string | 아니오 | 거래처 직급명 |
...
| 위치 | 필드 | 타입 | 필수 | validation | 설명 |
|---|---|---|---:|---|---|
| body | `jobGradeName` | string | 예 | trim 후 1자 이상 | 거래처 직급명 |
...
| 위치 | 필드 | 타입 | 필수 | validation | 설명 |
|---|---|---|---:|---|---|
| path | `jobGradeId` | string | 예 | UUID | 삭제할 거래처 직급 ID |
...
| 거래처에서 사용 중 | `ContactJobGradeInUse` | 409 | 삭제 불가 상태 표시 | log |

### FE/BE 처리 기준

- FE: 성공 시 직급 목록과 거래처 목록을 필요한 범위에서 재조회한다.
- FE: 409는 삭제 불가 안내로 표시한다.
- BE: 사용 중인 직급은 삭제하지 않는다.

## 10. 거래처 필터용 부서 전체 조회 API
...
| 필드 | 타입 | nullable | 설명 |
|---|---|---:|---|
| `items` | `ContactDepartmentResponse[]` | 아니오 | 거래처 부서 목록 |
| `items[].id` | string | 아니오 | 거래처 부서 ID |
| `items[].departmentName` | string | 아니오 | 거래처 부서명 |
...
| 위치 | 필드 | 타입 | 필수 | validation | 설명 |
|---|---|---|---:|---|---|
| body | `departmentName` | string | 예 | trim 후 1자 이상 | 거래처 부서명 |
...
| 위치 | 필드 | 타입 | 필수 | validation | 설명 |
|---|---|---|---:|---|---|
| path | `departmentId` | string | 예 | UUID | 삭제할 거래처 부서 ID |
...
| 거래처에서 사용 중 | `ContactDepartmentInUse` | 409 | 삭제 불가 상태 표시 | log |

### FE/BE 처리 기준

- FE: 성공 시 부서 목록과 거래처 목록을 필요한 범위에서 재조회한다.
- FE: 409는 삭제 불가 안내로 표시한다.
...
회사에 소속된 거래처(담당자)를 생성한다. 생성 시 선택 메모가 있으면 같은 transaction에서 `ContactMemoLog` 첫 데이터로 저장한다.
...
| 위치 | 필드 | 타입 | 필수 | validation | 설명 |
|---|---|---|---:|---|---|
| body | `username` | string | 예 | trim 후 1자 이상 | 거래처 이름 |
| body | `mobile` | string | 예 | `^010-\d{4}-\d{4}$` | 핸드폰번호 |
| body | `email` | string | 예 | 이메일 형식 | 이메일 |
| body | `companyId` | string | 예 | UUID | 소속 회사 ID |
| body | `contactDepartmentId` | string | 예 | UUID | 거래처 부서 ID |
| body | `contactJobGradeId` | string | 예 | UUID | 거래처 직급 ID |
| body | `contactMemo` | string \| null | 아니오 | trim 후 빈 문자열이면 미작성 처리 | 생성 시 함께 남길 초기 거래처 메모 |
...
- 필요 여부: 필요
- 이유: 거래처 생성과 초기 메모 로그 생성이 하나의 사용자 행동이다.
...
- FE: 성공 `201 Created`를 받으면 response body를 기대하지 말고 거래처 목록을 재조회한다.
...
| 위치 | 필드 | 타입 | 필수 | validation | 설명 |
|---|---|---|---:|---|---|
| path | `contactId` | string | 예 | UUID | 거래처 ID |

### 비즈니스 로직 흐름

1. AuthGuard로 현재 사용자를 확인한다.
2. `contactId`를 validation한다.
3. 현재 사용자 ownership 기준으로 거래처 단건을 조회한다.
4. `Company`, `ContactDepartment`, `ContactJobGrade` relation을 포함한다.
5. 거래처가 없으면 `ContactNotFound`로 중단한다.
6. 거래처 상세 응답 DTO로 변환한다.
...
|---|---|---:|---|---|
| 인증 없음 또는 invalid | `Unauthorized` | 401 | 로그인 또는 refresh 흐름 | warn |
| 거래처가 없거나 본인 소유가 아님 | `ContactNotFound` | 404 | not found 화면 또는 목록 이동 | log |

### FE/BE 처리 기준

- FE: 상세 화면의 기본 정보 영역에 response 필드를 그대로 사용한다.
- FE: 상세 응답의 회사/부서/직급 ID는 수정 form의 기본 선택값으로 사용한다.
- BE: 타 사용자 거래처는 404로 처리해 존재 여부를 노출하지 않는다.
...
|---|---|---|---:|---|---|
| path | `contactId` | string | 예 | UUID | 거래처 ID |
| body | `username` | string | 아니오 | trim 후 1자 이상 | 거래처 이름 |
| body | `mobile` | string | 아니오 | `^010-\d{4}-\d{4}$` | 핸드폰번호 |
| body | `email` | string | 아니오 | 이메일 형식 | 이메일 |
| body | `companyId` | string | 아니오 | UUID | 소속 회사 ID |
| body | `contactDepartmentId` | string | 아니오 | UUID | 거래처 부서 ID |
| body | `contactJobGradeId` | string | 아니오 | UUID | 거래처 직급 ID |
...
|---|---|---:|---|---|
| 인증 없음 또는 invalid | `Unauthorized` | 401 | 로그인 또는 refresh 흐름 | warn |
| 거래처가 없거나 본인 소유가 아님 | `ContactNotFound` | 404 | 목록 이동 또는 not found 안내 | log |
...
- FE: 성공 `201 Created`를 받으면 거래처 단건과 거래처 목록을 필요한 범위에서 재조회한다.
- FE: response body를 기대하지 않는다.
- BE: 요청에 포함되지 않은 필드는 변경하지 않는다.

## 16. 거래처 일반 메모 로그 단건 생성 API
...
- 필요 여부: 없음
- 이유: 거래처 ownership 검증 후 단일 메모 로그 생성이다.
...
|---|---|---:|---|---|
| 인증 없음 또는 invalid | `Unauthorized` | 401 | 로그인 또는 refresh 흐름 | warn |
| 거래처가 없거나 본인 소유가 아님 | `ContactNotFound` | 404 | not found 안내 | log |
...
1. AuthGuard로 현재 사용자를 확인한다.
2. `contactId`와 request body를 validation한다.
3. 비밀 메모 대상 거래처가 현재 사용자 소유인지 검증한다.
4. `memo`를 trim한다.
5. 값이 비어 있으면 `ValidationError`로 중단한다.
6. 비밀 메모 원문을 암호화한다.
...
| `ContactNotFound` | 404 | 거래처가 없거나 현재 사용자 소유가 아님 |
| `ContactDepartmentNotFound` | 404 | 거래처 부서가 없거나 현재 사용자 소유가 아님 |
| `ContactJobGradeNotFound` | 404 | 거래처 직급이 없거나 현재 사용자 소유가 아님 |
...
| `ContactDepartmentInUse` | 409 | 거래처가 사용하는 부서를 삭제하려 함 |
| `ContactJobGradeInUse` | 409 | 거래처가 사용하는 직급을 삭제하려 함 |
