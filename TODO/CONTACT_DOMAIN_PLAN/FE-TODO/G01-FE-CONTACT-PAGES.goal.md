...
아래 문서를 먼저 읽고, 사용자 페이지 거래처(Contact) 화면과 API 연동을 완료해줘.
...
- `FE/user-web/src/**`
- 기존 거래처/회사/메모 관련 화면과 API 클라이언트

## 목표

사용자 페이지에서 거래처(Contact) 목록, 생성, 상세, 수정, 일반 메모, 개인 비밀 메모, 필터 옵션 관리를 실제 백엔드 API 계약에 맞게 구현한다.

거래처는 회사에 소속된 담당자다. 회사 선택 없이 거래처를 저장하는 UI 흐름은 만들지 않는다.
...
- `address`는 거래처 범위에서 사용하지 않음
- 거래처 삭제/복구/영구삭제 UI 제거
- `permanentDeleteAt`, 휴지통, 복구 상태 표시 제거
- `ContactLog`, `PersonalMemo` 같은 오래된 명칭을 새 API 계약에 맞게 정리

## 완료 체크리스트

- [x] 거래처 목록 화면이 API 계약과 일치한다.
- [x] 거래처 이름 검색만 제공한다.
- [x] 회사/거래처 부서/거래처 직급 필터가 동작한다.
- [x] 거래처 목록 xlsx 내보내기가 현재 검색어와 필터를 반영한다.
- [x] export API 응답을 blob 다운로드로 처리한다.
- [x] 거래처 생성에서 회사 선택이 필수다.
- [x] 거래처 생성에서 선택적 `contactMemo`가 전송된다.
- [x] 거래처 부서 생성/삭제 UI가 동작한다.
- [x] 거래처 직급 생성/삭제 UI가 동작한다.
- [x] 거래처 상세 화면이 계약 응답값을 표시한다.
- [x] 거래처 수정 화면이 계약 요청값만 전송한다.
...
rg -n "initialMemo|phone|position|address|permanentDeleteAt|DELETE /api/contacts|restore|trash" FE/user-web/src TODO/CONTACT_DOMAIN_PLAN
git diff --check
```
