...
아래 문서를 먼저 읽고, 사용자 페이지 거래처(Contact) 도메인 백엔드 구현을 완료해줘.
...
사용자 페이지 거래처(Contact) 도메인의 DB 스키마, Prisma 마이그레이션, API, 비즈니스 로직, 관측성 로그, API 계약 문서를 구현한다.

거래처는 회사에 소속된 담당자다. `Contact.companyId`는 필수이며, 회사 없이 거래처를 생성하거나 저장하는 흐름은 만들지 않는다.
...
`TODO/CONTACT_DOMAIN_PLAN/COMMON/API-SPEC/CONTACT_API_DETAIL.md`를 기준으로 아래 API를 구현한다.
...
- 거래처 목록 필터는 `companyId`, `contactDepartmentId`, `contactJobGradeId`만 지원한다.
- 거래처 목록 페이지 크기 기본값은 20개다.
...
- 거래처 생성 시 `contactMemo`가 있으면 같은 트랜잭션에서 `ContactMemoLog`를 생성한다.
- 거래처 생성 시 `contactMemo`가 없으면 `ContactMemoLog`를 생성하지 않는다.
- 거래처 생성 시 초기 일반 메모의 `memoType`은 `"초기 메모"`로 저장한다.
- 거래처 일반 메모 수정 API는 `memoType`, `memo`를 모두 수정할 수 있어야 한다.
- 거래처 개인 비밀 메모는 평문을 DB에 저장하지 않는다.
- 거래처 개인 비밀 메모 목록/상세 응답은 복호화된 `memo`를 반환한다.
- 거래처 직급/부서 삭제는 사용 중이면 `409 Conflict`를 반환한다.
- 본 작업에 거래처 삭제/복구/영구삭제 API를 만들지 않는다.
...
rg -n "companyId nullable|initialMemo|ContactLog|PersonalMemo|DELETE /api/contacts|permanentDeleteAt" BE TODO/CONTACT_DOMAIN_PLAN
