...
- 성공 메시지 표시 후 2초 뒤 모달 종료

### Help / Support Request

- 도움말 모달 `지원요청` 탭 진입 시 기능 문의 템플릿 우선 표시
- 문의 유형은 기능 문의, 요금제 문의, 전화 상담 요청, 기능 제안, 기타 문의를 제공
- 템플릿 기본 문구만 있는 상태에서는 `보내기` 비활성화
- 템플릿 외 내용을 작성한 상태에서 문의 유형 변경 시 교체 확인 모달 표시
- 문의 내용 1000자 입력 제한 및 `0/1000` 글자 수 표시
- `보내기` 시 `POST /api/support-requests` JSON 전송
- 성공 메시지 표시 후 2초 뒤 모달 종료
