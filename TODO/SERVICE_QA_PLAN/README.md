...
4. 1차 전체 QA에서는 S0/S1이 아니면 바로 고치지 말고 기록 후 다음 항목으로 이동한다.
5. 로그인 불가, 주요 화면 진입 불가, 데이터 유실, 민감 정보 노출처럼 QA 진행이나 보안에 치명적인 문제만 즉시 수정한다.
6. 1차 전체 QA가 끝난 뒤 `FIX`, `IMPROVE`, `HIDE`, `REMOVE`, `DEFER`, `RETHINK` 항목을 묶어서 수정 라운드로 처리한다.
7. 바로 고친 항목은 `결과=PASS`, `판단=FIX` 또는 `IMPROVE`, `메모=수정 후 재확인 내용`으로 남긴다.
8. 시간이 걸리는 문제는 `결과=FAIL`, `판단=FIX/IMPROVE/HIDE/DEFER/RETHINK`, `Issue ID`를 채운다.

1차 QA의 목적은 전체 화면과 API를 한 번씩 실제로 사용하면서 기능 유지/수정/개선/숨김/제거/보류 판단 근거를 모으는 것이다. UX/UI도 이 단계에서는 먼저 기록하고, 반복적으로 발견되는 문제를 목록/상세/form/dialog/navigation 같은 패턴 단위로 묶어서 고친다.
