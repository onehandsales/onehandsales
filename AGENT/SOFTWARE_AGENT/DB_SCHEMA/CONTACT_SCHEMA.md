...
이 문서는 현재 구현된 거래처(Contact) 도메인의 데이터베이스 구조를 설명한다.
...
- `ContactJobGrade`는 사용자별 거래처 직급 필터 옵션이다.
...
현재 거래처 기본 기능에는 아래 항목을 넣지 않는다.

- 거래처 휴지통과 soft delete 컬럼
- 거래처 삭제/복구/영구삭제 API
- 회사 없이 저장되는 거래처
- 거래처 부서/직급 수정 API
- 거래처 목록의 최근 수정일 응답
...
사용자가 등록한 거래처 담당자 기준 테이블이다. 거래처는 반드시 `Company`에 소속되며, 부서와 직급은 문자열 직접 저장이 아니라 각각 `ContactDepartment`, `ContactJobGrade`의 ID를 FK로 가진다.

| 컬럼 | 타입 | Null | 기본값 | 주석 |
|---|---|---:|---|---|
| `id` | `String @db.Uuid` | 아니오 | `uuid()` | 거래처 PK |
| `userId` | `String @db.Uuid` | 아니오 | 없음 | 거래처를 생성한 내부 `User.id` FK |
| `companyId` | `String @db.Uuid` | 아니오 | 없음 | 거래처가 소속된 `Company.id` FK |
| `username` | `String` | 아니오 | 없음 | 거래처 이름. 거래처 목록 검색 대상 |
| `mobile` | `String` | 아니오 | 없음 | 핸드폰번호. API validation 기준은 `010-1111-2222` 형식 |
| `email` | `String` | 아니오 | 없음 | 이메일 |
| `contactJobGradeId` | `String @db.Uuid` | 아니오 | 없음 | 거래처 직급 `ContactJobGrade.id` FK |
| `contactDepartmentId` | `String @db.Uuid` | 아니오 | 없음 | 거래처 부서 `ContactDepartment.id` FK |
| `createdAt` | `DateTime` | 아니오 | `now()` | 거래처 등록일. 거래처 목록은 이 값을 기준으로 DESC 정렬한다. |
| `updatedAt` | `DateTime` | 아니오 | `@updatedAt` | 거래처 최근 수정일. 거래처 목록 응답에는 포함하지 않고 단건 조회 응답에만 포함한다. |
...
- `userId + createdAt`: 사용자별 거래처 목록 등록일 DESC 정렬 기준
- `userId + username`: 사용자별 거래처 이름 검색 기준
- `userId + companyId`: 사용자별 회사 필터 기준
- `userId + contactDepartmentId`: 사용자별 거래처 부서 필터 기준
- `userId + contactJobGradeId`: 사용자별 거래처 직급 필터 기준
...
거래처 직급 필터 옵션 테이블이다. 사용자가 직접 생성하고 삭제할 수 있지만, 수정은 제공하지 않는다.

| 컬럼 | 타입 | Null | 기본값 | 주석 |
|---|---|---:|---|---|
| `id` | `String @db.Uuid` | 아니오 | `uuid()` | 거래처 직급 PK |
| `userId` | `String @db.Uuid` | 아니오 | 없음 | 직급을 생성한 내부 `User.id` FK |
| `jobGradeName` | `String` | 아니오 | 없음 | 거래처 직급명 |
...
- `userId`: 사용자별 거래처 직급 전체 조회 기준
- `userId + jobGradeName`: 사용자별 직급 중복 검사 기준

주석:

- 거래처 직급 전체 조회 API는 `id`, `jobGradeName`만 반환한다.
- 이미 거래처에 매핑된 직급은 삭제할 수 없다.
- 삭제 성공 시 API는 `204 No Content`를 반환한다.

## 6. Table: ContactDepartment

거래처 부서 필터 옵션 테이블이다. 사용자가 직접 생성하고 삭제할 수 있지만, 수정은 제공하지 않는다.

| 컬럼 | 타입 | Null | 기본값 | 주석 |
|---|---|---:|---|---|
| `id` | `String @db.Uuid` | 아니오 | `uuid()` | 거래처 부서 PK |
| `userId` | `String @db.Uuid` | 아니오 | 없음 | 부서를 생성한 내부 `User.id` FK |
| `departmentName` | `String` | 아니오 | 없음 | 거래처 부서명 |
...
- `userId`: 사용자별 거래처 부서 전체 조회 기준
- `userId + departmentName`: 사용자별 부서 중복 검사 기준

주석:

- 거래처 부서 전체 조회 API는 `id`, `departmentName`만 반환한다.
- 이미 거래처에 매핑된 부서는 삭제할 수 없다.
- 삭제 성공 시 API는 `204 No Content`를 반환한다.

## 7. Table: ContactMemoLog

거래처 일반 메모 로그 테이블이다. 거래처 생성 시 `contactMemo`가 있으면 첫 데이터로 저장한다.

| 컬럼 | 타입 | Null | 기본값 | 주석 |
|---|---|---:|---|---|
| `id` | `String @db.Uuid` | 아니오 | `uuid()` | 거래처 일반 메모 로그 PK |
...
| `memo` | `String` | 아니오 | 없음 | 거래처 일반 메모 원문 |
...
- `contactId + createdAt`: 거래처 단건 화면의 일반 메모 로그 무한스크롤 기준
- `userId + contactId`: 사용자 ownership 검증 기준

주석:

- 거래처 생성 시 `contactMemo`로 만들어지는 첫 메모 로그는 서버가 `memoType`을 `초기 메모`로 저장한다.
...
거래처별 사용자 비밀 메모 로그 테이블이다. 메모 원문은 데이터베이스에 암호화된 값으로만 저장한다.

| 컬럼 | 타입 | Null | 기본값 | 주석 |
|---|---|---:|---|---|
| `id` | `String @db.Uuid` | 아니오 | `uuid()` | 거래처 개인 비밀 메모 로그 PK |
| `contactId` | `String @db.Uuid` | 아니오 | 없음 | 메모가 속한 `Contact.id` FK |
...
- `contactId + createdAt`: 거래처 단건 화면의 개인 비밀 메모 무한스크롤 기준
- `userId + contactId`: 작성자 본인 조회와 복호화 권한 검증 기준

주석:

- 독립적인 거래처 개인 비밀 메모 로그 생성 API는 `memo`만 필수로 받는다.
