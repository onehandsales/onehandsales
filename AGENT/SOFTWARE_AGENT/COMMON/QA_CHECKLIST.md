# onehand.sales QA 체크리스트

작성일: 2026-07-06
전략 보강: 2026-08-11 `AGENT/PM_AGENT/DECISIONS/030_global_b2c_closeout_and_paddle_defer.md`

이 문서는 `BE`, `FE`, `AGENT` 문서를 기준으로 만든 QA 체크리스트입니다. QA를 처음 진행하는 사람이 그대로 따라가며 확인할 수 있도록 기능 QA와 UX/UI QA를 함께 정리합니다.

현재 QA 대상 제품은 `Web + 반응형 Web + 모바일 브라우저 Web`입니다. 모바일 QA는 모바일 브라우저에서 현재 Web 제품의 핵심 업무가 사용 가능한지 확인하는 기준으로 진행합니다.

2026-08-11 PM 전략 기준으로, Global B2C 01~11 foundation은 완료 archive입니다. 지금 바로 해야 할 일은 Paddle checkout 구현이 아니라 기존 기능 유지보수, UX/UI 상품성 개선, 결제창 없는 100명 베타 준비입니다. 이 체크리스트는 그 과정에서 핵심 흐름과 반응형/접근성/보안 품질을 확인하는 기준으로 사용합니다.

## 1. QA 목적

QA는 제품이 의도한 대로 동작하는지, 사용자가 실제 업무를 끝까지 수행할 수 있는지, 데이터가 안전하게 저장되는지 확인하는 과정입니다.

이번 QA의 핵심 질문은 다음과 같습니다.

- 사용자가 로그인 후 핵심 영업 업무를 끝까지 처리할 수 있는가?
- 회사, 담당자, 제품, 딜, 일정, 회의록 데이터가 정확하게 생성/조회/수정/삭제/복구되는가?
- 화면이 데스크톱, 태블릿, 모바일 브라우저에서 깨지지 않고, 사용자가 다음 행동을 이해할 수 있는가?
- 잘못된 입력, 서버 오류, 권한 오류, 삭제된 데이터 접근 같은 예외 상황에서 앱이 무너지지 않는가?
- 현재 구현 범위가 아닌 기능이 사용자에게 잘못 노출되지 않는가?

## 2. 이번 QA 범위

### 포함

- `BE`
  - NestJS API
  - Prisma schema/migration/seed
  - 인증/권한
  - 회사, 담당자, 제품, 딜, 일정, 회의록
  - 검색
  - 휴지통
  - 도메인별 XLSX export
  - Weekly Schedule Report
  - Google Calendar Integration
  - AI Weekly Sales Report/Follow-up
  - DealActivity
  - Product Analytics

- `FE/user-web`
  - 로그인
  - 보호 라우팅
  - 홈
  - 회사, 담당자, 제품, 딜
  - 일정
  - 회의록
  - 검색
  - 휴지통
  - 설정/더보기
  - UX/UI, 반응형, 모바일 브라우저, 접근성 기본

- `FE/admin-web`
  - 관리자 token 통과, non-admin 차단, `GET /admin/api/me` 호출을 확인합니다.

### 제외

사용자 요청에 따라 아래 기능은 이번 QA에서 제외합니다.

- 과금 및 구독 결제

현재 구현 상태 기준으로 아래 항목도 실패로 처리하지 않고 `N/A` 또는 `후속 범위`로 기록합니다.


## 3. 참고 기준 문서

QA 판단 기준은 아래 파일을 우선합니다.

- `AGENT/PM_AGENT/PLANNING/IMPLEMENTATION_STATUS.md`
- `AGENT/PM_AGENT/PLANNING/MVP_SCOPE.md`
- `AGENT/PM_AGENT/PLANNING/PRD.md`
- `AGENT/PM_AGENT/PLANNING/GLOBAL_B2C_SERIES_A_ROADMAP.md`
- `AGENT/PM_AGENT/DECISIONS/029_global_b2c_series_a_priority.md`
- `AGENT/PM_AGENT/DECISIONS/030_global_b2c_closeout_and_paddle_defer.md`
- `AGENT/UXUI_AGENT/UX_REVIEW_CHECKLIST.md`
- `AGENT/UXUI_AGENT/PLANNING/USER_FLOW_AND_SCREENS.md`
- `AGENT/UXUI_AGENT/PLANNING/UX_UI_DIRECTION.md`
- `AGENT/UXUI_AGENT/PLANNING/UX_WRITING_GUIDE.md`
- `AGENT/SOFTWARE_AGENT/BACKEND_AGENT/ARCHITECTURE/TESTING.md`
- `AGENT/SOFTWARE_AGENT/FRONT_AGENT/ARCHITECTURE/TESTING.md`
- `FE/user-web/src/app/router/router.tsx`
- `FE/admin-web/src/app/router/router.tsx`
- `BE/src/modules/**/presentation/http/*.controller.ts`

## 4. QA 결과 표기법

각 항목은 아래 상태 중 하나로 기록합니다.

- `[PASS]`: 기대한 대로 동작함
- `[FAIL]`: 기대 결과와 다르게 동작함
- `[BLOCKED]`: 환경, 계정, 외부 API, 데이터 문제 때문에 확인 불가
- `[N/A]`: 현재 제품 범위가 아니거나 이번 QA 제외 범위
- `[NEEDS CHECK]`: 애매해서 기획/개발 확인 필요

버그를 발견하면 반드시 아래 4가지를 남깁니다.

- 재현 경로
- 기대 결과
- 실제 결과
- 화면 캡처 또는 콘솔/API 에러

## 5. 심각도 기준

- `S0 Blocker`: 로그인 불가, 앱 실행 불가, 핵심 데이터 전체 접근 불가
- `S1 Critical`: 데이터 손실, 다른 사용자 데이터 노출, 결제/권한/보안급 문제
- `S2 Major`: 핵심 업무 흐름 실패, 생성/수정/삭제/복구 실패
- `S3 Minor`: 일부 화면/필터/문구/정렬 문제, 우회 가능
- `S4 Polish`: 시각적 어색함, 문구 다듬기, 정렬 간격 개선

## 6. QA 전 준비

### 공통 환경

- [ ] Node 버전이 24.x인지 확인
- [ ] pnpm 사용 가능 여부 확인
- [ ] Docker 실행 가능 여부 확인
- [ ] `BE/.env` 존재 여부 확인
- [ ] `FE/user-web/.env` 존재 여부 확인
- [ ] `FE/admin-web/.env` 존재 여부 확인 또는 Admin QA 제외 사유 기록
- [ ] 실제 `.env` 값은 문서나 이슈에 복사하지 않음
- [ ] `.env.example`, `.env.local`이 아니라 각 앱의 `.env`와 공통 환경 문서를 정본으로 삼는다고 기록
- [ ] 환경 변수명과 용도는 `AGENT/SOFTWARE_AGENT/COMMON/ENVIRONMENT.md` 기준으로 확인

확인 명령:

```powershell
node -v
pnpm -v
docker --version
```

PowerShell 실행 정책 때문에 `pnpm`이 막히면 같은 명령을 `pnpm.cmd`로 실행합니다.

### 로컬 서버 기준

- BE 기본 주소: `http://localhost:3000`
- User Web 기본 주소: `http://localhost:5173`
- Admin Web 기본 주소: `http://localhost:5174`
- BE health check: `GET http://localhost:3000/api/health`

### DB 준비

```powershell
cd BE
pnpm db:dev:up
pnpm prisma:validate
pnpm prisma:generate
pnpm prisma:migrate
pnpm prisma:seed
```

- [ ] DB 컨테이너가 정상 실행됨
- [ ] Prisma schema validation 성공
- [ ] Prisma client generate 성공
- [ ] migration 성공
- [ ] 로컬 DB smoke가 필요하면 seed 성공
- [ ] 실제 Supabase OAuth QA를 할 때는 테스트 사용자/CRM 데이터를 초기화한 깨끗한 DB에서 시작

### 2026-07-09 QA 전 준비 확인 결과

- [x] Node v24.18.0 확인
- [x] pnpm 8.14.1 확인
- [x] Docker 29.5.3 확인
- [x] Docker daemon 실행 확인
- [x] `BE/.env` 존재 확인
- [x] `FE/user-web/.env` 존재 확인
- [x] `FE/admin-web/.env` 존재 확인
- [x] `.env` 실제 값은 문서에 복사하지 않음
- [x] `.env`와 공통 환경 문서를 정본으로 삼는 정책 확인. `.env.example`, `.env.local`은 현재 정본이 아님
- [x] 환경 변수명과 용도는 `AGENT/SOFTWARE_AGENT/COMMON/ENVIRONMENT.md`에 문서화
- [x] 로컬 Postgres 컨테이너 `sales_b2c_postgres` healthy 확인
- [x] `pnpm prisma:validate` 성공
- [x] BE health check `http://localhost:3000/api/health` 200 확인
- [x] User Web `/` 200 확인
- [x] User Web legacy `/login`이 선호 locale login URL로 redirect됨을 확인
- [x] DB 연결 대상 확인: `DATABASE_URL`과 `DIRECT_URL` 모두 Supabase pooler의 `postgres` DB를 가리킴. 실제 URL 값은 기록하지 않음
- [x] DB 데이터 확인: `User` 3, `UserOAuthAccount` 3, `AuthDevice` 4, `AuthSession` 5
- [ ] `pnpm prisma:generate` 미완료. 실행 중인 BE 프로세스가 Prisma query engine DLL을 잡고 있어 `EPERM rename` 발생. BE를 중지한 뒤 재실행 필요
- [ ] migration 기록 정합성 미완료. 현재 `BE/.env`의 Prisma datasource는 Supabase pooler를 가리키며, `_prisma_migrations`에는 `20260611000000_add_company_domain` 1건만 있고 `finished_at`이 비어 있음. `pnpm exec prisma migrate status`에서도 이후 migration들이 미적용으로 표시됨. 수동 QA 전 데이터 조회는 가능하지만, 배포/DB 운영 전 migration 기록 정리가 필요함
- [ ] seed 미실행. 실제 Supabase OAuth/CRM QA 데이터와 섞일 수 있어 자동 실행하지 않음

## 7. 자동 점검 체크리스트

### BE 자동 점검

```powershell
cd BE
pnpm typecheck
pnpm lint
pnpm test
pnpm build
```

- [x] `pnpm typecheck` 성공
- [x] `pnpm lint` 성공
- [x] `pnpm test` 성공. 17 suites / 82 tests passed
- [x] `pnpm build` 성공
- [x] 실패 없음. 별도 에러 로그 저장 불필요
- [x] 테스트 실패 없음

### FE/user-web 자동 점검

```powershell
cd FE/user-web
pnpm typecheck
pnpm lint
pnpm build
pnpm test:e2e
```

- [x] `pnpm typecheck` 성공
- [x] `pnpm lint` 성공. 기존 Fast Refresh warning 1건 있음: `src/app/router/router.tsx`
- [x] `pnpm build` 성공. Vite chunk size warning 있음
- [x] `pnpm test:e2e` 성공. Chromium 1 test passed
- [x] E2E 실패 없음. 스크린샷/trace 기록 불필요
- [x] E2E가 현재 로그인 UI와 실제 구현 범위 기준으로 통과함

### FE/admin-web 자동 점검

관리자 페이지는 현재 auth/role smoke만 QA 포함 대상입니다.

```powershell
cd FE/admin-web
pnpm typecheck
pnpm lint
pnpm build
pnpm test:e2e
```

- [x] `pnpm typecheck` 성공 또는 제외 사유 기록
- [x] `pnpm lint` 성공 또는 제외 사유 기록
- [x] `pnpm build` 성공 또는 제외 사유 기록
- [x] `pnpm test:e2e` 성공 또는 환경/fixture 이슈를 별도 기록

### FE/admin-web 자동 점검 결과 기록 기준

- 최신 실행 결과는 `TODO/SERVICE_QA_PLAN/COMMON/QA-RESULTS.md`에 기록한다.
- `FE/admin-web/tests/e2e/admin-web-smoke.spec.ts`는 현재 auth/role smoke 기준이어야 한다.

## 8. 수동 QA 기본 시나리오

처음 QA할 때는 아래 시나리오를 먼저 끝까지 진행합니다.

### 2026-07-09 기능 수동 QA 결과

- [x] 로그인: Google 로그인 동작. Kakao 로그인은 제품 로그인 기능에서 제거
- [x] 회사 생성: 기본 동작 통과
- [x] 담당자 생성: 기본 동작 통과
- [x] 제품 생성: 기본 동작 통과
- [x] 딜 생성: 기본 동작 통과
- [x] 일정 생성: 기본 동작 통과
- [x] 회의록 생성: 최초 수동 QA에서 `InternalServerError`로 저장 실패했으나, 최신 BE 재시작 후 실제 `POST /api/meeting-notes` `201 Created` 확인 및 UI 재시도 성공
- [x] 검색: 동작
- [x] 각 목록에서 조회/필터/정렬 확인: 동작
- [x] 데이터 수정: 동작
- [x] 도메인별 XLSX 다운로드: 동작
- [x] 삭제: 동작
- [x] 휴지통에서 복구: 동작
- [x] 복구된 데이터 재확인: 동작

### 2026-07-09 기능 QA 후속 항목

- `BUG-001`: 회의록 생성 시 최초 수동 QA에서 `InternalServerError`로 저장 실패했으나 최신 BE 재시작 후 API 직접 호출과 UI 재시도 모두 성공. 원인 후보는 오래된 `BE/dist/main` 프로세스 사용이다. 상태: Resolved
- `FOLLOW-UP-001`: 08_GLOBAL_DATA_I18N 완료 기준 Google/LINE/Apple은 runtime provider다. 실제 provider smoke는 Supabase/provider 운영 설정과 secret 준비 후 별도 QA로 확인
- `FOLLOW-UP-002`: 일정이 9일~10일처럼 여러 날짜에 걸칠 때 Google Calendar처럼 9일과 10일 양쪽에 표시되어야 함
- `FOLLOW-UP-003`: 회사/담당자/제품/딜/일정 생성 UXUI 개선 필요. 이번 기능 QA의 실패로 보지는 않음

이 흐름이 통과하면 MVP 핵심 업무가 대체로 살아 있다고 볼 수 있습니다.

## 9. 인증/세션 QA

### 로그인

- [x] 현재 노출 언어 URL locale smoke: `/ko`, `/en-us/pricing`, `/en-ca/privacy`가 public/auth 화면으로 진입함
- [x] Compatibility redirect smoke: `/`, `/login`, `/signup`, `/pricing`, `/contact`, `/about`, `/security`, `/terms`, `/privacy`가 선호 locale URL로 redirect됨
- [x] OAuth callback smoke: `/auth/callback`은 locale prefix 없이 유지되고 Supabase session exchange를 시도함
- [x] `/app/*`는 locale prefix 없이 유지되고, 비로그인 접근 시 선호 locale의 login URL로 이동함

- [x] `/` 접근 시 공개 랜딩/진입 화면이 보임
- [x] legacy `/login` 접근 시 선호 locale login URL의 로그인 화면으로 이동함
- [x] 로그인 전 `/app` 보호 라우트 접근 시 로그인으로 이동
- [x] 개발용 mock login 버튼이나 mock session 진입점이 노출되지 않음
- [x] Google provider 버튼 클릭 시 현재 로그인 페이지를 유지한 채 OAuth authorize URL이 browser popup으로 열림
- [x] Google OAuth login/signup이 성공함
- [x] 로그인 후 `/app` 홈으로 진입함
- [x] 로그인 후 사용자 이름/프로필 정보가 표시됨
- [ ] 로그인 후 앱 DB에 `User`, `UserOAuthAccount`, `AuthDevice`, `AuthSession`이 생성 또는 갱신됨
- [x] 새로고침 후 로그인 상태가 기대한 정책대로 유지됨
- [N/A] 만료된 토큰 상태에서 refresh 또는 재로그인 흐름이 자연스러움. 이번 배포 수동 QA 범위에서 제외
- [x] 로그아웃 후 선호 locale의 login URL로 이동함
- [x] 로그아웃 후 보호 라우트 접근이 차단됨
- [N/A] 로그인 실패 시 빈 화면이나 무한 로딩이 아니라 이해 가능한 에러가 표시됨. 이번 배포 수동 QA 범위에서 제외

### 2026-07-10 URL locale smoke 결과

- 현재 노출 언어 기준 Playwright smoke 대상은 `/ko`, `/en-us/pricing`, `/en-ca/privacy`다.
- 2026-07-10 당시 전체 route smoke 기록에는 `/ja/login`, `/en-gb/contact`, `/en-sg/about`, `/en-au/security`도 포함됐지만, 이 locale들은 현재 언어 선택 UI에서 보류 상태다.
- fresh `ko-KR` context 기준 legacy public URL이 `/ko` 계열로 redirect됨을 확인했다.
- 저장된 선호 언어가 `en-CA`일 때 legacy `/login`과 비로그인 `/app/deals`가 `/en-ca/login`으로 이동하는지를 현재 KR/US/CA smoke 기준으로 확인한다.
- `/auth/callback`은 locale prefix 없이 `/auth/callback`에 유지됨을 확인했다.

### Provider 상태

- [x] Google OAuth는 실제 provider smoke로 가입/로그인 확인
- [N/A] Kakao OAuth는 로그인 기능에서 제거되어 provider QA 대상이 아님
- [ ] LINE/Apple OAuth 실제 provider smoke 또는 Supabase/provider 환경 미구성 N/A 사유 기록

### 기기/세션 정책

- [x] 데스크톱 웹 로그인은 `personal_laptop` slot으로 등록됨
- [N/A] 모바일 폭 브라우저 로그인은 `mobile` slot으로 등록됨. 이번 배포 수동 QA 범위에서 제외
- [N/A] 같은 slot의 다른 브라우저/기기 로그인은 기존 active device/session을 교체함. 이번 배포 수동 QA 범위에서 제외
- [N/A] 같은 브라우저 재로그인은 새 session row를 무한히 늘리지 않고 기존 active session을 재사용/회전함. 이번 배포 수동 QA 범위에서 제외

### 권한/API 보호

- [x] access token 없이 보호 API 호출 시 401
- [x] 잘못된 token으로 보호 API 호출 시 401
- [x] 일반 사용자가 `GET /admin/api/me`를 호출하면 403 또는 접근 차단
- [x] FE User Web API client가 일반 `/api/*` 계약만 사용함
- [N/A] 다른 사용자의 UUID를 추측해 조회/수정/삭제할 수 없음. 이번 배포 수동 QA 범위에서 제외

### 2026-07-10 권한/API 보호 smoke 결과

- 실제 로컬 BE HTTP smoke로 보호 API 인증 없음 401, 잘못된 token 401을 확인했다.
- BE `AdminGuard` 자동 테스트로 일반 사용자 `GET /admin/api/me` 접근 403을 확인했다.
- User Web `apiClient`/`apiBlobClient`가 일반 `/api/*` 계약만 사용함을 코드로 확인했다.

### 2026-07-09 배포 사이트 인증/세션 QA 결과

- Google 로그인, `/app` 진입, 새로고침 후 세션 유지, 새 탭 `/app` 세션 유지, 로그아웃 후 선호 locale login URL 이동, 로그아웃 후 뒤로가기 보호, 재로그인 후 기존 CRM 데이터 유지, 설정/계정 화면 사용자 정보 표시를 확인했다.
- 가입 국가/마지막 로그인 국가는 `기록 없음`이어도 현재 정책상 정상으로 본다.
- Apple/LINE provider, 만료 토큰 강제 테스트, 모바일 여러 대 동시 로그인, 관리자 권한 침투성 테스트는 2026-07-09 QA 범위에서 제외했다. Apple/LINE은 08 G08에서 구현/QA 대상으로 승격한다.

## 10. 홈 QA

- [x] `/app` 홈이 정상 로딩됨
- [x] 오늘 일정이 보임
- [x] 진행 중인 딜 요약이 보임
- [x] 마감 임박 딜 또는 다음 행동이 보임
- [x] 최근 회의록이 보임
- [N/A] 데이터가 없을 때 empty state가 보임. 이번 기능 QA 범위에서 제외
- [N/A] empty state에서 다음 행동이 명확함. 이번 기능 QA 범위에서 제외
- [x] 홈 카드/리스트 클릭 시 올바른 상세 화면으로 이동
- [N/A] 로딩 중 skeleton 또는 loading state가 보임. 이번 기능 QA 범위에서 제외
- [N/A] API 실패 시 재시도 가능한 에러 상태가 보임. 이번 기능 QA 범위에서 제외
- [N/A] 모바일에서 홈 정보가 너무 길게 밀리거나 겹치지 않음. 이번 기능 QA 범위에서 제외

### 2026-07-09 홈 기능 QA 결과

- 배포 사이트 기준 홈 기능은 동작 확인 완료.
- 홈에 표시되는 정보와 구성은 추후 전체적으로 변경될 예정이므로, 이번 QA에서는 데이터 표시와 클릭 이동만 통과 기준으로 본다.
- UX/UI, empty/loading/error state, 모바일 레이아웃 평가는 이번 범위에서 제외한다.

## 11. 회사 QA

### 목록

- [x] `/app/companies` 진입 가능
- [x] 회사 목록이 로딩됨
- [x] 회사명, 분야, 지역, 담당자 수, 딜 수 등 주요 정보가 보임
- [x] 회사명 검색이 동작함
- [x] 분야 필터가 동작함
- [x] 지역 필터가 동작함
- [x] 정렬이 동작함
- [N/A] 페이지네이션이 동작함. 이번 기능 QA에서 별도 확인하지 않음
- [N/A] 목록 empty state가 적절함. 이번 기능 QA 범위에서 제외
- [N/A] 로딩/에러 상태가 깨지지 않음. 이번 기능 QA 범위에서 제외

### 생성

- [x] 회사 추가 버튼이 오른쪽 문서형 생성 패널을 연다
- [x] `/app/companies/new` 진입 시 회사 목록을 유지하고 생성 패널이 열린다
- [x] 데스크톱 생성 패널이 화면 최상단~최하단까지 붙어 있다
- [x] 생성 패널 좌우 resize가 동작한다
- [x] 생성 패널을 넓혀도 회사 목록 컬럼 개수가 줄어들지 않는다
- [x] 공간이 부족한 경우 회사 목록은 가로 스크롤로 모든 컬럼을 유지한다
- [N/A] 필수값 누락 시 저장이 막힘. 이번 기능 QA에서 별도 확인하지 않음
- [x] 회사명 입력 후 생성 가능
- [x] 분야 선택 또는 새 분야 생성 가능
- [x] 지역 선택 또는 새 지역 생성 가능
- [x] 메모 입력 가능
- [x] 저장 성공 후 목록에 새 회사가 보임
- [N/A] 저장 성공 후 성공 피드백이 보임. 이번 UX/UI QA 범위에서 제외
- [N/A] 저장 실패 시 입력값이 사라지지 않음. 이번 기능 QA 범위에서 제외
- [N/A] 중복 클릭 시 중복 생성되지 않음. 이번 기능 QA 범위에서 제외

### 상세/수정

- [x] 회사 상세 진입 가능
- [x] 회사 기본 정보가 정확히 보임
- [x] 연결된 담당자 목록이 보임
- [x] 연결된 딜 목록이 보임
- [x] 회사명/분야/지역/메모 수정 가능
- [x] 수정 후 새로고침해도 변경값 유지
- [N/A] 일반 메모 로그 생성/수정/삭제 가능. 이번 기능 QA에서 별도 확인하지 않음
- [N/A] 개인 메모 로그 생성/수정/삭제 가능. 이번 기능 QA에서 별도 확인하지 않음
- [N/A] 개인 메모가 일반 메모와 시각적으로 구분됨. 이번 UX/UI QA 범위에서 제외

### 삭제/복구

- [N/A] 회사 삭제 전 확인 dialog가 보임. 이번 UX/UI QA 범위에서 제외
- [x] 삭제 후 회사 목록에서 사라짐
- [x] 삭제된 회사가 휴지통에 보임
- [N/A] 삭제된 회사 상세 직접 접근 시 정책에 맞는 에러 또는 삭제 상태 표시. 이번 기능 QA에서 별도 확인하지 않음
- [x] 휴지통에서 회사 복구 가능
- [x] 복구 후 회사 목록에 다시 보임
- [x] 복구 후 연결된 담당자/딜 관계가 기대대로 유지됨

### Export

- [x] 회사 목록에서 XLSX 다운로드 가능
- [x] 다운로드 파일이 열림
- [N/A] 헤더가 이해 가능함. 이번 UX/UI QA 범위에서 제외
- [x] 회사 데이터가 누락되지 않음
- [N/A] 검색/필터 적용 여부가 제품 정책과 일치함. 이번 기능 QA에서 별도 확인하지 않음

### 2026-07-09 회사 기능 QA 결과

- 배포 사이트 기준 회사 목록, 검색, 분야/지역 필터, 정렬, 생성, 상세, 수정, 삭제, 휴지통 복구, XLSX 다운로드 동작 확인 완료.
- UX/UI, empty/loading/error state, 중복 클릭, 실패 복구, 메모 로그 세부 동작은 이번 범위에서 제외한다.

## 12. 담당자 QA

### 목록

- [x] `/app/contacts` 진입 가능
- [x] 담당자 목록이 로딩됨
- [x] 이름, 회사, 부서, 직급, 휴대폰, 이메일이 보임
- [x] 이름 검색이 동작함
- [x] 회사 필터가 동작함
- [x] 부서 필터가 동작함
- [x] 직급 필터가 동작함
- [x] 정렬/페이지네이션이 동작함
- [N/A] empty/loading/error state가 적절함. 이번 기능 QA 범위에서 제외

### 생성

- [N/A] 담당자 추가 버튼이 명확함. 이번 UX/UI QA 범위에서 제외
- [N/A] 이름 필수값 검증이 동작함. 실패/예외 케이스 QA에서 확인
- [x] 회사 연결 가능
- [x] 부서 선택 또는 새 부서 생성 가능
- [x] 직급 선택 또는 새 직급 생성 가능
- [N/A] 휴대폰 형식 검증이 동작함. 실패/예외 케이스 QA에서 확인
- [N/A] 이메일 형식 검증이 동작함. 실패/예외 케이스 QA에서 확인
- [x] 저장 후 목록에 표시됨
- [x] 저장 후 연결 회사 상세에서도 담당자가 보임
- [N/A] 저장 실패 시 입력값 유지. 실패/예외 케이스 QA에서 확인

### 상세/수정

- [x] 담당자 상세 진입 가능
- [x] 기본 정보가 정확히 보임
- [x] 연결 회사로 이동 가능
- [x] 연결된 딜 목록이 보임
- [x] 이름/회사/부서/직급/휴대폰/이메일 수정 가능
- [N/A] 일반 메모 로그 생성/수정/삭제 가능. 이번 기능 QA에서 별도 확인하지 않음
- [N/A] 개인 메모 로그 생성/수정/삭제 가능. 이번 기능 QA에서 별도 확인하지 않음
- [N/A] 민감 가능 정보가 불필요하게 강조되거나 노출되지 않음. 이번 UX/UI QA 범위에서 제외

### 삭제/복구/Export

- [x] 담당자 삭제 가능
- [x] 삭제 후 목록에서 사라짐
- [x] 휴지통에서 복구 가능
- [x] 복구 후 회사/딜 연결이 기대대로 유지됨
- [x] 담당자 XLSX 다운로드 가능
- [N/A] XLSX에 이메일/휴대폰 값이 정책대로 포함됨. 이번 기능 QA에서 별도 확인하지 않음

### 2026-07-09 담당자 기능 QA 결과

- 배포 사이트 기준 담당자 목록, 검색, 회사/부서/직급 필터, 정렬/페이지네이션, 생성, 회사 연결, 상세, 수정, 삭제, 휴지통 복구, XLSX 다운로드 동작 확인 완료.
- 필수값/형식 검증, 저장 실패 복구, 메모 로그 세부 동작, 민감정보 표시, UX/UI는 이번 성공 케이스 QA 범위에서 제외한다.

## 13. 제품 QA

### 목록

- [x] `/app/products` 진입 가능
- [x] 제품 목록이 로딩됨
- [x] 제품명, 카테고리, 상태, 가격, 연결 딜 수가 보임
- [x] 제품명 검색이 동작함
- [x] 카테고리 필터가 동작함
- [x] 상태 필터가 동작함
- [x] 정렬/페이지네이션이 동작함

### 생성/수정

- [N/A] 제품 추가 버튼이 명확함. 이번 UX/UI QA 범위에서 제외
- [N/A] 제품명 필수값 검증이 동작함. 실패/예외 케이스 QA에서 확인
- [N/A] 가격 숫자 입력 검증이 동작함. 실패/예외 케이스 QA에서 확인
- [N/A] 음수/비정상 가격 입력이 막히거나 명확히 처리됨. 실패/예외 케이스 QA에서 확인
- [x] 카테고리 선택 또는 새 카테고리 생성 가능
- [x] 상태 선택 또는 새 상태 생성 가능
- [x] 저장 후 목록에 보임
- [x] 상세에서 제품 정보 수정 가능
- [x] 수정 후 연결된 딜 화면에도 기대대로 반영됨

### 메모/삭제/복구/Export

- [N/A] 일반 메모 로그 생성/수정/삭제 가능. 이번 기능 QA에서 별도 확인하지 않음
- [N/A] 개인 메모 로그 생성/수정/삭제 가능. 이번 기능 QA에서 별도 확인하지 않음
- [x] 제품 삭제 가능
- [x] 삭제 후 목록에서 사라짐
- [x] 휴지통에서 복구 가능
- [x] 복구 후 딜 연결이 기대대로 유지됨
- [x] 제품 XLSX 다운로드 가능

### 2026-07-09 제품 기능 QA 결과

- 배포 사이트 기준 제품 목록, 검색, 카테고리/상태 필터, 정렬/페이지네이션, 생성, 카테고리/상태 선택 또는 새 생성, 상세, 수정, 삭제, 휴지통 복구, XLSX 다운로드 동작 확인 완료.
- 필수값/가격 형식/음수 입력 검증, 저장 실패 복구, 메모 로그 세부 동작, UX/UI는 이번 성공 케이스 QA 범위에서 제외한다.

## 14. 딜 QA

### 목록/파이프라인

- [x] `/app/deals` 진입 가능
- [x] 딜 목록이 로딩됨
- [x] 딜이름, 금액, 단계, 회사, 담당자, 제품, 다음 행동, 예상 마감일이 보임
- [x] 단계별 count가 실제 목록과 일치함
- [x] 단계 필터가 동작함
- [x] 키워드 검색이 동작함
- [x] 회사 필터가 동작함
- [x] 담당자 필터가 동작함
- [x] 정렬/페이지네이션이 동작함
- [N/A] 모바일에서 가로 칸반이나 깨지는 테이블 없이 사용 가능함. 이번 UX/UI QA 범위에서 제외

### 생성

- [N/A] 딜 추가 버튼이 명확함. 이번 UX/UI QA 범위에서 제외
- [N/A] 딜이름 필수값 검증이 동작함. 실패/예외 케이스 QA에서 확인
- [N/A] 금액 숫자 입력 검증이 동작함. 실패/예외 케이스 QA에서 확인
- [x] 회사 연결 가능
- [x] 담당자 연결 가능
- [x] 제품 연결 가능
- [x] 단계 선택 가능
- [x] 예상 마감일 입력 가능
- [x] 다음 행동 입력 가능
- [x] 메모 입력 가능
- [x] 저장 후 딜 상세로 이동하거나 목록에 표시됨
- [x] 연결된 회사/담당자/제품 상세에서도 딜이 보임

### 상세/수정

- [x] 딜 상세 진입 가능
- [x] 기본 정보가 정확히 보임
- [x] 회사/담당자/제품 연결 정보가 정확히 보임
- [x] 단계 변경 가능
- [x] 단계 변경 후 count가 갱신됨
- [x] 금액/예상 마감일/다음 행동 수정 가능
- [x] 연결 회사/담당자/제품 추가/제거 가능
- [x] 수정 후 새로고침해도 유지됨

### 팔로잉 액션/메모

- [x] 팔로잉 액션 로그 목록이 보임
- [x] 팔로잉 액션 생성 가능
- [x] 완료 체크 가능
- [x] 팔로잉 액션 수정 가능
- [x] 팔로잉 액션 삭제 가능
- [x] 딜 메모 생성/수정/삭제 가능
- [N/A] 다음 행동이 목록/상세/홈에서 1급 정보로 보임. 이번 UX/UI QA 범위에서 제외

### 삭제/복구/Export

- [x] 딜 삭제 가능
- [x] 삭제 후 목록에서 사라짐
- [x] 휴지통에서 복구 가능
- [x] 복구 후 연결 관계가 기대대로 유지됨
- [N/A] 삭제된 회사/담당자/제품이 연결된 딜에서 삭제 상태로 이해 가능하게 보임. 이번 UX/UI QA 범위에서 제외
- [x] 딜 XLSX 다운로드 가능

### 2026-07-09 딜 기능 QA 결과

- 배포 사이트 기준 딜 목록, 단계 count, 단계/키워드/회사/담당자 필터, 정렬/페이지네이션, 생성, 회사/담당자/제품 연결, 상세, 수정, 팔로잉 액션, 메모, 삭제, 휴지통 복구, XLSX 다운로드 동작 확인 완료.
- 필수값/금액 형식 검증, 삭제된 연결 데이터 표시, 모바일 레이아웃, UX/UI는 이번 성공 케이스 QA 범위에서 제외한다.

## 15. 일정 QA

### 목록

- [x] `/app/schedules` 진입 가능
- [x] 일정 목록 또는 캘린더가 로딩됨
- [x] 일정 제목, 시작/종료 시간, 장소, 연결 딜이 보임
- [x] 날짜 이동 또는 필터가 있다면 정상 동작함
- [x] 오늘 일정이 홈과 일치함

### 생성/수정

- [x] 일정 생성 버튼이 명확함
- [x] 제목 필수값 검증이 동작함
- [x] 시작 시간이 종료 시간보다 늦으면 저장이 막힘
- [x] `Asia/Seoul` 기준 시간이 의도대로 표시됨
- [x] 장소 입력 가능
- [x] 메모 입력 가능
- [x] 딜 연결 가능
- [x] 저장 후 목록에 보임
- [x] 상세에서 수정 가능
- [x] 새로고침 후 시간 표시가 바뀌지 않음

### 삭제

- [x] 일정 삭제 가능
- [x] 2026-07-09 QA 당시에는 hard delete로 확인했으나, 04 Google Calendar Integration 이후 현재 정본은 Schedule soft delete/Trash 대상이다.
- [x] 삭제 후 목록에서 사라짐
- [x] 현재 기준 삭제한 일정은 Trash 정책과 Google Calendar sync 정책에 맞게 복구 가능성을 확인해야 한다.

### 주간 경로

- [x] `/app/schedules/week` 접근 시 주간 일정 보고서 화면이 활성 표시되는 것을 확인했다.
- [x] 일정 화면의 `주간 보고서` link가 `/app/schedules/week`로 이동하는 것을 확인했다.

### 2026-07-09 일정 기능 QA 결과

- 배포 사이트 기준 일정 목록, 생성, 상세, 수정, 시간 표시, 딜 연결, 새로고침 후 유지, 삭제, 주간 경로 redirect 동작 확인 완료. 단, 삭제 정책은 이후 04 Google Calendar Integration에서 soft delete/Trash로 변경됐다.
- 현재는 기능 QA 기준 통과로 기록한다. 세부 UX/UI와 모바일 레이아웃 평가는 별도 UX/UI QA에서 확인한다.

## 16. 회의록 QA

### 목록

- [x] `/app/meeting-notes` 진입 가능
- [x] 회의록 목록이 로딩됨
- [x] 제목, 회의일시, source type, 연결 회사/담당자/제품/딜 요약이 보임
- [x] 검색이 동작함
- [x] 회사 필터가 동작함
- [x] 담당자 필터가 동작함
- [x] 정렬/페이지네이션이 동작함

### 수동 생성

- [x] 회의록 추가 버튼이 명확함
- [x] 제목 필수값 검증이 동작함
- [x] 상세 내용 필수값 검증이 동작함
- [x] 회의일시 입력 가능
- [x] 회사 연결 가능
- [x] 담당자 연결 가능
- [x] 제품 연결 가능
- [x] 딜 연결 가능
- [x] 다음 계획 입력 가능
- [x] 필요한 액션 입력 가능
- [x] 저장 후 목록/상세에 표시됨
- [x] 연결된 딜 상세에서 회의록 연결이 기대대로 보임

### AI draft

- [x] 텍스트 기반 AI 초안 버튼이 보임
- [x] 필요한 입력값이 없으면 실행이 막힘
- [N/A] `OPENAI_API_KEY`가 없는 환경에서 앱이 깨지지 않고 에러 안내가 보임. 이번 성공 케이스 기능 QA 범위에서 제외
- [x] AI 성공 시 초안 내용이 입력 영역에 반영됨
- [N/A] AI 실패 시 기존 입력값이 사라지지 않음. 이번 성공 케이스 기능 QA 범위에서 제외
- [x] 저장된 회의록에 불필요한 raw text가 노출되지 않음

### STT draft

- [x] 음성 파일 업로드 UI가 보임
- [N/A] 지원하지 않는 파일 형식이면 명확히 막힘. 이번 성공 케이스 기능 QA 범위에서 제외
- [N/A] 너무 큰 파일이면 명확히 막힘. 이번 성공 케이스 기능 QA 범위에서 제외
- [x] STT 성공 시 초안 내용이 반영됨
- [N/A] STT 실패 시 재시도 가능 상태가 보임. 이번 성공 케이스 기능 QA 범위에서 제외
- [x] 원문/오디오가 UI에 불필요하게 노출되지 않음

### 상세/수정/삭제/복구

- [x] 회의록 상세 진입 가능
- [x] 연결 회사/담당자/제품/딜 snapshot이 정확함
- [x] 회의록 수정 가능
- [x] 회의록에 딜 연결 추가 가능
- [x] 삭제 후 목록에서 사라짐
- [x] 삭제된 회의록이 휴지통에 보임
- [x] 휴지통에서 복구 가능
- [x] 복구 후 연결 관계가 기대대로 유지됨

### 2026-07-09 회의록 기능 QA 결과

- 배포 사이트 기준 회의록 목록, 검색/필터/정렬, 수동 생성, 연결 회사/담당자/제품/딜, AI/STT 성공 케이스, 상세, 수정, 딜 추가 연결, 삭제, 휴지통 복구 동작 확인 완료.
- OpenAI key 미설정, AI/STT provider 실패, 지원하지 않는 파일 형식, 대용량 파일 같은 예외 케이스는 이번 성공 케이스 기능 QA 범위에서 제외한다.


## 17. 검색 QA

- [x] 전역 검색 UI가 열림
- [x] 검색어 입력 시 로딩 상태가 보임
- [x] 회사 검색 결과가 보임
- [x] 담당자 검색 결과가 보임
- [x] 제품 검색 결과가 보임
- [x] 딜 검색 결과가 보임
- [x] 회의록 검색 결과가 보임
- [x] 결과 클릭 시 올바른 상세 화면으로 이동
- [x] 결과가 없을 때 empty state가 적절함
- [x] 짧은 검색어/공백 검색 처리가 자연스러움
- [x] 빠르게 입력해도 오래된 결과가 뒤늦게 덮어쓰지 않음
- [x] 삭제된 데이터가 검색 결과에 노출되지 않음

### 2026-07-10 검색 수동 QA 결과

- 전역 검색 UI, 검색어 입력, 회사/담당자/제품/딜/회의록 결과 표시, 결과 상세 이동이 동작함을 확인했다.
- 결과 없음, 짧은 검색어/공백 검색, 빠른 입력, 삭제 데이터 미노출 처리가 기대대로 동작함을 확인했다.

## 18. 휴지통 QA

### 목록/상세

- [x] `/app/trash` 진입 가능
- [x] 삭제된 항목 목록이 보임
- [x] 회사 삭제 항목이 보임
- [x] 담당자 삭제 항목이 보임
- [x] 제품 삭제 항목이 보임
- [x] 딜 삭제 항목이 보임
- [x] 회의록 삭제 항목이 보임
- [x] 타입 필터가 동작함
- [x] 삭제일/복구 가능 기한이 이해 가능하게 보임
- [x] 휴지통 상세 진입 가능

### 복구

- [x] 회사 복구 가능
- [x] 담당자 복구 가능
- [x] 제품 복구 가능
- [x] 딜 복구 가능
- [x] 회의록 복구 가능
- [x] 복구 후 원래 목록에 다시 보임
- [x] 복구 후 검색 결과에도 다시 보임
- [x] 복구 후 연결 관계가 기대대로 유지됨
- [N/A] 이미 복구된 항목을 다시 복구하려 하면 적절한 에러가 보임. 이번 수동 UI QA 범위에서 제외
- [N/A] 만료된 항목 복구 정책이 명확함. 이번 수동 UI QA 범위에서 제외

### 2026-07-10 휴지통 수동 QA 결과

- 회사/담당자/제품/딜/회의록 삭제 항목 조회, 타입 필터, 상세 진입, 복구 흐름이 동작함을 확인했다.
- 복구 후 원래 목록과 검색 결과에 다시 표시되고, 연결 관계가 기대대로 유지됨을 확인했다.
- 이미 복구된 항목 재복구와 만료 항목 복구 정책은 일반 UI에서 재현하기 어려워 이번 수동 UI QA 범위에서 제외한다.

## 19. 도메인별 XLSX Export QA


- [x] 회사 XLSX 다운로드 가능
- [x] 담당자 XLSX 다운로드 가능
- [x] 제품 XLSX 다운로드 가능
- [x] 딜 XLSX 다운로드 가능
- [x] 파일명이 도메인과 날짜를 이해할 수 있게 구성됨
- [x] 파일이 Excel 또는 호환 프로그램에서 열림
- [x] 헤더가 깨지지 않음
- [x] 한글이 깨지지 않음
- [x] 숫자/날짜 형식이 이해 가능함
- [x] 삭제된 데이터가 포함되지 않음
- [N/A] 다른 사용자의 데이터가 포함되지 않음. 이번 수동 UI QA 범위에서 제외
- [N/A] 대량 데이터에서도 다운로드가 실패하지 않음. 이번 수동 UI QA 범위에서 제외
- [N/A] export 실패 시 앱이 깨지지 않고 에러 안내가 보임. 이번 수동 UI QA 범위에서 제외

### 2026-07-10 도메인별 XLSX Export 수동 QA 결과

- 회사/담당자/제품/딜 목록에서 XLSX 다운로드가 동작함을 확인했다.
- 다운로드 파일명, 파일 열림, 헤더, 한글, 숫자/날짜 형식, 삭제 데이터 미포함이 기대대로 동작함을 확인했다.
- 다른 사용자 데이터 격리, 대량 데이터, export 실패 안내는 이번 수동 UI QA 범위에서 제외한다.

## 20. 설정/더보기 QA

- [x] 계정 메뉴 또는 `/app?account=settings` query로 계정 설정 모달 진입 가능
- [x] 내 프로필 조회 가능
- [x] 프로필 수정 가능
- [x] 수정 후 새로고침해도 유지됨
- [x] 내 기기 목록 조회 가능
- [x] 가입 locale/timezone과 마지막 로그인 locale/timezone이 표시됨
- [x] 가입 국가/마지막 로그인 국가는 geo header가 없는 환경에서 `기록 없음`으로 표시될 수 있음을 기록
- [x] Google Calendar 설정, follow-up delivery 설정가 계정 모달 `Settings` 안에서 동작함
- [x] `/app/settings`와 legacy `/settings` route는 사용자-facing route로 유지하지 않음
- [x] `/app/more` 진입 가능
- [x] 더보기에서 현재 노출해야 하는 기능만 보임
- [x] 로그아웃 진입점이 명확함
- [x] 로그아웃 후 선호 locale의 login URL로 이동함

### 2026-07-10 설정/더보기 수동 QA 결과

- 설정 화면 진입, 프로필 조회/수정, 새로고침 후 유지, 내 기기 목록, locale/timezone 표시가 동작함을 확인했다.
- geo header가 없는 환경에서는 가입 국가/마지막 로그인 국가가 `기록 없음`으로 표시될 수 있음을 확인했다.

### 2026-08-23 계정 설정 모달 이관 결과

- `TODO/DONE/ACCOUNT_SETTINGS_MODAL_PLAN` 기준으로 `/app/settings` 사용자-facing route는 제거됐고, 계정 설정 진입은 `/app?account=settings` modal-open 흐름을 기준으로 확인한다.
- Google Calendar callback과 follow-up email callback은 Settings modal 안에서 처리한다.

## 21. API 공통 QA

### 상태 코드

- [x] 정상 조회는 200
- [x] 정상 생성/수정/삭제가 현재 API 계약과 일치하는 status를 반환
- [x] validation 실패는 400
- [x] 인증 없음은 401
- [x] 권한 없음은 403
- [x] 존재하지 않는 리소스는 404
- [N/A] 삭제된 리소스 조회는 정책에 맞게 410 또는 삭제 상태 응답. 이번 무인 API smoke 범위에서 제외
- [x] 삭제된 리소스 수정/복구 conflict는 정책에 맞게 409

### 입력 검증

- [x] DTO whitelist가 적용되어 알 수 없는 필드가 거절됨
- [x] 필수값 누락 시 구체적인 validation message가 있음
- [x] 문자열 길이 제한이 동작함
- [x] 날짜 형식 검증이 동작함
- [x] 숫자 필드 검증이 동작함
- [x] 배열 관계 필드에 존재하지 않는 ID를 넣으면 실패함
- [x] 잘못된 enum 값이 실패함

### 페이지네이션/필터

- [x] page/pageSize 기본값이 동작함
- [x] 너무 작은 page/pageSize가 안전하게 처리됨
- [x] 너무 큰 pageSize가 제한됨
- [x] totalCount/totalPages가 실제 결과와 일치함
- [x] 검색어 trim 처리가 자연스러움
- [x] 필터 조합 결과가 정확함

### 트랜잭션/데이터 무결성

- [x] 딜 생성 중 관계 연결 실패 시 부분 데이터가 남지 않음
- [x] 삭제/복구 중 관계 데이터가 깨지지 않음
- [x] 메모 로그 삭제가 부모 리소스 상태와 충돌하지 않음
- [N/A] 동시에 저장 버튼을 두 번 눌러도 중복 생성되지 않음. 이번 무인 API smoke 범위에서 제외

### 2026-07-10 API 공통 QA 결과

- 실제 로컬 BE HTTP smoke로 `GET /api/health` 200, 보호 API 인증 없음 401, 잘못된 토큰 401, 존재하지 않는 route 404를 확인했다.
- BE 자동 테스트 17 suites / 82 tests passed 기준으로 controller status 계약, validation 400, DTO whitelist, enum/date 검증, pagination/filter 변환, AdminGuard 403, 휴지통 복구 conflict를 확인했다.
- 삭제된 리소스 직접 조회와 동시 저장 중복 방지는 별도 인증 세션/DB 상태 조작이 필요해 이번 무인 API smoke 범위에서 제외한다.

## 22. 보안/개인정보 QA

- [x] `.env` 값이 화면에 노출되지 않음
- [x] `.env` 값이 콘솔 로그에 노출되지 않음
- [x] access token이 URL query에 노출되지 않음
- [x] API 에러에 stack trace가 사용자 화면에 그대로 보이지 않음
- [N/A] 다른 사용자 데이터에 접근할 수 없음. 별도 다중 계정 보안 QA에서 확인
- [x] 개인 메모가 일반 메모와 구분됨
- [N/A] 민감할 수 있는 연락처/이메일이 불필요한 위치에 반복 노출되지 않음. 별도 개인정보 UX QA에서 확인
- [x] 브라우저 콘솔에 민감한 원문 데이터가 찍히지 않음
- [N/A] XLSX export에 다른 사용자 데이터가 섞이지 않음. 별도 다중 계정 보안 QA에서 확인
- [x] 로그아웃 후 뒤로가기로 보호 화면 내용을 볼 수 없음

### 2026-07-10 보안/개인정보 smoke 결과

- Playwright fresh context 기준 로그인 URL과 보호 라우트 redirect URL에 token-like query가 붙지 않음을 확인했다.
- 비로그인 로그인/보호 라우트 접근 시 앱 access token이 localStorage에 생성되지 않음을 확인했다.
- `localhost:5173` 기준 login/protected redirect smoke에서 browser page error와 console error/warning이 없음을 확인했다.
- FE/BE 소스에서 `console.*` 사용자 데이터 출력이 발견되지 않음을 확인했다.
- 실제 401/404 API 응답 body에 stack trace가 포함되지 않음을 확인했다.
- refresh token은 BE에서 httpOnly, sameSite=lax cookie로 설정되고 저장소에는 hash만 보관하는 구조임을 확인했다.

## 23. UX/UI 공통 QA

### 레이아웃

- [ ] 데스크톱 1440px에서 주요 화면이 깨지지 않음
- [ ] 노트북 1280px에서 주요 화면이 깨지지 않음
- [ ] 태블릿 768px에서 주요 화면이 사용 가능함
- [ ] 모바일 390px에서 주요 화면이 사용 가능함
- [ ] 모바일 360px에서 텍스트/버튼이 겹치지 않음
- [ ] 브라우저 확대 125%에서 핵심 흐름이 사용 가능함
- [ ] 긴 회사명/담당자명/제품명/딜이름이 레이아웃을 깨지 않음
- [ ] 긴 이메일/전화번호/URL이 부모 영역을 뚫지 않음
- [ ] 카드 안에 카드가 과하게 중첩되어 있지 않음
- [ ] 페이지 섹션이 불필요하게 떠 있는 카드처럼 보이지 않음

### 정보 구조

- [ ] 사용자가 화면에 들어왔을 때 현재 위치를 알 수 있음
- [ ] 주요 액션 버튼이 한눈에 보임
- [ ] 위험 액션은 일반 액션과 구분됨
- [ ] 목록에서 비교해야 할 값이 너무 숨겨져 있지 않음
- [ ] 상세 화면에서 기본 정보, 연결 정보, 메모/활동이 구분됨
- [ ] 복잡한 입력은 한 화면에 과하게 몰려 있지 않음
- [ ] empty state가 다음 행동을 안내함

### 상호작용 상태

- [ ] 버튼 클릭 후 로딩/disabled 상태가 보임
- [ ] 저장 성공 후 성공 피드백이 보임
- [ ] 저장 실패 후 실패 피드백이 보임
- [ ] 삭제 전 확인 dialog가 보임
- [ ] dialog 닫기 동작이 명확함
- [ ] 저장 중 dialog를 닫거나 이동할 때 데이터 손실 위험이 명확함
- [ ] 모달/드롭다운이 화면 밖으로 잘리지 않음
- [ ] toast가 중요한 버튼을 가리지 않음

### 접근성 기본

- [ ] Tab 키로 주요 입력/버튼 이동 가능
- [ ] 현재 focus 위치가 보임
- [ ] Enter/Escape 키가 dialog에서 자연스럽게 동작
- [ ] 입력 필드 label이 있음
- [ ] icon-only 버튼에는 tooltip 또는 aria-label이 있음
- [ ] 색상만으로 상태를 구분하지 않음
- [ ] 에러 메시지가 입력 필드와 가까이 표시됨
- [ ] 화면 리더가 읽기 어려운 장식 텍스트가 과하지 않음

### UX 라이팅

- [ ] 사용자 노출 문구가 해요체 중심임
- [ ] `없습니다`, `되었습니다`, `필요합니다` 같은 딱딱한 종결이 과하게 남아 있지 않음
- [ ] 버튼 문구가 짧은 행동형임
- [ ] validation message가 짧고 구체적임
- [ ] error message가 문제와 다음 행동을 알려줌
- [ ] empty state가 다음 행동을 알려줌
- [ ] `해주세요`가 아니라 `해 주세요`로 띄어쓰기 되어 있음
- [ ] `오프더레코드`, `상품`, `Customer`처럼 정본에서 제외된 표현이 사용자 화면에 남아 있지 않음

### 시각 톤

- [ ] 개인 영업 실무 도구처럼 조용하고 빠르게 읽히는 톤임
- [ ] Notion식 작업도구처럼 sidebar/page/database/detail 구조가 느껴짐
- [ ] Notion의 브랜드, 로고, 고유 문구, 고유 화면을 그대로 복제하지 않음
- [ ] 과한 마케팅형 hero/layout이 업무 화면에 들어오지 않음
- [ ] 화이트/중립 그레이 중심의 조용한 workspace tone이 유지됨
- [ ] hover/선택/navigation 상태가 파랑 중심이 아니라 neutral gray 중심으로 정돈됨
- [ ] 아이콘 컬러는 메뉴/도메인 인식 또는 상태 의미가 있을 때만 일관되게 사용됨
- [ ] 베이지/크림, 과한 다크 네이비, 과한 그라데이션이 화면을 지배하지 않음
- [ ] 중요한 값의 대비가 충분함
- [ ] 작은 보조 텍스트가 너무 낮은 대비로 밀려나지 않음
- [ ] 버튼/입력/탭/필터 스타일이 화면마다 일관됨

## 24. 모바일 브라우저 QA

이 섹션은 User Web의 모바일 브라우저 QA입니다. 현재 Web 제품이 작은 화면에서도 핵심 업무를 수행할 수 있는지 확인합니다.

- [ ] 로그인 화면이 모바일에서 사용 가능함
- [ ] 하단 또는 모바일 내비게이션이 명확함
- [ ] 모바일 하단 홈 링크가 로그인 화면이 아니라 `/app` 홈으로 연결됨
- [ ] 홈에서 오늘 일정과 다음 행동이 바로 보임
- [ ] 회사 목록 카드가 읽기 쉬움
- [ ] 담당자 전화/이메일 정보가 터치하기 쉬움
- [ ] 딜 단계 탭이 가로로 넘칠 때 사용 가능함
- [ ] 딜 목록이 테이블 때문에 깨지지 않음
- [ ] 일정 생성 form이 모바일에서 잘리지 않음
- [ ] 회의록 긴 입력이 모바일에서 사용 가능함
- [ ] 휴지통 복구 버튼이 실수로 누르기 어렵지 않음
- [ ] dialog가 작은 화면에서 화면 밖으로 벗어나지 않음
- [ ] 키보드가 올라와도 입력 중인 필드와 저장 버튼을 사용할 수 있음

## 25. 브라우저 QA

우선순위는 Chrome입니다. 가능하면 Edge까지 확인합니다.

- [ ] Chrome 최신 버전에서 핵심 시나리오 통과
- [ ] Edge 최신 버전에서 핵심 시나리오 통과
- [ ] 새로고침 후 상태 유지 확인
- [ ] 뒤로가기/앞으로가기 동작 확인
- [ ] 여러 탭에서 같은 데이터 수정 시 화면이 심하게 꼬이지 않음
- [ ] 네트워크 slow 3G 또는 throttling에서 로딩 상태가 보임

## 26. 알려진 한계 확인

아래는 현재 실패로 보지 않고, QA 결과에 `N/A` 또는 `Known limitation`으로 기록합니다.

- [ ] 과금/구독 결제는 현재 제품 QA 범위에서 제외
- [ ] MeetingNote raw text는 미래 범위
- [ ] Google/LINE/Apple 실제 provider smoke는 Supabase/provider 운영 설정과 secret 준비 후 별도 provider 설정/QA 필요
- [ ] 가입 국가/마지막 로그인 국가는 proxy geo header가 없는 환경에서 `기록 없음`일 수 있음
- [ ] 현재 User Web은 `mobile`/`personal_laptop` 두 device slot만 사용하며 모바일 여러 대 동시 active session은 보장하지 않음
- [ ] 현재 전화번호 입력/검증은 KR/US 1차 지원이며, KR/US/CA 우선 전략에 맞춘 CA 전화번호 모델은 후속 구현 대상

## 27. 버그 리포트 템플릿

버그를 발견하면 아래 형식으로 기록합니다.

```markdown
## BUG-001 제목

- 상태: Open
- 심각도: S2 Major
- 영역: FE/user-web > 딜 생성
- 발견일: 2026-07-06
- 환경: Chrome / Windows / localhost
- 계정: local.user@example.com

### 재현 절차

1. `/app/deals`로 이동
2. `딜 추가` 클릭
3. 딜이름과 금액 입력
4. 회사 선택
5. 저장 클릭

### 기대 결과

딜이 생성되고 상세 화면 또는 목록에 표시되어야 함.

### 실제 결과

저장 버튼을 눌러도 아무 반응이 없고 콘솔에 500 에러가 발생함.

### 증거

- 스크린샷: `파일명`
- 콘솔 에러:
- API 응답:

### 비고

- 새로고침 후에도 동일하게 재현됨
- 필수값을 모두 입력해도 재현됨
```

## 28. 최종 QA 완료 기준

2026-07-10 기준 1차 기능 QA 완료 조건은 아래와 같이 충족했다.

- [x] BE `typecheck`, `lint`, `test`, `build` 결과 기록 완료
- [x] FE/user-web `typecheck`, `lint`, `build`, `test:e2e` 결과 기록 완료
- [x] FE/admin-web 점검 `typecheck`, `lint`, `build` 결과 기록 완료. 현재 Admin Web route smoke E2E 결과는 최신 QA 실행 시 `SERVICE_QA_PLAN`에 기록
- [x] 핵심 수동 시나리오 1회 이상 완료
- [x] 회사/담당자/제품/딜/일정/회의록 CRUD 확인 완료
- [x] URL locale smoke 확인 완료
- [x] API/security 기본 smoke 확인 완료
- [x] 제외 범위와 알려진 한계가 QA 결과에 기록됨

출시 전 추가 품질 QA로 남은 조건은 아래와 같다.

- [ ] UX/UI 공통 QA 완료
- [ ] 모바일 브라우저 390px/360px 기준 핵심 시나리오 확인 완료
- [ ] Chrome/Edge 브라우저 QA 완료
- [ ] 다중 계정 보안 QA 완료
- [ ] DB/Prisma/migration 운영 정합성 정리
- [ ] 발견 버그가 심각도별로 정리됨
- [ ] S0/S1 버그가 없거나 모두 수정됨
- [ ] S2 버그가 출시/배포 판단에 맞게 정리됨
