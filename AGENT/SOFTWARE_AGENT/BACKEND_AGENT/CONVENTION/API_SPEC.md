...
API 계약의 위치, 상태, goal 실행 기준은 `API_CONTRACT.md`를 함께 따른다.
...
상태값은 `draft`, `confirmed`, `implemented`, `deprecated` 중 하나를 사용한다.
...
`TRANSACTION.md` 기준으로 transaction 필요 여부와 rollback 범위를 적는다.
...
- 필요 여부: 필요
- 이유: Company와 CompanyMemoLog를 같은 사용자 행동으로 생성한다.
