...
- Mobile App의 `deviceSlot`은 `native_mobile`이고 Backend Prisma enum은 `NATIVE_MOBILE`이다. 새 네이티브 모바일 기기 로그인은 기존 active native mobile device/session을 교체한다.
...
- 같은 active device 재로그인은 session row를 새로 만들지 않고 refresh token을 회전한다. Mobile App refresh token rotation은 기존 refresh token hash와 session id 조건이 모두 맞을 때만 성공해야 한다. 같은 slot의 다른 device login은 기존 device/session을 교체한다.
