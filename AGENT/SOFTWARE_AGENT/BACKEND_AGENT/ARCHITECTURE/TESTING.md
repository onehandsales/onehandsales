...
- Mobile App auth exchange/refresh/logout and one active native mobile device policy
...
Mobile App 인증 foundation API를 구현하는 경우에는 `/api/auth/mobile/exchange`, `/api/auth/mobile/refresh`, `/api/auth/mobile/logout`, `deviceSlot: "native_mobile"`, Prisma enum `NATIVE_MOBILE`, `replaceExistingDevice=true`, 조건부 refresh token rotation, 재사용 refresh token 실패, previous native mobile session revoke, `/api/me` 연동을 별도 Backend 테스트 범위로 추가한다.
