...
- 08 App I18N 목표는 public/auth URL locale i18n과 `/app` 내부 i18n을 분리하는 것이다. public-site i18n은 `FE/user-web/src/features/public-site/i18n`에 유지하고, `/app` 문구는 별도 app i18n feature에서 `User.preferredLocale` 기준으로 처리한다. `/app` route에는 locale prefix를 붙이지 않는다.
