...
MVP Frontend 테스트 자동화는 User Web 핵심 업무 흐름을 우선한다. Admin Web은 현재 auth/role smoke를 별도 Playwright 파일로 유지한다.
...
Admin Web은 현재 `typecheck`, `lint`, `build`와 auth/role smoke E2E를 검증 후보로 둔다. `FE/admin-web/tests/e2e/admin-web-smoke.spec.ts`는 현재 Admin router 기준으로 관리자 token 통과, non-admin 차단, `GET /admin/api/me` 호출을 확인한다.

현재 우선순위:

- admin login and role guard
- `GET /admin/api/me` route mock
...
- Admin Web Playwright: `FE/admin-web`. 현재 auth/role smoke 파일을 유지한다.

CI timing:

- Pull request: User Web smoke E2E. Admin Web은 route 변경 또는 auth 변경이 있을 때 auth/role smoke E2E를 실행한다.
...
- FE/admin-web `typecheck`, `lint`, `build` 선택 점검 통과.

2026-09-10 기준 FE/admin-web 검증 상태는 다음이다.

- FE/admin-web `typecheck`, `lint`, `test:e2e` 통과.
- FE/admin-web E2E는 현재 auth/role smoke 기준으로 유지한다.

남은 출시 전 Web Front QA는 기존 기능 유지보수, UX/UI 공통 QA, 모바일 브라우저 QA, Chrome/Edge 브라우저 QA, 결제창 없는 100명 베타 준비다.

Paddle checkout 관련 route smoke는 `TODO/PADDLE_PLAN`이 confirmed 된 이후에 추가한다.
