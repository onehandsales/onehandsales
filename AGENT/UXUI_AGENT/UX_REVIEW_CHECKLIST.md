...
- `/app/companies`에서 회사명, 분야, 지역, 주소, 메모 맥락을 빠르게 확인할 수 있는가?
- 회사 등록이 빠른 등록 흐름으로 가능하다면 최소 필드만 요구하는가?
...
- 회사 row/card 정보 순서가 회사명, 분야, 지역, 주소, 등록일 기준으로 스캔되는가?
- 데스크톱은 목록/테이블과 오른쪽 생성 패널이 함께 동작하는가?
- 모바일은 목록, 생성, 상세, 메모 입력이 겹치지 않게 흐르는가?
- 회사 메모와 개인 비밀 메모가 명확히 구분되는가?
- Contact/Product/Deal 정보가 현재 활성 화면처럼 노출되지 않는가?
...
- 회사 분야/지역 옵션이 없을 때 현재 구현 범위에 맞는 추가 흐름이 명확한가?
- inline option creation을 구현 범위에 포함한다면 전체 상세 form이 아니라 최소 생성 form인가?
