...
| #   | 심각도 | 위반 내용                                                                     | 영향 범위                              | 작업량 |
| --- | ------ | ----------------------------------------------------------------------------- | -------------------------------------- | ------ |
| A-1 | **H**  | 강조색 분열 — 모바일 전체가 보라(#5e5ce6), 데스크톱은 파랑(#2563eb)           | 모바일 전 화면                         | M      |
...
### A-1 [H] 강조색 분열 — 보라 #5e5ce6 vs 파랑 #2563eb

**위반 규칙**: 앱 전체에서 "primary / 활성" 강조 의미는 `#2563eb` 단일 색으로 표현 (PageHeader primary, 데스크톱 FilterChip active, 데스크톱 딜탭 active 모두 #2563eb 확인)

**증거 (실측)**

| 요소                     | 뷰포트 | 라우트                      | 실측 색                 |
| ------------------------ | ------ | --------------------------- | ----------------------- |
| PageHeader + 추가 버튼   | 1280px | 전 라우트                   | `#2563eb` ✅            |
| FilterChip "전체" active | 1280px | companies/contacts/deals/mn | `#1d4ed8` (blue-700) ✅ |
| DealTab active underline | 1280px | /deals                      | `#2563eb` ✅            |
...
**수정안**: FAB bg, 모바일 FilterChip active(bg/text/border), 모바일 DealTab active underline/text를 `#2563eb` 및 blue 계열(`#eaf2ff` bg, `#1d4ed8` text, `#c7d7fe` border)로 교체.
...
**위반 규칙**: 같은 primary 색상(`#2563eb`) 버튼끼리 radius가 통일되어야 함

**증거 (실측)**

| 컴포넌트                      | 위치             | border-radius | 배경      |
| ----------------------------- | ---------------- | ------------- | --------- |
| PageHeader primary (+ 버튼)   | 전 라우트 topbar | `6px`         | `#2563eb` |
| Button.primary.sm (저장 버튼) | /settings        | `8px`         | `#2563eb` |
...
**관찰**: DealTab은 `border-radius: 0px`, `border-bottom: 2px solid` 방식. 데스크톱에서는 #2563eb 파랑 하단 선이 충분히 구분됨. 모바일에서는 동일 구조에 #5e5ce6 보라 — A-1이 수정되면 이 문제도 함께 개선됨.
