...
2. 빠른 등록 모달에서 딜명, 회사, 담당자, 제품, 금액, 단계, 가능성을 입력한다.
...
pen 디자인 반영 완료/정리 도메인 (2026-06-15 기준):
...
- 현재 `/`는 딜 파이프라인이 아니라 준비중 화면이다. 딜 파이프라인은 `/deals`에서 운영한다.
...
- 좌측 사이드바
- 상단바
- 현재 로그인 후 `/`는 준비중 상태이며, 핵심 작업 화면은 `/deals` 딜 파이프라인이다.
...
- 회사/담당자 목록의 분야/지역/부서/직급 필터는 제품 category/status처럼 전체 옵션 API를 초회 조회한 select 필터로 제공한다.
- 목록 페이지에서는 분야/지역/부서/직급 생성/삭제 관리 UI를 제공하지 않는다.
