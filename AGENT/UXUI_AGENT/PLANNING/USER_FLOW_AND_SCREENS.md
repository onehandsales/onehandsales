...
5. 담당자 불러오기에서 새 회사 생성이 필요하면 보정값을 입력한다. 딜 불러오기에서 새 회사/담당자/제품 보정값 전달은 현재 코드 기준 FE API와 BE controller 검토가 필요하다.
6. Import를 확정한다.
...
- `/app/import` — 회사/담당자/제품/딜 양식 다운로드, CSV/XLSX 업로드, AI 컬럼 매핑, row 수정/검증, 확정 저장, 성공 내역 목록/상세 조회 구현 완료. 확정 전 job 이어받기는 후속이고, 딜 누락 참조 보정값 전달은 현재 코드 기준 검토 필요
