...
| 기능 | 현재 상태 | 판단 |
| --- | --- | --- |
| Admin Web | Backend는 `/admin/api/me`만 제공한다. Admin Web 운영 조회 route는 현재 root로 redirect하고 메뉴에서 숨긴다 | 관리자 페이지와 운영 조회 API는 후속 |
| Generic Export route | `features/import-export` 코드는 남아 있으나 User Web `/export`는 root로 redirect한다 | 현재 제품 정본이 아니며 신규 확장 금지 |
...
| 기능 | 현재 상태 | 후속 작업 |
| --- | --- | --- |
| BusinessCard OCR | FE feature는 남아 있으나 `/business-cards`, `/contacts/scan`은 구현된 화면으로 redirect하고 메뉴에서 숨긴다. Backend OCR module/API 없음 | OCR 업로드, 후보 추출, 확정 API 설계 및 구현 |
| Import | FE feature는 남아 있으나 `/import`는 root로 redirect하고 메뉴에서 숨긴다. 범용 Import job Backend 없음 | ImportJob, mapping, validation, confirm 흐름 설계 및 구현 |
| Notification | FE feature/page는 남아 있으나 `/notifications`는 root로 redirect하고 알림 진입 버튼은 숨긴다. Backend notification module/API 없음 | 알림 목록, 읽음 처리, unread count, 설정 API 설계 및 구현 |
