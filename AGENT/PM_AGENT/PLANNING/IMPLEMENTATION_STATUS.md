...
| DataImport | 완료 | 완료 | `ImportTemplate`, 회사/담당자/제품 양식 다운로드, CSV/XLSX 업로드, AI 컬럼 매핑, row 수정/검증, 확정 저장, `ImportUserLog` 목록/상세 |
...
| DataImport 제한 | 회사/담당자/제품 불러오기는 완료. 딜 불러오기는 validation error로 차단하고, 확정 전 임시 job은 in-memory store를 사용한다 | 딜 불러오기와 서버 재시작 후 job 복구는 후속 |
