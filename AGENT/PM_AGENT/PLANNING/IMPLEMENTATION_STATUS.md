...
| Public site | 해당 없음 | 완료 | `/`, `/pricing`, `/contact`, `/about`, `/security`, `/terms`, `/privacy` 공개 진입면 |
...
| DataImport | 완료 | 완료 | `ImportTemplate`, 회사/담당자/제품/딜 양식 다운로드, CSV/XLSX 업로드, AI 컬럼 매핑, row 수정/검증, 확정 저장, `ImportUserLog` 목록/상세 |
...
| DataImport 제한 | 회사/담당자/제품/딜 불러오기는 구현되어 있다. 확정 전 임시 job은 in-memory store를 사용한다 | 서버 재시작 후 job 복구는 후속 |
| DataImport 딜 보정값 | 딜 import 누락 회사/담당자/제품 보정 배열은 type/use case에 있으나 현재 FE API 함수와 BE controller confirm 전달 경로 검토가 필요하다 | 릴리즈 전 검증/수정 필요 |
