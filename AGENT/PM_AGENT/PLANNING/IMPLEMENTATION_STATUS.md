...
| 도메인 | API | 정본 액션명 |
| --- | --- | --- |
| Company | `GET /api/companies/export/xlsx` | `회사 엑셀 다운로드` |
| Contact | `GET /api/contacts/export/xlsx` | `담당자 엑셀 다운로드` |
| Product | `GET /api/products/export/xlsx` | `제품 엑셀 다운로드` |
| Deal | `GET /api/deals/export/xlsx` | `딜 엑셀 다운로드` |

현재 FE icon action은 공통 tooltip/aria-label `액셀 다운로드`를 사용한다. UI 문구를 정본 액션명과 맞추는 작업은 별도 FE 라벨 정렬 범위로 본다.
...
| Export UI label | 도메인별 API 호출은 완료, 표시 라벨은 공통 `액셀 다운로드` | 정본 액션명과 맞추려면 별도 FE 작업 필요 |
