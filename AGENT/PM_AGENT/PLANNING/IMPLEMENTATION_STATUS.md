...
`TODO/DONE/GLOBAL_B2C_FEATURE_ROADMAP_PLAN` 기준 User Web 중심 기능 선구현 로드맵은 완료 archive로 둔다. `TODO/DONE/NEXT_BACKEND_API_BACKLOG_PLAN`과 `TODO/DONE/USER_WEB_PRODUCTIZATION_GAP_PLAN`도 완료/이관 archive다.

01~10 각 완료 기능의 세부 범위, API/DB/FE 경계, 제외 범위는 `GLOBAL_B2C_01_11_FEATURE_CATALOG.md`를 따른다.
...
| Admin auth | 완료 | 완료 | `GET /admin/api/me`, AdminGuard, Admin Web admin/non-admin 보호 route |
...
| Global Data / I18N | 완료 | 완료 | 사용자 global settings, `/app` i18n foundation, currency/phone/region/address, export localization, Google/LINE/Apple auth foundation |
| Product Analytics | 완료 | 완료 | `ProductAnalyticsEvent`, activation snapshot, retention cohort snapshot, server/client event foundation |
| Mobile Field Use | 완료 | 완료 | 모바일 브라우저 현장 입력성, meeting note mobile recording, local draft, mobile analytics event foundation |
...
- FE/admin-web `typecheck`, `lint`, `build`, `test:e2e` 통과. 현재 Admin auth/role smoke E2E 기준이다.
...
| Permanent delete API | 사용자 즉시 완전 삭제 API 없음 | 보존 정책과 권한 정책 확정 후 구현 |
| Sensitive export | 민감 데이터 포함 export 없음 | 마스킹, 경고, 권한 정책과 함께 구현 |

## 5A. 글로벌 B2C 유료 판매/Series A 관점의 미완성 영역

2026-09-10 기준으로, User Web 기능 foundation은 완료됐지만 결제/구독/세금과 베타 검증 전이므로 아직 글로벌 B2C 유료 판매나 Series A급 제품/사업 상태로 보지 않는다.
...
| 제품 분석 | 09 Product Analytics foundation 완료 | paid conversion, churn, ARPU, LTV/CAC, billing funnel, AI cost/user는 Paddle/Billing 이후 확정 |
| 정책/신뢰 문서 | 기본 문서 있음 | 환불, 개인정보, 보안 문구의 실제 판매 범위 정합성 |
