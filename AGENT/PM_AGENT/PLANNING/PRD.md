# PRD

> 제품명: 한손에 영업 / onehand.sales  
> 상태: MVP 정본 초안  
> 기준: `AGENT/PM_AGENT/DECISIONS/000_확정_결정.md`

---

## 1. 제품 정의

한손에 영업은 B2C 개인 영업자를 위한 영업 업무 관리 도구다.

영업자가 회사 시스템이나 개인 메모장, 엑셀, 명함, 캘린더에 흩어진 영업 정보를 한곳에 모아 관리하고, 거래처 앞에서 필요한 정보를 빠르게 확인하며, 딜/일정/회의록을 개인 기준으로 정리할 수 있게 한다.

## 2. 주 타겟

- 30~50대 남성 영업직
- 특히 40~50대 비중이 높을 것으로 가정
- 회사/팀이 도입하는 솔루션이 아니라 영업자 개인이 직접 사용하는 B2C 도구
- 초기 가격 가설: 월 5,900~6,900원

## 3. 핵심 문제

- 회사, 담당자, 제품, 딜 정보가 흩어져 있다.
- 영업자가 개인적으로 기억해야 하는 맥락을 공식 시스템에 남기기 어렵다.
- 딜 금액, 진행 상태, 후속 조치, 일정이 한 화면에서 연결되지 않는다.
- 주간 일정 보고를 위해 캘린더/엑셀/메모를 다시 정리해야 한다.
- 회의 후 정리한 내용을 딜 활동으로 연결하는 과정이 번거롭다.
- 명함 기반 담당자 입력이 귀찮고 누락되기 쉽다.

## 4. 핵심 가치

1. 회사/거래처/제품/딜을 개인 기준으로 통합 관리한다.
2. 딜 금액, 가능성, 단계, 활동 기록을 한곳에서 관리한다.
3. 일정을 딜/회사/거래처와 연결하고 주간 보고서로 출력한다.
4. 회의 내용을 AI 회의록으로 정리하고 딜 활동 로그에 연결한다.
5. 명함 OCR과 AI Import 매핑으로 초기 입력 비용을 줄인다.

## 5. MVP 핵심 루프

1. 회사/거래처/제품을 등록한다.
2. 회사/거래처/제품을 기반으로 딜을 생성한다.
3. 딜 단계, 금액, 가능성, 활동 로그를 관리한다.
4. 일정을 딜/회사/거래처와 연결한다.
5. 주간 일정 보고서를 화면/PDF/Excel로 확인한다.
6. 회의 내용을 텍스트로 입력해 AI 회의록을 생성한다.
7. 회의록을 딜과 연결해 딜 활동 로그를 자동 생성한다.

## 6. MVP 포함 기능

- 소셜 인증: 카카오, 구글, 네이버, 애플
- 회사 관리
- 거래처(담당자) 관리
- 제품 관리
- 제품 연결 타입 관리
- 딜 관리
- 딜 활동 로그
- 일정 관리
- 구글 캘린더 일정 가져오기
- 주간 일정 보고서 화면/PDF/Excel 출력
- 알림: 이메일, 브라우저 푸시
- 회의록 텍스트 입력 기반 AI 생성
- 회의록 딜 연결
- 명함 이미지 업로드 OCR
- Excel/CSV Import AI 컬럼 매핑
- Excel/PDF Export
- 태그 관리
- 개인 메모
- 민감정보 저장 경고
- 휴지통 30일 보관
- 최소 Admin 웹

## 7. MVP 제외 기능

- 결제/구독 자동화
- 모바일 앱
- 음성 녹음/STT
- 구글 캘린더 양방향 동기화
- 우리 서비스 일정의 구글 캘린더 내보내기
- 카카오 알림톡
- 사용자 커스텀 필드 UI
- 다중 통화
- 자동 민감정보 키워드/패턴 감지
- 팀 공유/협업

## 8. 결제 전략

MVP에는 결제를 붙이지 않는다.

MVP 이후 초기 유료 운영은 계좌이체 수동 처리로 시작한다. 약 1~2개월 동안 유지보수와 고객 니즈를 반영한 뒤 웹과 앱 모두에 구독형 결제를 도입한다.

## 9. 성공 기준

- 사용자가 회사/거래처/제품을 등록하고 딜을 만들 수 있다.
- 딜에 금액, 가능성, 단계, 활동 로그를 누적할 수 있다.
- 일정이 딜/회사/거래처와 연결되고 주간 보고서로 출력된다.
- 명함 OCR 결과를 확인/수정 후 회사/거래처로 저장할 수 있다.
- AI 회의록을 생성하고 딜 활동 로그로 연결할 수 있다.
- 민감 데이터는 기본적으로 보호되고 Export/Admin 조회 시 경고/마스킹/감사 로그 정책이 적용된다.


