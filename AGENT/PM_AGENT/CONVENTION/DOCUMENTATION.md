...
제품 방향은 저장소 루트 `README.md`를 최상위 기준으로 삼는다. `AGENT` 문서는 루트 제품 방향을 역할별 실행 기준으로 구체화한다. 루트 `README.md`와 역할 문서가 충돌하면 먼저 충돌을 기록하고, PM 결정 문서에서 최종 기준을 남긴다.
...
- `OneHand Sales`, `onehand.sales`는 과거 명칭 또는 역사적 문맥이 아니면 사용하지 않는다.
- `Customer`를 조직과 담당자를 모두 포함하는 모호한 전역 도메인 용어로 사용하지 않는다.
- `Company`, `Product`, `Deal`을 OneHand CRM 전체의 전역 기본 도메인처럼 쓰지 않는다.
- `담당자`, `제품`, `딜`, `상품`은 현재 활성 구현 도메인이 아니므로 현재 구현 범위 문서에서 활성 기능처럼 쓰지 않는다.
- 특정 직군 Kit 안에서는 `회사`, `상품`, `계약`, `딜` 같은 업무 언어를 사용할 수 있다.
- MVP UI 문맥에서 `오프더레코드` 표현을 사용하지 않는다.

문서에서 내부 개념과 사용자-facing 표현을 구분한다.

| 내부 개념 | 사용자-facing 표현 예 |
| --- | --- |
| Workspace | 내 CRM, 내 업무 공간 |
| Object | 관리할 항목, 고객, 매물, 후보자 |
| Attribute | 필요한 정보 |
| Relationship | 기록 연결 |
| Record | 업무 기록 |
| View | 보기 방식 |

PM 문서에서는 내부 개념을 정의할 수 있지만, 사용자 경험을 설명할 때는 업무 언어를 우선한다.
...
- TODO 작성자는 Software Agent의 아키텍처, modular monolith/MSA 대비 module boundary, API 명세, API 계약, transaction, observability, Backend/Frontend/Admin 컨벤션, DB schema, 테스트, 배포, 주석/로그 규칙 중 해당 계획에 영향을 주는 기준을 TODO 문서에 구체적으로 옮긴다.
...
- Backend는 API 명세, 비즈니스 로직, DB 스키마, 권한, structured log, 외부 Provider adapter를 책임진다.
...
- API가 포함된 TODO 문서에는 계약 상태, 소비자, 요청값 형태, 응답값 형태, 내부 비즈니스 로직, 연결 DB 스키마, transaction, idempotency/outbox 필요 여부, observability, 에러 응답, FE/BE 처리 기준을 반드시 상세하게 적는다.
...
- 내부 비즈니스 로직은 인증, 권한, ownership, validation 이후 흐름, transaction, 외부 Provider 호출, 자동 생성 데이터, 암호화, 에러 분기를 use case 흐름으로 적는다.
- transaction은 필요 여부, 이유, transaction model, rollback 범위, 부수 로그/이력 포함 여부, 외부 Provider 호출 위치를 적는다.
- module 간 협력이 필요한 경우 owning module, application/query port 또는 use case facade, orchestration owner를 적는다.
- observability는 log event key, request id, redaction 기준, provider error context를 적는다.
...
- TODO 문서가 SOFTWARE_AGENT의 Clean Architecture, DDD, domain/application/infrastructure/presentation 계층, port/adapter, Prisma infrastructure-only, modular monolith/MSA 대비 module boundary, User API와 관리자 확인 API 분리, API contract, transaction, idempotency/outbox, observability 규칙을 구체 문서로 옮겼는지 확인한다.
...
## 14. Goal 완료 후 검토와 TODO_LOG 기록 규칙

특정 `/goal` 또는 명시적인 작업 단위가 끝나면 AI 작업자는 바로 다음 작업으로 넘어가기 전에 사용자에게 다음 질문을 한다.
...
- 사용자가 검토를 요청하면 완료된 작업의 적용 범위, 변경 파일, 검증 명령, 실패/보류 항목, 남은 리스크를 검토한다.
- 검토 결과는 `통과`, `조건부 통과`, `수정 필요`, `보류` 중 하나로 판정한다.
- 검토가 `통과` 또는 사용자가 수용한 `조건부 통과`가 된 뒤에만 `TODO_LOG`에 작업 기록을 남긴다.
- `TODO_LOG`는 `TODO_LOG/YYYY-MM-DD/` 날짜 폴더 아래에 작성한다.
- 로그 파일명은 계획명과 goal 또는 작업명을 알 수 있게 작성한다.
- 로그에는 작업명, 작업 일자, 관련 계획, 적용 범위, 변경 파일, 검증 결과, 검토 결과, 남은 리스크, 다음 권장 작업을 포함한다.
- 실패한 작업은 통과한 작업처럼 기록하지 않고 실패/보류 상태와 다음 조치를 명확히 남긴다.
- 비밀값, access token, service role key, 개인정보 원문은 `TODO_LOG`에 기록하지 않는다.
...
- 검토가 통과됐다면 `TODO_LOG/YYYY-MM-DD/` 아래에 작업 기록을 남겼는가?
