...
OneHand CRM의 현재 인증은 Supabase OAuth를 외부 identity provider로 사용하고, 실제 앱 사용자/기기/session은 Backend가 별도로 관리한다.
...
인증 QA 이후의 다음 제품 QA는 로그인 후 foundation 플로우다.

User Web QA:

1. `/app` 빈 홈 진입
2. `/app/more` 진입
3. 계정 설정 모달의 프로필/환경설정/기기 조회
4. 오류 신고와 지원 요청
5. 공개 문의 접수
