...
export class ContactApplicationService {
  ...
  // 기능 : 현재 사용자의 담당자 목록을 15개 단위 페이지로 조회합니다.
  async listContacts(
  ...
  async exportContactsXlsx(
    ...
    await this.recordServerAnalyticsEvent({
      ...
      idempotencyKey: `export_downloaded:${currentUser.id}:CONTACT:${requestId ?? "internal"}`,
      targetType: "EXPORT",
      targetId: null,
      payload: {
        exportType: "CONTACT",
        rowCountBucket: toProductAnalyticsExportRowCountBucket(contacts.length),
  ...
  // 기능 : 담당자 서버 이벤트를 best-effort로 기록해 본 API 응답을 막지 않습니다.
  private async recordServerAnalyticsEvent(
    ...
    await recordProductAnalyticsServerEventBestEffort({
      ...
      logger: this.logger,
      command,
      logContext: "ContactApplicationService",
    });
  }
