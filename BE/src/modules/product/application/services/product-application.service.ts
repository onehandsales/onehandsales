...
export class ProductApplicationService {
  ...
  async exportProductsXlsx(
    ...
    await this.recordServerAnalyticsEvent({
      ...
      idempotencyKey: `export_downloaded:${currentUser.id}:PRODUCT:${requestId ?? "internal"}`,
      targetType: "EXPORT",
      targetId: null,
      payload: {
        exportType: "PRODUCT",
        rowCountBucket: toProductAnalyticsExportRowCountBucket(products.length),
  ...
  // 기능 : 상품 서버 이벤트를 best-effort로 기록해 본 API 응답을 막지 않습니다.
  private async recordServerAnalyticsEvent(
    ...
    await recordProductAnalyticsServerEventBestEffort({
      ...
      logger: this.logger,
      command,
      logContext: "ProductApplicationService",
    });
  }
