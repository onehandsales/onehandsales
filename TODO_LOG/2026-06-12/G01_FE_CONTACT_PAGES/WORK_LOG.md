...
User Web 거래처 목록/상세/생성/메모/내보내기 화면 구현
...
- 거래처 삭제/복구/휴지통 UI를 모두 제거했다.
- xlsx export를 blob 다운로드로 처리하고 Content-Disposition 파일명을 우선 사용했다.
- deal/product/schedule의 거래처 관련 표시 필드를 새 계약에 맞게 보정했다.
...
- 딜 생성 모달의 인라인 거래처 생성은 새 거래처 필수값(부서/직급 ID) 정책으로 인해 제거됨. 별도 UX가 필요하면 미니 거래처 생성 모달을 별도 설계한다.
- 거래처 회사 옵션(`/api/contacts/company-options`)은 전체 목록을 반환하므로 회사가 매우 많을 경우 성능 확인 필요.
