...
- 기존 Deal 검색 필드와 회사/거래처/딜 API 패턴을 재사용해 연결 UI를 구성한다.
...
- 일정 생성/수정 dialog에 제목, 시작/종료일시, 종일, 장소, 딜/회사/거래처 검색 연결, 알림 시간, 메모 입력을 추가했다.
...
- 일정은 딜 없이 생성 가능하며, 딜 선택 시 회사/거래처 입력이 기본 상속되도록 했다.
