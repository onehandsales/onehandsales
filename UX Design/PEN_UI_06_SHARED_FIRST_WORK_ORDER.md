...
- BusinessCard, 범용 Import job, Notification은 FE feature/route가 있어도 Backend module이 없어 후속 범위다. Trash와 Company/Contact/Product/Deal 도메인별 xlsx export는 현재 구현 완료 상태이며, 범용 ExportJob은 비정본이다.
...
- Admin 운영 조회, BusinessCard, Import, Notification은 Backend 부재가 분명하므로 별도 계획 없이 FE만 확장하면 계약 불일치가 커진다. Search와 Trash는 현재 구현 완료 기준으로 유지보수하고, 범용 ExportJob은 신규 확장하지 않는다.
