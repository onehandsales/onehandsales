...
- 딜/회사/거래처/제품 화면이 동일한 card/button/filter 문법을 공유할 수 있어야 한다.
...
- 다음 행동, 금액, 단계, 가능성의 표시 문법이 고정돼야 한다.
...
1. `ModalShell + 공용 상태 UI` 정리
2. `Deal Quick Create Modal` pen 기준 재구성
3. `Company 도메인` 시작

이 순서를 권장하는 이유:

- 공용 modal/state가 먼저 있어야 quick create와 company create가 중복 없이 붙는다.
- quick create가 딜 기준 modal 문법을 고정해 주고,
- company가 그 다음 공용 기반을 실제로 재사용하는 첫 도메인이 된다.
