...
- 플로우 기준:
  ...
  - 현재 User Web `CreateMeetingNoteInput`과 form 변환은 `MANUAL`, `TEXT_AI`, `STT_AI` 저장 경로를 지원한다.
  - 저장 후 딜 연동은 회의록 상세에서 별도 액션으로 수행하며, 신규 연결마다 `MeetingNoteDeal`과 `DealFollowingActionLog`를 생성한다.
...
- `데이터 업로드`와 `휴지통`은 sidebar에 노출되는 구현 완료 화면이다. `/export`, `/notifications`는 현재 숨김/redirect 상태다.
...
- `sourceType`: Backend 생성 API는 `MANUAL`, `TEXT_AI`, `STT_AI`를 허용한다. 현재 User Web 생성 form은 직접 저장 시 `MANUAL`, AI/STT 초안 저장 시 `TEXT_AI`/`STT_AI`를 전송한다.
...
| AI/STT draft endpoint | 존재 | 유지 | `ai-draft`, `stt-draft` Backend 구현됨 |
| AI/STT User Web 저장 연결 | 존재 | 유지 | 기존 생성 API에 `TEXT_AI`/`STT_AI` sourceType 전달 |
| 저장 후 딜 활동기록 생성 | 존재 | 유지 | `POST /api/meeting-notes/:meetingNoteId/deals`, `MeetingNoteDeal` + `DealFollowingActionLog` |
...
DataImport Backend module은 구현되어 있다. User Web `/import`와 `features/import-export`는 `/api/import-templates`, `/api/imports`, `/api/import-user-logs`에 연결되며 회사/담당자/제품/딜 업로드와 확정 저장을 지원한다. ImportJob은 현재 서버 메모리 기반이므로 새로고침/재시작 이후 이어받는 영속화는 후속이다. `/export`와 범용 ExportJob Backend module은 정본 흐름으로 사용하지 않는다.
...
| 항목 | 현재 상태 | 추적 상태 | 메모 |
|---|---|---|---|
| job 기반 흐름 | 존재 | 유지 | 서버 메모리 기반. 영속화/재개는 후속 |
| 도메인별 xlsx export | 존재 | 유지 | Company/Contact/Product/Deal |
| import mapping | 존재 | 유지 | AI mapping, 수동 mapping, confirm |
...
- Backend `trash` module과 Company/Contact/Product/Deal/MeetingNote soft delete 기반 휴지통 조회/복구 API가 구현되어 있다. Schedule은 soft delete 없이 삭제한다. Permanent delete와 7일 이후 유료 복구는 후속 범위다.

추적:

| 항목 | 현재 상태 | 추적 상태 | 메모 |
|---|---|---|---|
| 기본 흐름 | Backend 있음 | 유지 | Company/Contact/Product/Deal/MeetingNote |

### Business Card

현재 상태:
- User Web `/business-cards`, `features/business-card`는 Backend와 연결되어 있다. `/contacts/scan`은 `/business-cards`로 redirect된다.
- Backend `business-card` module과 `/api/business-card-scans` OCR/확정 endpoint가 존재한다.

추적:

| 항목 | 현재 상태 | 추적 상태 | 메모 |
|---|---|---|---|
| OCR 상태 흐름 | Backend 있음 | 유지 | `/api/business-card-scans` |
| 후보 회사 응답 | Backend 있음 | 유지 | confirm 시 회사/담당자 생성/연결 |
...
- soft delete 필드:
  ...
  - 현재 Trash는 Company/Contact/Product/Deal/MeetingNote 삭제 목록과 7일 이내 복구 기준으로 구현되어 있다. Schedule은 soft delete 없이 삭제한다. Permanent delete와 유료 복구는 후속 범위다.
...
- 상태:
  ...
  - 당시 기준으로 BusinessCard/범용 Import/Notification은 FE feature만 있고 Backend module이 없는 상태로 정정. 이후 2026-07-03 기준 BusinessCard, DataImport, Trash는 구현 완료 상태이며, Notification과 범용 ExportJob은 후속/비정본으로 재정정.
...
- 상태:
  ...
  - MeetingNote AI/STT 초안 endpoint와 User Web draft UI 연결 상태로 정정
  - MeetingNote 직접 작성 저장을 기본 흐름으로 두고, AI/STT는 자동 저장 없는 초안 보조 액션으로 정정
  - User Web `CreateMeetingNoteInput`의 sourceType을 `MANUAL`/`TEXT_AI`/`STT_AI` 저장까지 확장한 상태로 반영
  - 저장 후 딜 활동기록 자동 생성은 AI/STT draft API 밖의 `POST /api/meeting-notes/:meetingNoteId/deals` 계약으로 정정
...
2. Notification/Admin 운영 API/ImportJob 영속화 계획. BusinessCard, DataImport, Trash는 구현 완료, 범용 ExportJob은 비정본.
