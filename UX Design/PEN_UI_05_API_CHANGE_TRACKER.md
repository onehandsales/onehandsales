...
범용 Import job Backend module은 현재 없다. 범용 ExportJob Backend module도 정본 흐름으로 사용하지 않는다. User Web에는 `/import`, `/export` 라우트와 `features/import-export` API client/type/schema가 남아 있지만 실제 Backend endpoint와 연결된 완료 기능으로 보지 않는다.
...
- 상태:
  ...
  - BusinessCard/범용 Import/Notification은 FE feature만 있고 Backend module이 없는 상태로 정정. 당시 Search도 FE-only로 기록했으나 2026-06-19에 구현 상태로 재정정. 현재 Trash와 Company/Contact/Product/Deal 도메인별 xlsx export는 구현 완료 상태이며, 범용 ExportJob은 비정본.
...
2. BusinessCard/Import/Notification Backend module 계획. Trash는 구현 완료, 범용 ExportJob은 비정본.
