...
- 숨김/redirect 처리된 라우트를 다시 노출할 때는 navigation 문서와 함께 갱신한다. Import와 Trash는 현재 navigation에 노출되는 구현 완료 화면이며, Export와 Notification은 숨김/redirect 상태다.
...
- 명함 스캔/Import/휴지통은 구현 완료 상태로 재분류한다. Export는 도메인별 xlsx 다운로드만 정본으로 본다.
- 알림 / 더보기 고도화
...
- Import는 회사/담당자/제품/딜 확정 저장 구현 기준을 따른다. ImportJob 영속화/재개와 범용 ExportJob은 후속/비정본 흐름으로 분리한다.
...
- stage metadata endpoint 필요 여부
- Admin 운영 조회 API 범위
- Notification/Admin 운영 API/ImportJob 영속화 구현 범위. BusinessCard, DataImport, Trash는 구현 완료, 범용 ExportJob은 비정본.
- Search 고도화 범위
...
- 딜 목록 필터는 stage tab과 별도로 `딜이름 검색`, `전체`, `회사`, `담당자`, 정렬 select를 사용한다.
